...
include 'ogsmout'
include 'tcs45q'
include 'indout'		! ONL read price and quantity from cement CCS 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! ONL MOD for Cement CCS
!
! Unit-level cement CO2 data, coming from IDM, will be read into OGSM through 
! the existing channel of WLCO2.
! 
! Existing CO2 source variables, to which cement CO2 data will go:
!	NS_SRC
!	NS_BIN
!	N_NS_RECS
!	NS_PRICE
...
!	NS_START
!
! CO2 variables from IDM:
! REAL INCM_CCSPOTQ(150,MNUMYR)		! Potential CCS quantity by kiln (KT CO2)
! INTEGER INCM_CCSREG(150)			! OGSM region where kiln is located
! REAL I_RY(150)					! First CCS year by Kiln
! REAL INCM_CO2BEP(150,MNUMYR)		! Breakeven price for CO2 to EOR by kiln
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

! Append CO2 data to the end for each variable
DO I_INDX = 1, MAX_KILNS

IF(INCM_CCSREG(I_INDX).EQ.0) EXIT

T_REG=INCM_CCSREG(I_INDX)

! Add to the number of CO2 sources by OGSM region
N_NS_RECS(T_REG)=N_NS_RECS(T_REG)+1
J_INDX=N_NS_RECS(T_REG)

! Assign source type. All these sources are from Cement (isrc=5)
NS_SRC(T_REG,J_INDX)=5

! Append the quantity data: NS_QUANT is in MMT CO2
NS_QUANT(T_REG,J_INDX)=INCM_CCSPOTQ(I_INDX,CURIYR)/1000.0

! ! Cement data starts in 2026
! ! If CURIYR <= 36(2025), get CCSPOTQ in (37)2026
! ! If CURIYR >= 37(2026), get CCSPOTQ from the CURIYR slot
! IF (CURIYR .LE. 36) THEN 
    ! NS_QUANT(T_REG,J_INDX)=0.0
! ELSE
    ! NS_QUANT(T_REG,J_INDX)=INCM_CCSPOTQ(I_INDX,CURIYR)/1000.0
! ENDIF

! Append data on starting year with selling CO2 to EOR
NS_START(T_REG,J_INDX)=I_RY(I_INDX)

! Append the price data
DO YR=1,MNUMYR
    IF (YR .LE. CURIYR) THEN
        IF (INCM_CO2BEP(I_INDX,YR) .GT. 0.0001) THEN
            NS_PRICE(T_REG,J_INDX,YR)=INCM_CO2BEP(I_INDX,YR)
        ELSE
            NS_PRICE(T_REG,J_INDX,YR)=9999.9
        ENDIF
    ELSE
        ! Use value in curiyr for every year afterwards
        NS_PRICE(T_REG,J_INDX,YR)=INCM_CO2BEP(I_INDX,CURIYR)
    ENDIF

    ! IF (CURIYR .LE. 36) THEN
        ! NS_PRICE(T_REG,J_INDX,YR)=9999.9
    ! ELSEIF (YR .LE. CURIYR) THEN
        ! IF (INCM_CO2BEP(I_INDX,YR) .GT. 0.0001) THEN
            ! NS_PRICE(T_REG,J_INDX,YR)=INCM_CO2BEP(I_INDX,YR)
        ! ELSE
            ! NS_PRICE(T_REG,J_INDX,YR)=9999.9
    ...
    ! Search for the bin number based on price and end point definition
    NS_BIN(T_REG, J_INDX, YR) = 8
    Do T_INDX=1,7
        IF (BIN_DOLAMT(T_INDX) .GT. NS_PRICE(T_REG,J_INDX,YR)) THEN 
...
T_REG=1
write(6778,*)'===================================='
write(6778,*)'READ_CO2 curiyr = ',curiyr, ' CCSREG = ',INCM_CCSREG(:)

write(6778,*)'===================================='
DO J_INDX=1,N_NS_RECS(T_REG)
write(6778,*)'READ_CO2 pre45Q curiyr = ',curiyr,' REC = ',J_INDX,' NS_PRICE',NS_PRICE(T_REG,J_INDX,:)
ENDDO

write(6782,*)'===================================='
DO T_REG=1,MAX_REG-1
write(6782,*)'READ_CO2 curiyr = ',curiyr,' REG = ',T_REG,' NS_QUANT',NS_QUANT(T_REG,:)
ENDDO

write(6783,*)'===================================='
write(6783,*)'READ_CO2 curiyr = ',curiyr,' INCM_CCSPOTQ',INCM_CCSPOTQ(:,CURIYR)
...
DO T_REG=1,MAX_REG
    do YR = 1, MNUMYR
        ...
        IF (N_NS_RECS(T_REG).gt.0) THEN
            ...
            if(N_NS_RECS(T_REG) .gt. 1) CALL WELLHEAPSORT(N_NS_RECS(T_REG),HOLDIT,SINDEX) ! ONL debug: bypass sorting if a region only has one record, which will cause error in wellheapsort.
...
T_REG=1
DO J_INDX=1,N_NS_RECS(T_REG)
    write(6778,*)'READ_CO2 aft45Q curiyr = ',curiyr,' REC = ',J_INDX,' NS_PRICE',NS_PRICE(T_REG,J_INDX,:)
ENDDO
...
! ONL MOD
! Use these temporary variables to dynamically update remaining CO2 in nat_availco2
real nat_availco2_prev(max_reg,max_src,max_yr)
real bse_availco2_prev(max_reg,max_src,max_yr)
real src_availco2_prev(max_reg,max_src,max_yr)
...
! ONL MOD: Pass remaining CO2 and total CO2 to the temporary variables, once in the analysis period
! Skip when SETUP_CONSTRAINTS is called for the first time
if (itimeyr.ge.1) then
  do ireg = 1 , max_reg - 1
     do isrc = 1, max_src               
        do iyr = 1, max_yr
           nat_availco2_prev(ireg,isrc,iyr) = nat_availco2(ireg,isrc,iyr)
           bse_availco2_prev(ireg,isrc,iyr) = bse_availco2(ireg,isrc,iyr)
           src_availco2_prev(ireg,isrc,iyr) = src_availco2(ireg,isrc,iyr)
        end do
     end do
  end do
endif
...
! ONL debug: check on nat_availco2 updating
if (itimeyr .ge. 1) then
  
  iyr=max_yr-5      

  write(6784,*)'==================setup:    curiyr  itimeyr',curiyr+1989,itimeyr
 
  write(6784,*)'=========src: iyr ireg src_avail'
  do ireg=1,max_reg-1
    write(6784,'(a,2i4,13f15.5)')'src_pre',iyr,ireg,src_availco2_prev(ireg,:,iyr)*1000.0/18.0
  enddo

  do ireg=1,max_reg-1
    write(6784,'(a,2i4,13f15.5)')'src_now',iyr,ireg,(src_availco2(ireg,:,iyr))*1000.0/18.0
  enddo

  write(6784,*)'===========ind src: iyr ireg nat_pre bse_prev nat_now'
  do ireg=1,max_reg-1
      write(6784,'(2i4,3f15.5)')iyr,ireg, 1000.0/18.0*( &
                            nat_availco2_prev(ireg,1,iyr)+ &
                            nat_availco2_prev(ireg,2,iyr)+ &
                            nat_availco2_prev(ireg,3,iyr)+ &
                            nat_availco2_prev(ireg,5,iyr)+ &
                            nat_availco2_prev(ireg,6,iyr)+ &
                            nat_availco2_prev(ireg,9,iyr)+ &
                            nat_availco2_prev(ireg,12,iyr)+ &
                            nat_availco2_prev(ireg,13,iyr)), &
                            1000.0/18.0*( &
                            bse_availco2_prev(ireg,1,iyr)+ &
                            bse_availco2_prev(ireg,2,iyr)+ &
                            bse_availco2_prev(ireg,3,iyr)+ &
                            bse_availco2_prev(ireg,5,iyr)+ &
                            bse_availco2_prev(ireg,6,iyr)+ &
                            bse_availco2_prev(ireg,9,iyr)+ &
                            bse_availco2_prev(ireg,12,iyr)+ &
                            bse_availco2_prev(ireg,13,iyr)), &
                            1000.0/18.0*( &
                            nat_availco2(ireg,1,iyr)+ &
                            nat_availco2(ireg,2,iyr)+ &
                            nat_availco2(ireg,3,iyr)+ &
                            nat_availco2(ireg,5,iyr)+ &
                            nat_availco2(ireg,6,iyr)+ &
                            nat_availco2(ireg,9,iyr)+ &
                            nat_availco2(ireg,12,iyr)+ &
                            nat_availco2(ireg,13,iyr))
  enddo 
  
  write(6784,*)'===========special slots'
  write(6784,*)'iyr ireg    isrc    bse_pre    nat_pre    nat_now'
  do ireg=1,max_reg-1
      do isrc=1,max_src
        if (isrc == 4 .or. isrc == 7 .or. isrc == 8 .or. isrc == 10 .or. isrc == 11) then
            iyr=1
            write(6784,'(3i4,3f15.5)')iyr,ireg, isrc, bse_availco2_prev(ireg,isrc,iyr)*1000.0/18.0,nat_availco2_prev(ireg,isrc,iyr)*1000.0/18.0,nat_availco2(ireg,isrc,iyr)*1000.0/18.0
            iyr=25
            write(6784,'(3i4,3f15.5)')iyr,ireg, isrc, bse_availco2_prev(ireg,isrc,iyr)*1000.0/18.0,nat_availco2_prev(ireg,isrc,iyr)*1000.0/18.0,nat_availco2(ireg,isrc,iyr)*1000.0/18.0
            iyr=max_yr-5
            write(6784,'(3i4,3f15.5)')iyr,ireg, isrc, bse_availco2_prev(ireg,isrc,iyr)*1000.0/18.0,nat_availco2_prev(ireg,isrc,iyr)*1000.0/18.0,nat_availco2(ireg,isrc,iyr)*1000.0/18.0
...
! ONL MOD: Update remaining co2
! The increase from newly-built capacity is added to the existing remaining CO2
! Skip when SETUP_CONSTRAINTS is called for the first time
if (itimeyr .ge. 1) then 
  do ireg = 1 , max_reg - 1
     do isrc = 1, max_src               
        do iyr = 1, max_yr
           nat_availco2(ireg,isrc,iyr) = nat_availco2_prev(ireg,isrc,iyr)+ &
                (nat_availco2(ireg,isrc,iyr)-bse_availco2_prev(ireg,isrc,iyr))
        end do
     end do
  end do
endif

! ONL debug: cement co2 price
! ireg=1
! do isrc=1, max_src
  ! write(6779,*)'SETUP curiyr=', curiyr+1989,'isrc=',isrc,'ireg=',ireg,'cregpryr=',cregpryr(IREG,ISRC,:)
! enddo
! write(6779,*)'==============================='

ireg=1
do isrc=1, max_src
  write(6779,*)'SETUP curiyr=', curiyr+1989,' ireg=',ireg,' isrc=',isrc,' cregpryrS=',cregpryrS(IREG,ISRC,:)
enddo
write(6779,*)'==============================='
...
include 'ctus'
include 'emmparm'
include 'uecpout' 	! ONL MOD to add transportation cost from CTUS: OR_OR_TRANCOST(8,8,MNUMYR)
                    ! OR_OR_TRANCOST(IR,RR,MNUMYR): IR=CO2 REGION; RR=EOR PRODUCTION REGION; COSTS STAY THE SAME FOR ALL YEARS
    ...
    DO ir=1,max_reg-1
        DO is = 1,max_src
            IF (is.eq.4) THEN
                ...
                END IF
            ELSE
                    COST = cregpryr(ir,is,t_yr) ! - PIPETAR                       ! ONL MOD to add transportation cost from CTUS
                    OGCO2PRCs(ir,is,t_cyr) = cregpryrS(ir,is,t_yr) ! - PIPETAR    ! ONL MOD to add transportation cost from CTUS
            end if

            reg_COSTCO2(rr,ir,is,t_yr) = COST + or_or_trancost(ir,rr,t_cyr)/18.0 ! PIPETAR * PIPESEG(rr,IR)  ! ONL MOD to add transportation cost from CTUS
            OGCO2TAR(rr,ir) =  or_or_trancost(ir,rr,t_cyr)/18.0 ! PIPETAR * PIPESEG(rr,IR)                   ! ONL MOD to add transportation cost from CTUS
...
ir=1
do is=1, max_src
  write(6781,*)'CO2COST_CALC curiyr=', curiyr+1989,' ireg=',ir,' isrc=',is,' cregpryrS=',cregpryrS(ir,IS,:)
enddo
write(6781,*)'==============================='
is=5
do ir=1, max_reg-1
