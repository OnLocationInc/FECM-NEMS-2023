...
!     Change array which specifies if plants with CO2 capture technology must store captured CO2 given a Carbon Cap or Carbon Tax
      ...
      DO XYR = 1 , UNYEAR
  ! ONL MOD      IF (EMETAX(1,XYR) .GT. 0.0 .AND. NOT(TRAN_FLAG)) THEN
            DO XFR = 1 , UNFRGN
...
!!! ADD DAC NG consumption  SQNGELGR is in tBTU!!!
               temp_dac_ngc=0.0
               do i=1,23
                if (EPGSMP(i).eq.irg) then
                   temp_dac_ngc=temp_dac_ngc+dac_ngc(i,curiyr) !map fuel regions to emm regions
                endif
               enddo
             write(*,*) 'debug DAC ng',temp_dac_ngc, SQNGELGR(IRG,CURIYR,ISP),irg,isp,curiyr+1989
            SQNGELGR(IRG,CURIYR,ISP) = SQNGELGR(IRG,CURIYR,ISP) +  temp_dac_ngc/3.0 !dac not seasonal so divide evenly among the 3 seasons?
!!!!!!!!!!!!!!!!!!!!
        END DO
!
...
!RPS GENERATION AND CREDITS
      ...
      UCRNGNR(2,IRG,CURIYR) = UGNRPSN(UIACS,IRG) * UPRNWSHR(WICS) +  &
                              UGNRPSN(UIAC2,IRG) * UPRNWSHR(WIA2) +  &
