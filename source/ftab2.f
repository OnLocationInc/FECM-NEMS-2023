...
! writes out flat csv file for database and excel-filtering. Also for a planned web-based Application Program Interface (API).
   ...
   integer ibonus
   integer nf90 ! Number of alphanumeric fields to write using Format 90
...
! On first time through, open the output file and write header line
       ...
       open(unit=lg,  file=trim(frscen)//'.'//fdate(2:5)//trim(fdate(8:9))//'.log.txt',status='unknown')
       
       open(unit=io,  file=trim(frscen)//'.'//fdate(2:5)//trim(fdate(8:9))//'.api.txt',status='unknown')
...
! Header line for javascript datafile
       write(lg,'(150a)')    &
       'Scenario',   sep,    &
       'VarName',    sep,    &
       'Year',       sep,    &
...
! TableNumber,RowNum,RegionNum,VarName,GLabel,Gunits,RowFmt,DaType,SubDat,Sector,SubSec,Source,SubSrc,Geogr,(numeric(year),year=1,numeric)
 ...
 90     format(a,a1,a,a1,a4,a1,F13.6)

         !if(rowstr.eq.'003') then
                 ! write(lg,*) rowstr
                 !endif
        
         !if(TableNumber.eq.?) then 
                 !make a different output for each table?
          do i=1,nnumeric
            write(lg,90) &
            trim(Scenario),               sep,      &    ! Scenario
            trim(varname(irow,itab)),     sep,      &    ! VarName
            trim(collab(i)(2:)),          sep,      &    ! Year
            tabn(ir,icols(i),iscen(i))
          enddo           
         !endif 
