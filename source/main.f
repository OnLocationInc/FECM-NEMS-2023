...
! declares data shared among AIMMS invocation routines:  AIMMS_Hydrogen, AIMMS_Coal, AIMMS_NG, Read_AIMMS_Link, write_to_AIMMS_txt_cmd, read_from_AIMMS_txt, and End_AIMMS_cmd
      include 'parametr'
      include 'ncntrl'
      include 'fdict'
      include 'cdsparms'
...
! AIMMS-related variables
     integer Num_AIMMS_modules,Num_AIMMS_modules_d,coalmod,ngasmod,restore,hydrogen !ONL Hy mod
     parameter(Num_AIMMS_modules=4)
     parameter(coalmod=1)   ! coalmod, ngasmod, restore numbered as in aimms_module
     parameter(ngasmod=2)
     parameter(restore=3)
	 parameter(hmmmod=4) !ONL Hy mod
     ...
     integer aimms_module ! 1 for coal, 2 for natural gas, 3 for ReStore 4 for hydrogen used as index of projectHandle !ONL Hy mod

     character*5 modfolder(Num_AIMMS_modules)/'coal','ngas','rest','hmm'/ !ONL Hy mod
     ...
     logical opened_yet(Num_AIMMS_modules)/.false.,.false.,.false.,.false./   ! has monitordbg been opened for each module
...
! AIMMS runtime options
      integer AIMMSEFD,ii,jj  ! run-time option to invoke AIMMS EFD LP (if 1)
...
! TEMP variables for ECP - ONL Add for Ind CCS from LFMM
INTEGER*4 TEMP_L_PLTS
REAL*4 TEMP_L_RY(1000),TEMP_L_RG(1000),TEMP_L_IGRP(1000),TEMP_L_CFR(1000),TEMP_L_HRAT(1000),TEMP_L_CPTY(1000),TEMP_L_PTP(1000),TEMP_L_OPCAP(1000,MNUMYR)  
REAL*4 TEMP_I_CPTY(MAX_KILNS), TEMP_I_CPTY_NEW(MNUMCR,MNUMYR),TEMP_I_CFR(MAX_KILNS,MNUMYR),TEMP_I_CFR_NEW(MNUMCR,MNUMYR,MNUMYR)
COMMON /ECP_TEMP/ TEMP_L_RY,TEMP_L_RG,TEMP_L_IGRP,TEMP_L_CFR,TEMP_L_HRAT,TEMP_L_CPTY,TEMP_L_PTP,TEMP_L_OPCAP,TEMP_I_CPTY, TEMP_I_CPTY_NEW,TEMP_I_CFR,TEMP_I_CFR_NEW
COMMON /ECP_TEMP2/ TEMP_L_PLTS
...
! zeroing variables	  
	  integer i_MarketQuantityCode
	  integer i_CensusDivision
	  integer i_curiyr
...
! +++ Read RESTART Datafile using Prototype Version of FILER
      ...
      CALL NDATIN
! set a temp variables to hold values from the restart file for ECP use before they are zero'ed out in the LFMM and IDM
! ONL Add for Ind CCS
TEMP_L_PLTS=L_PLTS
TEMP_L_RY = L_RY
TEMP_L_RG = L_RG
...
TEMP_L_CPTY = L_CPTY
TEMP_L_PTP = L_PTP 
TEMP_L_OPCAP = L_OPCAP                      
TEMP_I_CPTY=I_CPTY
TEMP_I_CFR=I_CFR
TEMP_I_CPTY_NEW =I_CPTY_NEW
TEMP_I_CFR_NEW = I_CFR_NEW
! End ONL Add
...
! temporary
	 call hmm_main
!	    DO i_curiyr = 1, 
!			DO  i_MarketQuantityCode =1, 10
!				DO i_CensusDivision=1, MNUMCR
!					QHMM(i_MarketQuantityCode,i_CensusDivision,i_CURIYR) &  
!					 = QHMM(i_MarketQuantityCode,i_CensusDivision,34) * (1.03 ** ( max(0,curiyr - 34)))/2.;
!					= 0.0
!				ENDDO ! i_MarketQuantityCode
!			ENDDO ! i_CensusDivision
!		ENDDO ! i_curiyr
!	  write(*,*)'Replaced with AIMMS HMM'
!	  call hmm_main  
...
! If OML needed, call memory initialization routine
      if( exe.eq.1 .or. (exc.eq.1.and.excoal.eq.0) ) IOML=1
      if(IOML.eq.1) then
...
! GET LOCAL RUNTIME OPTIONS FOR GENERUAL PURPOSE MOREOPT FILE USING RTOVALUE FUNCTION
	  ...
	  RUNDAC  =RTOVALUE('RUNDAC  ',0) !  SWITCH TO RUN DAC
...
! +++ MUST USE UNADJUSTED PRICES, SINCE WE TEST THEM
                        ...
                        ENDDO
!  Hydrogen algorithm smoothing  copy previous &&&& ONL                     
                        DO IV=1,PHLFMM
                            DO IR = 1,MNUMCR  ! last sector
                               PHMM_PREV(IV,IR,curiyr) = PHMM(IV,IR,CURIYR)
                            ENDDO
                        ENDDO 
...
!  CALL EMISSION MODULE.  Computes carbon emission and implements some policy options.
               IWaitMs = 1000 * 60 * 15
               IF(RUNDAC.eq.1) then
			      call write_gdx_dac  !write NEMS values to NEMS_to_DAC.gdx
                  !cmdline='"gams"' 
				  call RTOSTRING('GAMSVERS',GAMSVERS)	 !td
                  cmdline=GAMSVERS !td
				  call OSCall(iwaitMS,cmdline,args,iRet) 
                  If (iret.eq.1) Istatus=.true.
                  args='DACshell.gms lo=0 lf=CTS.log s=ctstmp gdx=CTS'
				  Write(18,'(A,2I6)') 'Call Gams if new or retrofit CCS ',E_PLTS,N_PLTS
                  If (E_PLTS.GT.0.0 .OR. N_PLTS.GT.0.0) call OSCall(iwaitMS,cmdline,args,iRet)
                  If (iret.eq.1) Istatus=.true.
                  Write(18,'(A,I4,2x,L)')'Istatus: ',Iret,Istatus
! check to see if DAC ran by looking for output
                  inquire(file='DAC_TO_NEMS.gdx',exist=lexist)
                  if(.not. lexist) then !DAC didn't run so set DAC values to zero
                    write(*,*) 'DAC did not run .. setting DAC values to zero'
                    DAC_CO2_45Q=0.0
                    DAC_ELC=0.0
                    DAC_NGC=0.0
                    DAC_ELC_CS=0.0
                    ...
                    CALL READ_GDX_DAC !read DAC values from DAC_to_NEMS.gdx
                  endif
               ENDIF
      ...
      ELSEIF (IMODEL.EQ.8) THEN
        ...
        if (excoal .eq. 0) then
          ...
          if (CurCalYr .ge. CLBASEYR) then ! first coal year
            ...
            iret=0
! PASS EMMBENCH MOREOPT element Parameter to the coal module via file emmbench.txt
...
!    3) update Table 35 values
      ...
      ELSEIF (IMODEL.EQ.12) THEN
         CALL RENEW
      ELSEIF (IMODEL.EQ.13) THEN !hydrogen ONL Mod Hy
         if (CurCalYr .ge. 2023) then           ! first model year
             if (.not. opened_yet(4)) then
               opened_yet(4)=.true.
               call unitunopened(100,999,monitordbg(4))
               open(monitordbg(4),file='hmm\monitor.debug.txt',status='unknown')
               rewind monitordbg(4)
             endif
             FYearStart=curiyr
             FYearEnd=curiyr
             FYearSubset=0                 ! send year subset as specified by FYearStart and FYearEnd ! send all years=0
             if (CurCalYr .eq. 2023) FYearSubset=0                 ! send all years 
             if (CurCalYr .le. 2023 .and. curitr .eq. 1) FYearSubset=0
             iret=0
             retry=0
             call AIMMS_HMM(iret)
             write(*,*) 'dbg - called AIMMS',iret,curiyr+1989,curitr
             FYearsubset=0
             if(iret.ne.0) then
               write(6,*) 'ERROR running AIMMS hydrogen market module.  Stopping now to avoid confusion.'
               CALL NDATOT('   ')
               CALL NDATOT('GDX') ! call with argument to write GDX format restart file also
               write(6,*) 'ERROR return from AIMMS hydrogen market module is ',iret
               stop 999
             endif
           endif
      ELSE
...
!  Coal generation unit utilization factors
      ...
      ENDDO
! smooth hydrogen prices  &&&& ONL
!  HMM_RELAX is in hmmblk
      DO IV = 1, PHLFMM
        DO IR= 1,MNUMCR
            PHMM(IV,IR,CURIYR) = PHMM_PREV(IV,IR,CURIYR)+ &
             HMM_RELAX(IV) * (PHMM(IV,IR,CURIYR) - PHMM_PREV(IV,IR,CURIYR))
        ENDDO
      ENDDO  
      if (CURIYR .eq. 34 ) THEN
         write(*,2521)'CURIYR,CURITR,HMM_RELAX(1:10), PHMM(1:10,1:MNUMCR,CURIYR), PHMM_PREV(1:10,1:MNUMCR,CURIYR)' , &
         CURIYR,CURITR,HMM_RELAX(1:10), PHMM(1:10,1:MNUMCR,CURIYR), PHMM_PREV(1:10,1:MNUMCR,CURIYR)
      endif 

2521  FORMAT(1X,A40,I2,I2,1X, (30f10.4,1X))
...
!*******************************************************************
      ...
      READ(UNITI,*) EXH,MORDER(13),PRTDBGH   ! RUNMOD(13) HYDROGEN
! go out and get hydrogen module (RUNMOD(13) execution, order, print debug switches
    !ONL MOD  EXH=RTOVALUE('EXH     ',0)
    !ONL MOD  MORDER(13)=RTOVALUE('ORDH    ',13)
    !ONL MOD  PRTDBGH=RTOVALUE('PRTDBGH ',0)
...
! convert tax credits from nominal to 1987 dollars, from start year till 2026, then values are in real 2026 dollars
      ...
      DO II=I_S,I_R
        ...
        WRITE(6,2317) CURIRUN, CURCALYR, CURITR, I_45Q_SYR, I_45Q_DURATION, I_45Q_LYR_NEW, I_45Q_LYR_RET, BASEYR, I_S, I_R, II, &
           CCS_EOR_45Q(II), CCS_EOR_45Q(II) * MC_JPGDP(-2) / MC_JPGDP(II), CCS_SALINE_45Q(II), CCS_SALINE_45Q(II) * MC_JPGDP(-2) / MC_JPGDP(II), &
		   CCS_DAC_45Q(II), CCS_DAC_45Q(II) * MC_JPGDP(-2) / MC_JPGDP(II),MC_JPGDP(-2), MC_JPGDP(II)
 2317   FORMAT(1X,"45Q_Info",11(",",I5),8(",",F21.6))

        CCS_EOR_45Q(II)= CCS_EOR_45Q(II) * MC_JPGDP(-2) / MC_JPGDP(II)
        CCS_SALINE_45Q(II) = CCS_SALINE_45Q(II)  * MC_JPGDP(-2) / MC_JPGDP(II)
		CCS_DAC_45Q(II) = CCS_DAC_45Q(II)  * MC_JPGDP(-2) / MC_JPGDP(II)
      ENDDO
      ...
      Do II = I_R + 1 , I_L
        ...
        WRITE(6,2317) CURIRUN, CURCALYR, CURITR, I_45Q_SYR, I_45Q_DURATION, I_45Q_LYR_NEW, I_45Q_LYR_RET, BASEYR, I_R, I_L, II, &
           CCS_EOR_45Q(II), CCS_EOR_45Q(II) * MC_JPGDP(II-1) / MC_JPGDP(2025 - BASEYR + 1) * MC_JPGDP(-2) / MC_JPGDP(II), &
           CCS_SALINE_45Q(II), CCS_SALINE_45Q(II) * MC_JPGDP(II-1) / MC_JPGDP(2025 - BASEYR + 1) * MC_JPGDP(-2) / MC_JPGDP(II), &
           CCS_DAC_45Q(II), CCS_DAC_45Q(II) * MC_JPGDP(II-1) / MC_JPGDP(2025 - BASEYR + 1) * MC_JPGDP(-2) / MC_JPGDP(II)
...
! time limit to wait for AIMMS to return:
      integer aimms_time_limit /4800/         ! in half seconds, 1200 is 10 minutes (600 seconds)
...
! do this so subsequent invocation writes over file
      ...
      RETURN
      END
 Subroutine AIMMS_HMM(ireturn) !ONL Mod Hyd
    use ifport,only:timef
    use globmain
    use CALLING_AIMMS
    implicit none
    include 'indout'
    include 'pmmftab'
    include 'hmmblk' ! hydrogen temporary add
    integer ireturn ! return code
    logical done_yet/.false./   
    integer i_MarketQuantityCode,i_CensusDivision ! hydrogen temp variable  
    character*270 cline
    character*43 spaces
    character*18 fname  ! filename
    integer unum     !unit number for open file
    integer iy,jj       !counters
    integer fyr,nhyr    !first historical year, number of historical years
!temporary reset demands by 1/2 growing 3% per year 
!	if (CURIYR .ne. 34) THEN
!		DO  i_MarketQuantityCode =1, 10
!			DO i_CensusDivision=1, MNUMCR
!				QHMM(i_MarketQuantityCode,i_CensusDivision,CURIYR) &  
!				 = QHMM(i_MarketQuantityCode,i_CensusDivision,34) * (1.03 **  max(0,curiyr - 34))/4.
!			ENDDO ! i_MarketQuantityCode
!		ENDDO ! i_CensusDivision
!	ELSE	
!		if (CURITR .eq. 1) then
...
!				DO i_CensusDivision=1, MNUMCR
!					QHMM(i_MarketQuantityCode,i_CensusDivision,CURIYR) &  
!					 = QHMM(i_MarketQuantityCode,i_CensusDivision,34)/4. 
!					 write(*,*) 'CURIYR =  ', CURIYR,  'CURITR = ',CURITR, 'QHMM ', 	QHMM(i_MarketQuantityCode,i_CensusDivision,CURIYR)	
!				ENDDO ! i_MarketQuantityCode
!			ENDDO ! i_CensusDivision
!		ENDIF ! curitry
!	ENDIF ! curiyr
!    write(*,*)'Replaced with AIMMS HMM'
!		
! Handles data exchanges and invocation of the AIMMS Hydrogen module.
! Data exchanges of global variables to and from AIMMS are handled by calls to filer routines.
!  Each time through:
!  1) send NEMS input/output global data to AIMMS Hydrogen project
!  2) Open AIMMS project via the command line
!  3) invoke its MainExecution procedure and wait for it to finish.
!  4) retrieve output global data from AIMMS project
    ireturn=0
    aimms_module=4 !hmmmod    ! index to indicate the hydrogen module
    restarto='.\\hmm\\hmm.aimms'
! first time through:
    if (.not. done_yet) then
      keepopen(4)=RTOVALUE('KEEPOPEN',0)
      if (keepopen(4) .eq. 0) keepopen(4)=RTOVALUE('KEEPNG  ',0)
! read historical data and smoothing factors for hydrogen
      unum = FILE_MGR('O','H2HIST            ',.FALSE.)
      read(unum,'(a)') cline
      do while (cline(1:1) .eq. '@')
        read(unum,'(a)') cline
      enddo
      read(cline,998) nhyr,fyr
      fyr=fyr-1989
      read(unum,'(a)') cline
      read(unum,'(a)') cline !skip refinery history since it's read in the lfmm (rfhist.txt)
      !read(unum,999) (rfhcxh2in(mnumpr,iy),iy=fyr,fyr+nhyr-1)
      read(unum,'(a)') cline(1:70)
      read(unum,999) (hydrogen2ref(iy),iy=fyr,fyr+nhyr-1)
      qhmm(phlfmm,11,fyr:fyr+nhyr-1)=0.0
      HMOTPRD=0.0
      read(unum,999) (hmotprd(iy,11),iy=fyr,fyr+nhyr-1)
      hmotprd(fyr+nhyr:mnumyr,11)=hmotprd(fyr+nhyr-1,11)
      read(unum,'(a)') cline
      do while (cline(1:1) .eq. '@')
        read(unum,'(a)') cline
      enddo
      read(cline,997) (HMM_RELAX(jj),jj=1,10)
      unum = FILE_MGR('C','H2HIST            ',.FALSE.)
997 FORMAT(10F6.1)
998 FORMAT(I2,43X,I4)
999 FORMAT(42X,<nhyr>f8.2) 
    !write(666,*) 'nhyr=',nhyr, 'fyr=',fyr
    !write(666,*) rfhcxh2in(mnumpr,fyr:fyr+nhyr-1)
    !write(666,*) hydrogen2ref(fyr:fyr+nhyr-1)
    !write(666,*) hmotprd(fyr:fyr+nhyr-1,11)
    !  get AIMMS executable foldername
      call Read_AIMMS_Link
      done_yet=.true.
      if(no_AIMMS) then
        write(6,*) 'AIMMS Software not found in expected location so exiting AIMMS_HMM'
        ireturn=221
        return
      endif
    endif
! send input/output global data to AIMMS project and invoke its MainExecution procedure
  ...
  if(.not. closed(aimms_module)) then      ! once closed, don't invoke or reclose
    restarti='.\\hmm\\hmm.aimms'
    varlistfile='HMMGETVARS'   ! no longer used with text data transfer
    call read_from_AIMMS_txt
    ireturn=iret
    if (iret .ne. 0) then
! don't reopen after error
...
! call termination procedure and close AIMMS project unless option to keep aimms open after each invocation is in effect
    !   if (keepopen(4) .eq. 0) then
         Call End_AIMMS_cmd
         aimms_opened(aimms_module)=.false.
    !   endif
    endif
  endif
  if (curiyr.eq.lastyr .and. NCRL.eq.1 .and. aimms_opened(aimms_module) ) then  !   but may want to keep open until the end
    ! call termination procedure and close AIMMS project
    write(6,'(a,i4,a)') 'AIMMS ',curcalyr,' Closing AIMMS'
    Call End_AIMMS_cmd
    closed(aimms_module)=.true.
  ...
  return
  end subroutine AIMMS_HMM
       SUBROUTINE HMM_MAIN
	  implicit none
	  Include 'parametr'
	  Include 'ncntrl'
	  Include 'HMMBLK'
	  integer i_MarketQuantityCode,i_CensusDivision, I_Year
!set all sectors to high price before start of HMM 
	DO I_Year =1,33   
			DO  i_CensusDivision=1, MNUMCR
				DO i_MarketQuantityCode =1, 10
					PHMM(i_MarketQuantityCode,i_CensusDivision,I_YEAR)  = 35.0 
				ENDDO ! i_MarketQuantityCode
			ENDDO ! marketquantity
	ENDDO ! I_YEAR
!	write(*,*)' loop  high TDM curiyr curitr sums region 11 , curiyr, curitr, PHMM(1,1,I_YEAR)', CURIYR, CURITR
!sums for all region 11	
		DO I_Year =1,MNUMYR  
				DO i_MarketQuantityCode =1, 10
					QHMM(i_MarketQuantityCode,MNUMCR,I_YEAR)  = sum(QHMM(i_MarketQuantityCode,1: MNUMCR -2,I_YEAR))
				ENDDO ! i_MarketQuantityCode
		ENDDO ! I_YEAR
			write(*,*)' loop  curiyr curitr sums region 11 , curiyr, curitr, PHMM(1,1,I_YEAR)', CURIYR, CURITR
! &&&& ONL copy over QELHMM for varlistrec.txt number of seasons in electricity used for HMM
		DO I_Year =1,MNUMYR  
				DO i_MarketQuantityCode =1, 3
                   DO i_CensusDivision = 1,MNUMCR
					    QHMM(H2OFFSET + i_MarketQuantityCode,i_CensusDivision,I_YEAR)  = QELHMM(i_CensusDivision,I_YEAR,i_MarketQuantityCode)
                   ENDDO ! i_CensusDivision
				ENDDO ! i_MarketQuantityCode
			    write(*,*), I_YEAR, CURITR,PHMM(3,1,I_YEAR),PHMM(4,1,I_YEAR)
 7457           FORMAT(1X,'HMM I_YEAR curitr  PHMM',2(":",I4),2(":",F21.6))
		ENDDO ! I_YEAR

	 return  ! Don't need this now
!	DO I_Year =1,61   
!		if (I_YEAR .eq. 34 ) THEN
!			DO  i_MarketQuantityCode =1, 10
!				DO i_CensusDivision=1, MNUMCR
!					QHMM(i_MarketQuantityCode,i_CensusDivision,I_YEAR) &  
!					 = QHMM(i_MarketQuantityCode,i_CensusDivision,I_YEAR) /4.;
!				ENDDO ! i_MarketQuantityCode
!			ENDDO ! i_CensusDivision
!			write(*,*)' loop 1 I_YEAR, curitr, QHMM(1,1,I_YEAR)',I_YEAR, curitr, QHMM(1,1,I_YEAR)
!		else 
!				DO i_CensusDivision=1, MNUMCR
!					QHMM(i_MarketQuantityCode,i_CensusDivision,I_YEAR) &  
!					 = QHMM(i_MarketQuantityCode,i_CensusDivision,34) * (1.03 ** ( max(0,I_YEAR - 34)));
!				ENDDO ! i_MarketQuantityCode
!			ENDDO ! i_CensusDivision
!			write(*,*)' loop 2 I_YEAR, curitr, QHMM(1,1,I_YEAR)',I_YEAR, curitr, QHMM(1,1,I_YEAR)
!		ENDIF ! I_YEAR
!	ENDDO  ! I_YEAR
	  write(*,*)'New Replaced with AIMMS HMM resetting resetart file called once'
	  RETURN
      END  subroutine HMM_MAIN 
SUBROUTINE READ_GDX_DAC
      IMPLICIT NONE
      include 'parametr'
      include 'ncntrl'
...
! +++ End FILER Declarations
      ...
      FRETCD=0
      FSOURC=1          ! Get variable list from a file
      FNAMEI18='VARLISTDACOUT'  ! DD name for input file with variable names
      NEW=.FALSE.
      FUNITO=FILE_MGR('O',FNAMEI18,NEW)
      FUNFMT=5
      FRTYPE=2
      FNAMEI='DAC_TO_NEMS.gdx'
      FNAMEO=' '
!Read variables from GDX to NEMS restart file
      CALL FILER(FRTYPE,FSOURC,FUNITI,FUNITO,FNAMEI,FNAMEO,FRETCD,FUNFMT)
      End subroutine read_gdx_dac
SUBROUTINE write_gdx_dac
      IMPLICIT NONE
      include 'parametr'
      include 'ncntrl'
      include 'gdxiface' !DSA!
...
! +++ End FILER Declarations
      ...
      EXTERNAL FILE_MGR
           ...
           FSOURC=1          ! Get variable list from a file
           NEW=.FALSE.   
           FNAMEI18='VARLISTDACIN'  ! DD name for input file with variable names
           FUNITO=FILE_MGR('O',FNAMEI18,NEW)
           FUNITI=FILE_MGR('O',FNAMEI18,NEW)  
           FNAMEI=' '
           FNAMEO='NEMS_TO_DAC.gdx'
           FRETCD=0
           FUNFMT=5
        !Write variables to GDX from NEMS restart file
           CALL FILER(FRTYPE,FSOURC,FUNITI,FUNITO,FNAMEI,FNAMEO,FRETCD,FUNFMT)  
! write(666,*) 'TAX to DAC',curiyr+1989,curitr,EMETAX(1:2,curiyr)
return
End subroutine write_gdx_dac
SUBROUTINE READ_GDX_CTS(switch)
      IMPLICIT NONE
      include 'parametr'
      include 'ncntrl'
      include 'gdxiface' !DSA!
      ...
      include 'uecpout'
      integer i
      integer switch !which folder to read the CTSSoln file from 0=input folder 1=main folder
! +++ FILER Declarations:
      INTEGER*4 FUNITI,FUNITO,FRETCD,FRTYPE,FSOURC,FUNFMT
      CHARACTER*100 FNAMEI,FNAMEO
      CHARACTER*18 FNAMEI18
! +++ End FILER Declarations
      ...
      FRETCD=0
      FSOURC=1          ! Get variable list from a file
      FNAMEI18='VARLISTCTSOUT'  ! DD name for input file with variable names
      NEW=.FALSE.
      FUNITO=FILE_MGR('O',FNAMEI18,NEW)
      FUNFMT=5
      FRTYPE=2
      if (switch.eq.0) then
        FNAMEI='.\input\CTSSoln.gdx'
      else 
        FNAMEI='CTSSoln.gdx'
      endif
      FNAMEO=' '
!Read variables from GDX to NEMS restart file
      CALL FILER(FRTYPE,FSOURC,FUNITI,FUNITO,FNAMEI,FNAMEO,FRETCD,FUNFMT)
      End subroutine read_gdx_cts
SUBROUTINE WRITE_GDX_CTS
      IMPLICIT NONE
      include 'parametr'
      include 'ncntrl'
      include 'gdxiface'
...
! +++ End FILER Declarations
      ...
      EXTERNAL FILE_MGR
           ...
           FSOURC=1          ! Get variable list from a file
           NEW=.FALSE.   
           FNAMEI18='VARLISTCTSIN'  ! DD name for input file with variable names
           FUNITO=FILE_MGR('O',FNAMEI18,NEW)
           FUNITI=FILE_MGR('O',FNAMEI18,NEW)  
           FNAMEI=' '
           FNAMEO='CTSSavR.gdx'
        ...
        !Write variables to GDX from NEMS restart file
           CALL FILER(FRTYPE,FSOURC,FUNITI,FUNITO,FNAMEI,FNAMEO,FRETCD,FUNFMT)  
return
End subroutine write_gdx_cts
SUBROUTINE write_SAVR
!
!     THIS SUBROUTINE PUTS PRIOR CYCLE'S DECISIONS RE CCS RETROFITS AND CCS NEW BUILDS INTO CTSSAVR.GDX
!        
      IMPLICIT NONE
      include 'parametr'
      include 'ncntrl'
     ...
     ! include 'bildout'
      ...
      INTEGER ECP_TIME_BEGIN,ECP_TIME_END,IRG,IYR,LONGIYR(MNUMYR),IE_IGRP(MAX_CL),IR_IGRP(MAX_CL),IN_IGRP(1000),IL_IGRP(1000),LONGGNP(-2:MNUMYR+ECP_D_FPH),IG_IGRP(2000)  ! ONL IndCCS
      character*24 CLONGIYR(MNUMYR),CECLIGRP(MAX_CL),CIEIGRP(MAX_CL),CIRIGRP(MAX_CL),CINIGRP(1000),CILIGRP(1000),CLONGGNP(-2:MNUMYR+ECP_D_FPH),CMOR(MNUMOR), &  ! ONL IndCCS
                 M3(3), M8(8),M7(7),M13(13),M2(2),M4(4),CYR(MNUMYR),cMNUMCR(MNUMCR),CEGIGRP(2000),CMAX_KILNS(MAX_KILNs)
      character*10   istr    
...
!TEMP variables for ECP   ! ONL for Ind CCS sites
   ! TEMP variables for ECP - ONL Add for Ind CCS from LFMM
INTEGER*4 TEMP_L_PLTS
REAL*4 TEMP_L_RY(1000),TEMP_L_RG(1000),TEMP_L_IGRP(1000),TEMP_L_CFR(1000),TEMP_L_HRAT(1000),TEMP_L_CPTY(1000),TEMP_L_PTP(1000)  
REAL*4 TEMP_I_CPTY(MAX_KILNS), TEMP_I_CPTY_NEW(MNUMCR,MNUMYR),TEMP_I_CFR(MAX_KILNS,MNUMYR),TEMP_I_CFR_NEW(MNUMCR,MNUMYR,MNUMYR)
COMMON /ECP_TEMP/ TEMP_L_RY,TEMP_L_RG,TEMP_L_IGRP,TEMP_L_CFR,TEMP_L_HRAT,TEMP_L_CPTY,TEMP_L_PTP,TEMP_I_CPTY, TEMP_I_CPTY_NEW,TEMP_I_CFR,TEMP_I_CFR_NEW
COMMON /ECP_TEMP2/ TEMP_L_PLTS
! logging
...
!        create the gdx file
         ...
         DO I=1, 1000
            IN_IGRP(I) = 0
			IL_IGRP(I) = 0   ! ONL added sites for ind CCS
         ENDDO
         DO I=1,MAX_CL
            IE_IGRP(I) = 0
            IR_IGRP(I) = 0
         ...
         ENDDO
! ONL begin: add "L" group for industrial CCS       
         CILIGRP = '50000';
         CINIGRP = '50000';
         IF (N_PLTS.GE.1) THEN
            DO I= 1, N_PLTS
               IF (N_IGRP(I).GT.0) THEN
                  IN_IGRP(I) = N_IGRP(I) + 50000
            write(CINIGRP(i),*) IN_IGRP(I)
            CINIGRP(i)=TRIM(ADJUSTL(CINIGRP(i)))
               ENDIF
         ENDDO
         ENDIF
         IF (TEMP_L_PLTS.GE.1) THEN
            DO I= 1, TEMP_L_PLTS
               IF (TEMP_L_IGRP(I).GT.0) THEN
                   ! IL_IGRP(I) = L_IGRP(I) + 50000
                   IL_IGRP(I) = TEMP_L_IGRP(I) + 50000
                   write(CILIGRP(i),*) IL_IGRP(I)
                  CILIGRP(i)=TRIM(ADJUSTL(CILIGRP(i)))                           
                  WRITE(6,4932) CURIRUN, MNUMYR, CURITR, I, IL_IGRP(I), TEMP_L_IGRP(I), CILIGRP(I)            
               ENDIF
         ENDDO
         ENDIF  
IF (L_PLTS.GE.1) THEN
            DO I= 1, L_PLTS
               IF (L_IGRP(I).GT.0) THEN
                  IL_IGRP(I) = L_IGRP(I) + 50000
                  write(CILIGRP(i),*) IL_IGRP(I)
                  CILIGRP(i)=TRIM(ADJUSTL(CILIGRP(i)))   
                  WRITE(6,4932) CURIRUN, MNUMYR, CURITR, I, IL_IGRP(I), L_IGRP(I), CILIGRP(I)            
               ENDIF
         ENDDO
         ENDIF   
4932     FORMAT("L_IGRP",5(":",I8),1(":",F12.2),1(":",A24))
 ! ONL end  
         DO I= 1, 1000
         ...
         ENDDO           
		 DO I = 1,INUM_KILN
		    write(CMAX_KILNS(i),*) I_IGRP(i)
            CMAX_KILNS(i)=  TRIM(ADJUSTL(CMAX_KILNS(i)))          
         ENDDO           
         CF_NAT=0.0
         DO I = 1, MNUMYR
...
! ONL start: convey LFMM capture site info
write(667,*) TEMP_L_PLTS
write(667,*)
write(667,*)  TEMP_L_RY(1:100)
write(667,*)  
write(667,*)  TEMP_L_RG(1:100)
write(667,*)  
write(667,*)  TEMP_L_IGRP(1:100)
write(667,*)  
write(667,*)  TEMP_L_CFR(1:100)
write(667,*)  
write(667,*)  TEMP_L_HRAT(1:100)
         call WriteGDXElement(pgdxB, 0,'PMMOUT_L_IGRP', 'IGRP set for LFMM capture builds', 1, 1000, 1, 1, 1, 1, CILIGRP,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1,'PMMOUT_L_RY', 'LFMM capture build yr', TEMP_L_RY, 1000, 1, 1, 1, 1, CILIGRP,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1,'PMMOUT_L_RG', 'LFMM capture build fuel region', TEMP_L_RG, 1000, 1, 1, 1, 1, CILIGRP,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1,'PMMOUT_L_CFR', 'LFMM capture build Capacity factor', TEMP_L_CFR, 1000, 1, 1, 1, 1, CILIGRP,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1,'PMMOUT_L_HRAT', 'LFMM capture build heatrate', TEMP_L_HRAT, 1000, 1, 1, 1, 1, CILIGRP,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1,'PMMOUT_L_CPTY', 'LFMM capture build capacity (MMton)', TEMP_L_CPTY, 1000, 1, 1, 1, 1, CILIGRP,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1,'PMMOUT_L_PTP', 'LFMM capture build plant type', TEMP_L_PTP, 1000, 1, 1, 1, 1, CILIGRP,j2,k,l,m,'R')
! ONL end     
		 ...
		 call WriteGDXElement(pgdxB, 1, 'HMMBLK_CO2CAPFUEL', 'CO2 by fuel',CO2CAPFUEL, 3, MNUMCR, MNUMYR, 1, 1, M3, cMNUMCR, CYR, l,m,'R')
         call WriteGDXElement(pgdxB, 1, 'INDOUT_I_RY' ,'retrofit year for existing cement unit',I_RY, MAX_KILNS, 1, 1, 1, 1, CMAX_KILNS,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1, 'INDOUT_I_CPTY' ,'capture quanity for existing cement unit',I_CPTY,MAX_KILNS, 1, 1, 1, 1, CMAX_KILNS,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1, 'INDOUT_I_PTP' ,'Plant type(=70, for future use)',I_PTP, MAX_KILNS, 1, 1, 1, 1, CMAX_KILNS,j2,k,l,m,'R')
         call WriteGDXElement(pgdxB, 1, 'INDOUT_I_CPTY_NEW' ,'capture quanity for  cement ; census region/year',I_CPTY_NEW,MNUMCR, MNUMYR, 1, 1, 1,   cMNUMCR, CYR,k,  l,m,'R')
         call WriteGDXElement(pgdxB, 1, 'INDOUT_I_CFR_NEW', 'new capture Q cement; census region/year',I_CFR_NEW,MNUMCR, MNUMYR, MNUMYR, 1, 1,   cMNUMCR, CYR,CYR,  l,m,'R')
        ! write(900,*) 'icfr1=',I_CFR(1,:),INUM_KILN,MNUMYR,CYR(1),CMAX_KILNS(1)
        ! write(900,*) 'icfr10=',I_CFR(10,:),INUM_KILN,MNUMYR,CYR(10),CMAX_KILNS(10)
        ! write(900,*) 'icfr15=',I_CFR(15,:),INUM_KILN,MNUMYR,CYR(15),CMAX_KILNS(15)
        ! write(900,*) 'icfr20=',I_CFR(20,:),INUM_KILN,MNUMYR,CYR(20),CMAX_KILNS(20)
         call WriteGDXElement(pgdxB, 1, 'INDOUT_I_CFR' ,'capture existing sites ; unit/year',I_CFR,MAX_KILNS, MNUMYR, 1, 1, 1, CMAX_KILNS,CYR,k,l,m,'R')
         call WriteGDXElement(pgdxB, 0, 'INDOUT_I_IGRP' ,'facility+kilnid for existing capture sites',1,MAX_KILNS, 1, 1, 1, 1, CMAX_KILNS,j2,k,l,m,'R')
