...
!DAC addition, need a switch so it doesn't get added in if you don't want DAC, hardwired RNB.LE.24 needs a fix DAC_ELC not dimensioned to 25
      do I=1, Neu
        if ((EUGRP(I).eq. 21).and.(RNB.LT.MNUMNR)) then
           IF (mnumyr.eq.39) write(*,2038) 'checking dac electric b4 ', I,EUGRP(I),curiyr,curitr,rnb,LoadForec(I,1), DAC_ELC(RNB,CURIYR)
           LoadForec(I,1) = LoadForec(I,1) + (DAC_ELC(RNB,CURIYR)*1000) ! 21 is (PRIMARY MET) ; dac_elc is in billion kWh
           IF (mnumyr.eq.39) write(*,2038) 'checking dac electric af', I,EUGRP(I),curiyr,curitr,rnb,LoadForec(I,1), DAC_ELC(RNB,CURIYR)
2038    Format(A40,1x,5(I4,1x),2(F12.3,1x))           
        endif
            if ((EUGRP(I).eq. 22).and.(RNB.LT.MNUMNR.and. mnumyr.eq.39))  write(*,2038) 'checking dac electric 22', I,EUGRP(I),curiyr,curitr,rnb,LoadForec(I,1), DAC_ELC(RNB,CURIYR)
            if ((EUGRP(I).eq. 23).and.(RNB.LT.MNUMNR.and. mnumyr.eq.39))  write(*,2038) 'checking dac electric 23', I,EUGRP(I),curiyr,curitr,rnb,LoadForec(I,1), DAC_ELC(RNB,CURIYR)
      enddo      
