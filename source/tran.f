...
INTEGER      CAFECASE, &                                            ! "1" switch for CAFE cases
             LVPRCADJ, &                                            ! "1" switch for light vehicle price adjustment that subtracts discounted fuel savings !sb H2 project (added ,&)
             EXHYDROGEN                                             ! Switch to count hydrogen as reformed natural gas in TDM (0) or use HMM and restart file (1) !sb H2 project         
...
!...TRANEFF switches on alternate scenarios  
    ...
    CAFECASE   = 0
    LVPRCADJ   = 0
    EXHYDROGEN = 1.0  !sb H2 project USE HMM                                                                        
...
!...Add H2 consumption to NG consumption if the switch to use HMM and/or restart file is off. Otherwise use restart or HMM. Allows standalone TDM using restart file (!sb H2 project)
      if(EXHYDROGEN .eq.1) then ! ****FW: updated this option to include bus NG demand but exclude hydrogen (fuel=7) div by 0.7 eff for gas SMR
!        QGFTRFV(iregn,n) =  QGFTRFV(iregn,n) + (FLTFUELBTU(iregn,4,n)) * BENNG(iregn,n)   ! AEO22 line to be removed
		QGFTRFV(iregn,n) =  QGFTRFV(iregn,n) + (FLTFUELBTU(iregn,4,n)+ &
						   ...
						   * BENNG(iregn,n) 
 !                           (sum(VMTHH(N,iregn,14,1:maxvtyp))/MPGTECH(14,n)))                                                                        
      else	
...
!... sb H2 project
      if(EXHYDROGEN.eq.1) then
!	  
!						 
!                         FLTFUELBTU(iregn,7,n) + &                         ! ldv fleet    add this one
!                         QMTBR(1,8,IREGN,N)    + &                         ! transit bus
!                         QMTBR(2,8,IREGN,N)    + &                         ! intercity bus 
!                         QMTBR(3,8,IREGN,N)    + &                         ! school bus 
!                         QH2TRFV(iregn,iyr)  
		QHMM(PHTDM_CEN, iregn,N) = FLTFUELBTU(iregn,7,n) + &
		                  QMTBR(1,8,IREGN,N)    + &                         ! transit bus
                          QMTBR(2,8,IREGN,N)    + &                         ! intercity bus zeroed out below
                          QMTBR(3,8,IREGN,N)    + &                         ! school bus    zeroed out below
						  QH2TRFV(iregn,N)                                  ! fleet truck
	  endif
...
!... 1=large 2 = small 3 =rural use central for now?
!... sb H2 project                                                                 
    DO IREGN=1,MNUMCR-2
!...HMS-Hydrogen prices by market segment.  ******FW these prices ph*tr originally came from the HMM; &&&& ONL
...
!      HPrice(3,iregn,n)=ph3tr(iregn,n)*TMC_PGDP(1)*100.0*MG_HHV/1000.0
      ...
      HPrice(1,iregn,n)=PHMM(PHTDM_CEN,iregn,n)*TMC_PGDP(1)*100.0*MG_HHV/1000.0
      HPrice(2,iregn,n)=PHMM(PHTDM_CEN,iregn,n)*TMC_PGDP(1)*100.0*MG_HHV/1000.0
      HPrice(3,iregn,n)=PHMM(PHTDM_CEN,iregn,n)*TMC_PGDP(1)*100.0*MG_HHV/1000.0
	  if (CURIYR .eq. 41) then
  		write(*,*)'PHTDM_CEN  iregn, N, PHMM(PHTDM_CEN) HPRICE', &
		   PHTDM_LOC,iregn, N, PHMM(PHTDM_CEN, iregn,N),  HPrice(1,iregn,n)   
	  ENDIF ! curiyr                       
      DO ILDV=1,MAXLDV
...
!...HMS-Calculate hydrogen fuel cost by market segment. Changed to always use restart file. !sb from H2 project
            if(ILDV.eq.14 .and. LDV_MPG_CL(IVTYP,ILDV,ICL,yrs) .ne. 0.0) then
			  if(exh.eq.-1) then    !sb from H2 project      ******FW use of switch should still work here
...
!...******FW     load all H2 stations into hms=1 (urban) !sb from H2 project                                                                                                                                    
...
!...  report the total regional stations below.   !sb H2 project
      do iregn=1,mnumcr-2
        do ifuel=1,maxfuel
!          if(ifuel.eq.8.and.exh.eq.1) then ******FW remove this as HMM no longer caculating HSTAT
!            tmpsum = 0.0
!            do hms = 1,3
...
!          else
            ALTSTA(iregn,ifuel,n) = ALTSTAT(ifuel,n) * AFVSHREG(iregn,ifuel,n)   ! *****FW this applies to all fuels and this could moved to above
!          endif
        enddo 
      enddo      
    endif
...
!...Hydrogen fuel availability by market segment if hmm is on.    
                                                         
!...    Beginning in 2012 set hydrogen infrastructure equal to CNG if the HMM is off
! set to NG if HMM off.  !sb H2 project
!        if(curcalyr.ge.2012.and.ifuel.eq.8.and.exh.eq.0) FAVAIL(8,N,IREGN) = FAVAIL(5,N,IREGN)                                                                         
        if(curcalyr.ge.2012.and.ifuel.eq.8 .and. EXHYDROGEN.eq. 0) FAVAIL(8,N,IREGN) = FAVAIL(5,N,IREGN)                                                                         
!&&&& ONL 
		if(ifuel.eq.8 ) then
			do hms=1,3
...
!        HFAvl(hms,iregn,n)=max(HFAvl(hms,iregn,n-1),HSTAT(N-1,IREGN,hms)/(AltSta(iregn,1,n)*MSStkS(hms,iregn)))
			HFAvl(hms,iregn,n)=FAVAIL(8,n,iregn)
...
!...Write out fuel availability.  (Note:  The number for hydrogen here is not used, HFAvl is used instead)  !sb H2 Project
...
!      do iregn=1,mnumcr   !sb this section is removed in H2 project
...
! ...   cng   !sb H2 project don't add gas used for hydrogen if HMM on
      IF (EXHYDROGEN .EQ.1) THEN
      ...
      ELSE
        QNGTR(IREGN,N) = TQLDV(4,IREGN,N)      + &                         ! ldv
                         ...
                         QMTRR(4,IREGN,N)      + &                         ! passenger rail LNG       
                         SUM(TFRBTU_F_T(N,1:3,4,IREGN)) + &                ! heavy truck
                        (TQLDV(7,IREGN,N)/0.7) + &                         ! Add hydrogen FCVs to NG if HMM is off and not using restart file
                        (FLTFUELBTU(iregn,7,n)/0.7)                        ! Add hydrogen fleet FCVs to NG if HMM is off and not using restart file
      ENDIF
! ... lpg
...
! ... hydrogen     !sb from H2 project
        IF(EXHYDROGEN .eq. 1) THEN                                                            
           QHYTR(IREGN,N) = TQLDV(7,IREGN,N)      + &                         ! ldv
                         ...
                         QMTBR(2,8,IREGN,N)    + &                         ! intercity bus 
                         QMTBR(3,8,IREGN,N)    + &                         ! school bus                                                                                                                                                          
                         SUM(TFRBTU_F_T(N,1:3,7,IREGN))                    ! heavy truck previously 6						 
	        QHMM(PHTDM_LOC, iregn,N) = max(0.0 ,QHYTR(IREGN,N) - QHMM(PHTDM_CEN, iregn,N)) ! Fleet is CEN
		    write(*,*)'PHTDM_LOC PHTDM_CEN EXHYDROGEN iregn, N, QH2TRFV QHYTR QHMMCEN PHMM(PHTDM_CEN) tfrbtu_F_T', &
		     PHTDM_LOC,PHTDM_CEN, EXHYDROGEN, iregn, N,QH2TRFV(iregn,N) , &
		     QHYTR(IREGN,N) , QHMM(PHTDM_CEN, iregn,N) , PHMM(PHTDM_CEN, iregn,N),SUM(TFRBTU_F_T(N,1:3,7,IREGN))
        ENDIF ! h2                                                                                                                                                                                                                            
! ... diesel
