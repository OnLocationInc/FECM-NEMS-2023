...
! These include files are all in the Global Data Structure (i.e., the RESTART file)
      ...
      REAL*4 FSUM,FFF,ELECLOSS,H2LOSS  ! &&&& ONL HMM efficiency losses
      REAL modREFCON(19,5,MNUMYR)
...
! Index Constants for Industrial Tables 35 to 44
  ...
  character*60 nems,cmd*255
      ...
      REAL AGSCALPR
      real indccs(mnumyr)	! CCS from IDM (only cement currently)
      COMMON /AGSC/ AGSCALPR
      real MMBTU_HY
	  Parameter (MMBTU_HY = 0.13445)
...
! &&&& ONL subtract hydrogen blending, as the fuel needed make it is accounted for elsewhere (QHMM(PHNGMM) is blending amount)
! don't add QNGHM(11,IY) here becuase already added to QNGAS in Subroutine SUMQAS
      T1(20,IY,IS) = QNGAS(11,IY)+QGPTR(11,IY)+QLPIN(11,IY)+QNGLQ(11,IY) - QHMM(PHNGMM,11,IY)/1000. + DAC_NGC_CS(11,IY) / 1000. !- QHYTR(11,IY) / .7 
...
! remove nonbiogenic from msw/renew and put into other (wncmsel)
      T1(23,IY,IS)=QTRAS(11,IY) - WNCMSEL(IY,11) - QGERS(11,IY) - QSTRS(11,IY) - &
                   QSTCM(11,IY) - QPVCM(11,IY) - QPVRS(11,IY) - &
                   QBMAS(11,IY) - QHOAS(11,IY) - QETTR(11,IY)
      T1(24,IY,IS) = QEIEL(11,IY) + WNCMSEL(IY,11) !  + QHYTR(11,IY)  ONL exlude hydrogen since inputs to H2 are included in fuels
...
! RESIDENTIAL
      ...
      DO 20 IR=1,11
          IF (IR .EQ. 10) CYCLE
          ICY=IY+BASEYR-1
            QELAS(IR,IY) = QELAS(IR,IY) + DAC_ELC_CS(IR,IY) * 3.41214 / 1000. ! ONL: DAC
          ELECLOSS = QTSEL(IR,IY) - QELAS(IR,IY)
! &&&& ONL  would work exceptconvergence problems will show up as losses.
          H2LOSS = (QNGHM(11,IY) +QCLHM(11,IY) + QBMHM(11,IY) + QELHM(11,IY)- sum(QHMM(1:PHNUMSEC,11,IY))/1000.) ! &&&& ONL only works nationally, 
!          as demand for hydrogen in a region may not match fuel provided in that region because of trade
!          write(*,*)'IY H2LOSS sum(QHMM(1:PHNUMSEC,11,IY))', IY, H2LOSS, sum(QHMM(1:PHNUMSEC,11,IY))
          T2( 1,IR,IY,IS)=QDSRS(IR,IY)
!          T2( 2,IR,IY,IS)=QKSRS(IR,IY) ! ONL, this is no longer used
...
! INDUSTRIAL
          ...
          T2(115,IR,IY,IS)=QNGIN(IR,IY) - QGTLRF(IR,IY) + DAC_NGC_CS(IR,IY) / 1000. ! ONL: DAC
          T2(131,IR,IY,IS)=(QHMM(PHIDM_LOC,IR,IY)+QHMM(PHIDM_CEN,IR,IY))/1000.0   ! HMM industrial H2  why 11 before ?? &&&& ONL
          T2(116,IR,IY,IS)=QLPIN(IR,IY)
         
          T2(120,IR,IY,IS)=QGTLRF(IR,IY)
          T2(32,IR,IY,IS)=QNGIN(IR,IY)+QLPIN(IR,IY) + DAC_NGC_CS(IR,IY) / 1000. ! ONL: DAC
...
! cellulose and biodiesel losses are calculated from conversion factor differences; no need to share
      ...
      IF ((QMGTR(11,IY)-QMGBS(11,IY)) .NE. 0.0 .AND. CONEFF(IY) .NE. 0.0) T2(73,IR,IY,IS) = T2(73,IR,IY,IS) + &
          ...
          T2(38,IR,IY,IS)=QELIN(IR,IY) + DAC_ELC_CS(IR,IY) * 3.41214 / 1000.		! ONL: DAC
          T2(39,IR,IY,IS)=QTSIN(IR,IY) - QBMRF(IR,IY) + T2(73,IR,IY,IS) + DAC_ELC_CS(IR,IY) * 3.41214 / 1000. + DAC_NGC_CS(IR,IY) / 1000.		! ONL: DAC
          if(ir.eq.11) T44(58,IY,IS) = 1000*T2(39,11,IY,IS) ! copy industrial delivered energy total for use in Table 44 balancing item

          T2(40,IR,IY,IS)=(QELIN(IR,IY) + DAC_ELC_CS(IR,IY) * 3.41214 / 1000.) / QELAS(IR,IY) * ELECLOSS		! ONL: DAC
          T2(134,IR,IY,IS)=H2LOSS * T2(131,IR,IY,IS)/ (sum(QHMM(1:PHNUMSEC,IR,IY))/1000. + .001)! &&&& fraction of ONL Hydrogen losses IND 
          T2(41,IR,IY,IS)=FSUM(T2(39,IR,IY,IS),2)+T2(134,IR,IY,IS) ! &&&& ONL
! TRANSPORTATION
	  ...
	  T2(128,IR,IY,IS)=QNGLQ(IR,IY)
          T2(50,IR,IY,IS)=QNGTR(IR,IY) ! &&&& ONL only true if no hydrogen
		  ...
		  T2(135,IR,IY,IS)=H2LOSS * T2(53,IR,IY,IS)/(sum(QHMM(1:PHNUMSEC,11,IY))/1000. + .0001) ! &&&& ONL Hydrogen losses
          T2(57,IR,IY,IS)=FSUM(T2(55,IR,IY,IS),2) + T2(135,IR,IY,IS) ! &&&& ONL
! Unspecified sector
...
! DELIVERED ENERGY CONSUMPTION FOR ALL SECTORS
          ...
          T2(117,IR,IY,IS)=QNGAS(IR,IY) - QNGEL(IR,IY) - QNGHM(IR,IY) - QGTLRF(IR,IY) + DAC_NGC_CS(IR,IY) / 1000. 	! ONL: DAC; includes blended H2
...
! this means we subtract the gas production number
          ...
          T2(75,IR,IY,IS)=(sum(QHMM(1:PHNUMSEC,IR,IY)) - QHMM(PHNGMM,IR,IY))/1000.   ! ONL total minus blending;QHYTR(IR,IY) is just transportation
          T2(74,IR,IY,IS)=QELAS(IR,IY)
          T2(76,IR,IY,IS)=T2(66,IR,IY,IS) + T2(67,IR,IY,IS) + FSUM(T2(71,IR,IY,IS),5)
          T2(77,IR,IY,IS)=ELECLOSS
		  T2(136,IR,IY,IS)=H2LOSS * T2(75,IR,IY,IS)/(sum(QHMM(1:PHNUMSEC,11,IY))/1000.+.0001) ! ! &&&& ONL Hydrogen losses
          T2(78,IR,IY,IS)=FSUM(T2(76,IR,IY,IS),2) + T2(136,IR,IY,IS) ! &&&& ONL
! ELECTRIC GENERATORS (INCLUDES NONTRADITIONAL COGEN FOR AEO03)
		  ...
		  T2(137,IR,IY,IS)=(QHMM(PHEMM1,11,IY)+QHMM(PHEMM2,11,IY)+QHMM(PHEMM3,11,IY))/1000.0
...
! TOTAL PRIMARY
		  ...
		  ! subtract hydrogen in blending for NG because fuels to make hydrogen are included; leave NG to HMM incl in QNGAS
          T2(118,IR,IY,IS)=QNGAS(IR,IY) - QGTLRF(IR,IY) - QHMM(PHNGMM,IR,IY)/1000. + DAC_NGC_CS(IR,IY)/1000.  	! ONL:remove -QHYTR(IR,IY)/.7,add DAC
          T2(97,IR,IY,IS)=T2(118,IR,IY,IS) + T2(120,IR,IY,IS) + T2(128,IR,IY,IS) +   &
                          T2(116,IR,IY,IS) + T2( 52,IR,IY,IS) + T2( 49,IR,IY,IS)
! Fuel Consumption
		  T2(2,IR,IY,IS) = QCLHM(IR,IY)  !coal
		  T2(130,IR,IY,IS) = QNGHM(IR,IY)  !NG
		  T2(133,IR,IY,IS) = QBMHM(IR,IY)  !BM
		  T2(139,IR,IY,IS) = QELHM(IR,IY)  !EL
		  T2(140,IR,IY,IS) = T2(2,IR,IY,IS) + T2(130,IR,IY,IS) + T2(133,IR,IY,IS) + T2(139,IR,IY,IS)  !total
...
!  subtract nonbiogenic from msw/renew and add to total summed line
          T2(103,IR,IY,IS)=QBMRS(IR,IY) + QBMCM(IR,IY) + QTRIN(IR,IY) - QBMRF(IR,IY) + QTRSN(IR,IY) + &
                           QTREL(IR,IY) - WNCMSEL(IY,IR) + QBMHM(IR,IY)   ! ONL add bio to hydrogen
!          T2(105,IR,IY,IS)=QHYTR(IR,IY)  ! this is tran hdyrogen and shouldn't be in primary energy
          T2(123,IR,IY,IS)=WNCMSEL(IY,IR)
          T2(104,IR,IY,IS)=QEIEL(IR,IY)
          T2(106,IR,IY,IS)= FSUM(T2(96,IR,IY,IS),2) + FSUM(T2(101,IR,IY,IS),4) + &
...
!       --- ENERGY CONSUMPTION
        ELECLOSS = QTSEL(11,IY) - QELAS(11,IY)
		H2LOSS = (QNGHM(11,IY) +QCLHM(11,IY) + QBMHM(11,IY) + QELHM(11,IY)- sum(QHMM(1:PHNUMSEC,11,IY))/1000.) !
        T6(12,IY,IS)= QELIN(11,IY) + DAC_ELC_CS(11,IY) * 3.41214 / 1000.		! ONL: DAC
        T6(152,IY,IS)=(QHMM(PHIDM_LOC,11,IY)+QHMM(PHIDM_CEN,11,IY)+QHMM(PHLFMM,11,IY))/1000.0  ! &&&& ONL: ind+ref		
        T6(38,IY,IS)= QNGIN(11,IY) - INQNGPF(11,IY) / 1000. - QGTLRF(11,IY) - RFQNGPF(11,IY) / 1000. + DAC_NGC_CS(11,IY) / 1000.	! ONL: DAC
        T6(64,IY,IS)= INQNGPF(11,IY) / 1000. + RFQNGPF(11,IY) / 1000.
        T6(39,IY,IS)= QLPIN(11,IY)
        T6(82,IY,IS)= QNGLQ(11,IY)
! for coal, subtract out coal gasification coal, but leave in losses
! this means we subtract the gas production number
...
! Digress to separate Refinery consumption (including ethanol plant for CL, NG, EL) from Industrial for diagnostics
      ! catalytic coke (QCCRF) is sub-category of petroleum coke; converted to quads in CAPFLATE
           ...
           IF (CONEFF(IY) .NE. 0.0) T6(112,IY,IS) = T6(112,IY,IS) + &
             ...
             T6(150,IY,IS)= QHMM(PHLFMM,11,IY)/ 1000. !ONL HMM refining
	         T6(151,IY,IS)= T6(150,IY,IS)/(sum(QHMM(1:PHNUMSEC,MNUMCR,IY))/1000. + .0001) * H2LOSS   !  &&&& ONL 
! ONL: DAC CONSUMPTION
             T6(142,IY,IS) = DAC_NGC_CS(11,IY) / 1000. 
             T6(143,IY,IS) = T6(142,IY,IS)
             T6(144,IY,IS) = DAC_ELC_CS(11,IY) * 3.41214 / 1000.
             T6(145,IY,IS) = FSUM(T6(143,IY,IS),2)
             T6(146,IY,IS) = T6(144,IY,IS) / QELAS(11,IY) * ELECLOSS
             T6(147,IY,IS) = FSUM(T6(145,IY,IS),2)
             T22(89,IY,IS) = max(0.0, T6(142,IY,IS)*eNGin(IY)*C_CO2_FACTOR - Q_NG_CCS(11,IY))		! ONL: reporting of DAC NG CO2 separately (also incuded in total industry)
             ...
             T6(141,IY,IS) = QISINPF(MNUMCR,IY)
	         ...
	         T6(148,IY,IS)=(QHMM(PHIDM_LOC,11,IY)+QHMM(PHIDM_CEN,11,IY))/1000.0 ! &&&& ONL industrial
...
! this means we subtract the gas production number
             ...
             T6( 83,IY,IS) = FSUM(T6( 80,IY,IS),2) + T6( 39,IY,IS) !OL fix
             T6( 87,IY,IS) = FSUM(T6( 84,IY,IS),2)
             T6( 90,IY,IS) = T6( 79,IY,IS) + T6( 83,IY,IS) + FSUM(T6( 87,IY,IS),3)+T6(148,IY,IS)
             T6( 91,IY,IS) = (QELIN(11,IY) - REFCON(ixEL,5,IY) / 1000.) / QELAS(11,IY) * ELECLOSS
             T6(149,IY,IS)= H2LOSS * T6(148,IY,IS)/(sum(QHMM(1:PHNUMSEC,11,IY))/1000. + .0001) ! &&&& ONL
             T6( 92,IY,IS) = FSUM(T6( 90,IY,IS),2)+T6(149,IY,IS)
! Calculate total industrial natural gas
        T6(13,IY,IS) = FSUM(T6(38,IY,IS),2) + T6(64,IY,IS) + T6(106,IY,IS) + T6(82,IY,IS)
!       --- CONSUMPTION PER UNIT OF OUTPUT
        IF (T6(3,IY,IS) .NE. 0.0) THEN
          T6(25,IY,IS)= (QELIN(11,IY) + DAC_ELC_CS(11,IY) * 3.41214 / 1000.) * 1000. / T6(3,IY,IS)		! ONL: DAC
          T6(40,IY,IS)= (QNGIN(11,IY) * 1000. - INQNGPF(11,IY) - QGTLRF(11,IY) * 1000. - RFQNGPF(11,IY) + DAC_NGC_CS(11,IY) / 1000.) / T6(3,IY,IS)		! ONL: DAC
...
! this means we subtract the gas production number
          ...
          T6(118,IY,IS) = 1000. * &
           ...
           IF (CONEFF(IY) .NE. 0.0) T6(118,IY,IS) = T6(118,IY,IS) + 1000. / T6(3,IY,IS) * &
               ((0.9751*CLLETHCD(11,IY) * RDAYS * (CFBMQ(IY) * 42. / CONEFF(IY) - CFPET)) / 1000000.)
          T6(36,IY,IS)= (QELIN(11,IY) + DAC_ELC_CS(11,IY) * 3.41214 / 1000.) / QELAS(11,IY) * ELECLOSS * 1000. / T6(3,IY,IS)	! ONL: DAC
          T6(154,IY,IS)=(QHMM(PHIDM_LOC,11,IY)+QHMM(PHIDM_CEN,11,IY)+QHMM(PHLFMM,11,IY))/1000.0 / T6(3,IY,IS)	! &&&& ONL
          T6(155,IY,IS)= T6(152,IY,IS)/(sum(QHMM(1:PHNUMSEC,11,IY))/1000. + .0001) * H2LOSS / T6(3,IY,IS)	  ! &&&& ONL 
        ENDIF
        ...
        T6(23,IY,IS)= (QELIN(11,IY) + DAC_ELC_CS(11,IY) * 3.41214 / 1000.) / QELAS(11,IY) * ELECLOSS	! ONL: DAC
        T6(153,IY,IS)= T6(152,IY,IS)/(sum(QHMM(1:PHNUMSEC,MNUMCR,IY))/1000. + .0001) * H2LOSS      ! &&&& ONL
...
!       --- GENERATION BY FUEL TYPE (ADD NONTRADITIONAL COGEN FOR AEO03)
	...
	T8(74,IY,IS)=sum(QHMM(PHEMM1:PHEMM3,11,IY))/1000. ! &&&& ONL don't have by NERC region
...
! Generation by Everybody
        ...
        T8(62,IY,IS) = T8( 5,IY,IS) + T8(45,IY,IS) + T8(12,IY,IS) + T8(14,IY,IS) + T8(74,IY,IS) ! ONL hydrogen
...
! PRODUCTION
        T13( 1,IY,IS) =(OGQNGREP(1,IY) + OGQNGREP(2,IY) + &
                        ...
                        OGQNGREP(5,IY) + OGQNGREP(6,IY) + &
                        OGQNGREP(7,IY) + OGQNGREP(8,IY) + OGSHALENG(IY)) * .001
        T13( 2,IY,IS) = (OGPRSUP(IY) + (QHMM(PHNGMM,11,IY)/CFNGC(IY))) * .001 !&&&ONL
!     --- NET IMPORTS
...
!	  --- Hydrogen to Natural Gas	&&&& ONL
        T13(58,IY,IS)=QHMM(PHNGMM,11,IY)/1000./CFNGN(IY)  ! ONL
!     --- CONSUMPTION BY SECTOR - Convert from quads
        ...
        ENDIF
        T13(57,IY,IS) = DAC_NGC_CS(11,IY)/1030.		! ONL: DAC
...
!     --- Gas-to-Liquids
        ...
        ! hydrogen       
        T13(38,IY,IS)= QNGHM(11,IY)/CFNGN(IY) !&&ONL 
        ! total consumption
        T13(15,IY,IS)=FSUM(T13(8,IY,IS),7) + T13(17,IY,IS) + T13(37,IY,IS) + FSUM(T13(56,IY,IS),2)+ T13(38,IY,IS)	! ONL: DAC
         ! total industrial
        T13(35,IY,IS) = T13(10,IY,IS) + T13(12,IY,IS) + T13(17,IY,IS) + T13(37,IY,IS) + T13(38,IY,IS) + T13(57,IY,IS)
!     --- DISCREPANCY
...
!         --- ELECTRIC POWER SECTOR
          ...
          T17(22,IR,IY,IS) = em_elec(4,IR,ICY) + em_elec(8,IR,ICY)	! ONL: BECCS accounts for net-zero biomass captured
...
!         --- FUEL TOTALS
          ...
          T17(32,IR,IY,IS) = FSUM(T17(25,IR,IY,IS),4) - (Q_DAC_CS(IR,IY) - Q_NG_CCS(IR,IY))	! ONL: DAC - Subtract DAC less NG CCS
          T17(31,IR,IY,IS) = Q_DAC_CS(IR,IY) - Q_NG_CCS(IR,IY)	! ONL: DAC - Less DAC NG CCS
!         T118(74,iy,is)= T17(29,IR,IY,IS) ! See Table 118 if t17(29,,,,) changed. Search code for t17(29
          ...
          IF (MC_NP(IR,IY) .NE. 0.0) T17(33,IR,IY,IS) =T17(32,IR,IY,IS) / MC_NP(IR,IY)	! ONL: additional line to deal with DAC
        ENDDO
  170   CONTINUE
...
!  totals of supply and disposition
 ...
 !  Carbon Dioxide Emissions by End-Use (Some T22 calculations and assignments in end-use table sections, t4, t5, t46)
      DO 220 IY=1,LASTYR
        do ikiln=1,MAX_KILNS
	      INDCCS(IY) = INDCCS(IY) + I_CPTY(ikiln)*I_CFR(ikiln,IY)/1e6	!MMT CO2/yr
	    enddo
		do IYY=2,IY
            do ICY=1,9
                if (I_CPTY_NEW(ICY,IYY)-I_CPTY_NEW(ICY,IYY-1).gt.0.0) then
                    INDCCS(IY) = INDCCS(IY) + (I_CPTY_NEW(ICY,IYY)-I_CPTY_NEW(ICY,IYY-1))*I_CFR_NEW(ICY,IYY,IY)/1e6	!MMT CO2/yr
                endif
            enddo
		enddo
        ICY=IY+BASEYR-1
        T22(41,IY,IS) = indcarb(REFCON(1,1,IY),IY)-&
                        ((CO2TOEOR45q(6,iy)+CO2TOSALINE45q(6,iy)+CO2TOEOR245q(6,iy)+CO2TOSALINE245q(6,iy))*365./1000.) - &
                        ((CO2TOEOR45q(9,iy)+CO2TOSALINE45q(9,iy)+CO2TOEOR245q(9,iy)+CO2TOSALINE245q(9,iy))*365./1000.)	! ONL: Ind CCS
        ...
        T22(46,IY,IS) = indcarb(CEMENTCON(1,1,IY),IY) - INDCCS(IY)
        T22(47,IY,IS) = indcarb(STEELCON(1,1,IY),IY) - I_CCS_Q(IY)
        ...
        T22(62,IY,IS) = sum(em_indy(1:IEL_I,11,ICY))-(T22(57,IY,IS)+T22(61,IY,IS))-T22(89,IY,IS)	! benchmark discrepancy; ONL: incl DAC
...
! ONL begin: CO2 to enhanced oil recovery from ind CCS (soruce 10 in old xTLCCS from LFMM)
        OGCO2AVLs(1:8,3,IY)  = OGCO2AVLs(1:8,10,IY)
        OGCO2AVLs(1:8,10,IY) = 0.0
        
        OGCO2PUR(1:8,3,IY) = OGCO2PUR(1:8,10,IY)
        OGCO2PUR(1:8,10,IY) = 0
        
        OGCO2PRCs(1:8, 3,IY) = OGCO2PRCs(1:8, 10,IY)
        OGCO2PRCs(1:8, 10,IY) = 0
! ONL end
   
        T25( 1:13,IY,IS) = OGCO2AVLs(8,1:13,IY) / 18000.

! ONL begin: CO2 to enhanced oil recovery from ind CCS (sources 3,6,9 eth, H2, NGP)
        IF ((OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY)).GT.0.0) THEN      
                T25( 3,IY,IS) = OGCO2AVLs(8,3,IY)*OGCO2qlfs(8,3,IY)/(OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY))/18000
                T25( 1,IY,IS) = OGCO2AVLs(8,3,IY)*OGCO2qlfs(8,6,IY)/(OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY))/18000
                T25( 9,IY,IS) = OGCO2AVLs(8,3,IY)*OGCO2qlfs(8,9,IY)/(OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY))/18000
        ELSE
               T25( 3,IY,IS) = 0.0
               T25( 1,IY,IS) = 0.0
               T25( 9,IY,IS) = 0.0
...
! ONL begin: CO2 to enhanced oil recovery from ind CCS (sources 3,6,9 eth, H2, NGP)
        IF ((OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY)).GT.0.0) THEN  
                T25(17,IY,IS) = OGCO2PUR(8,3,IY)*OGCO2qlfs(8,3,IY)/(OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY))/18000
                T25(15,IY,IS) = OGCO2PUR(8,3,IY)*OGCO2qlfs(8,6,IY)/(OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY))/18000
                T25(23,IY,IS) = OGCO2PUR(8,3,IY)*OGCO2qlfs(8,9,IY)/(OGCO2qlfs(8,3,IY)+OGCO2qlfs(8,6,IY)+OGCO2qlfs(8,9,IY))/18000
        ELSE
               T25( 17,IY,IS) = 0.0
               T25( 15,IY,IS) = 0.0
               T25( 23,IY,IS) = 0.0
...
!            so assign a region to each division it contains:
           ...
           T34(60,IR,IY,IS) = FSUM(T34(41,IR,IY,IS),19) + T34(101,IR,IY,IS)
...
!          --- ENERGY CONSUMPTION
           ...
           T35(71,IY,IS) = QHMM(PHLFMM,11,IY)  !hydrogen &&&ONL
           ...
           IF (CONEFF(IY) .NE. 0.0) T35(48,IY,IS) = T35(48,IY,IS) + 1000. * &
                   ((0.9751*CLLETHCD(11,IY) * RDAYS * (CFBMQ(IY) * 42. / CONEFF(IY) - CFPET)) / 1000000.)
           T35(12,IY,IS) = FSUM(T35(8,IY,IS),4) + T35(48,IY,IS) + T35(71,IY,IS)
...
!       --- ENERGY CONSUMPTION PER UNIT OF OUTPUT
        ...
        T40(18,IY,IS) = INDCARB(CEMENTCON(1,1,IY),IY) - INDCCS(IY) + INCM_PREM(11,IY)*44.0/12.0   ! function indcarb calcs combustion CO2 emissions
        T40(47,IY,IS) = GHG_PROCESSIN(11,IY) - INCM_PREM(11,IY)*44.0/12.0            ! Cement and lime process CO2 emissions
        IL=18
        INDDIR=11
...
!       --- ENERGY CONSUMPTION PER UNIT OF OUTPUT
        ...
        T41(24,IY,IS) = INDCARB(STEELCON(1,1,IY),IY) - I_CCS_Q(IY) ! function indcarb calcs carbon emissions ONL Mod subtract fuel emissions captured
        IL=24
        INDDIR=12
...
! Bus transport
           ...
           c8(5)=fsum(emel(1,1,iy),4)/qelas(11,iy) !   elec      ! ONL mod add electric buses
           c8(6)=sum(TRQBUS(:,1,IY))
           c8(7)=sum(TRQBUS(:,2,IY))
           c8(8)=sum(TRQBUS(:,5,IY))
           c8(9)=sum(TRQBUS(:,6,IY))
           c8(10)=sum(TRQBUS(:,7,IY))
           T22(29,iy,is) = dot_product(c8(6:10),c8(1:5))              ! Bus Transportation ! ONL mod add electric buses
! freight trucks
           c8=0.
           c8(1)=(em_tran(1,11,icy)+em_tran(3,11,icy))/qMGtr(11,iy) ! motor gasoline adjusted for ethanol
           c8(2)=(em_tran(6,11,icy)+em_tran(7,11,icy))/qDStr(11,iy) ! diesel with biodiesel adjustment
           ...
           c8(4)=0.                      !   ???
           c8(5)=ePRtr(iy)* C_CO2_FACTOR !   propane
           if(qETtr(11,iy).gt.0) c8(6)=em_tran(2,11,icy)/qETtr(11,iy) ! e85  ! ONL mod
           c8(7)=fsum(emel(1,1,iy),4)/qelas(11,iy)                   !   elec! ONL mod
           T22(30,iy,is) = dot_product(TRQFTRK(1:7,IY),c8(1:7))     !  Freight Trucks ! ONL mod increase from 5 (still ignoring H2)
! passenger rail
           c8=0.
...
! CARBON IS IN Short Tons, NOT Metric Tons LIKE IN TABLE 17
         ...
         ! ONL begin: em_elec(9,11,ICY) replace em_elec(8,11,ICY)  BECCS
...
!endif
      ...
      INTEGER IR,IS,IY,IY1,COUNT,I,ICY,reg
...
!       --- ETHANOL
        ...
        T102( 132,IR,IY,IS) = REFPRODETRET(PADD,IY) * RDAYS * 42. / 1000000.	! ONL: ethanol CCS
        T102( 133,IR,IY,IS) = REFPRODETNEW(PADD,IY) * RDAYS * 42. / 1000000.	! ONL: ethanol CCS       
...
! plant capacity
        ...
        T102(130,IR,IY,IS) = REFCAPETRET(PADD,IY) * RDAYS * 42. / 1000000.	! ONL: ethanol CCS
        T102(131,IR,IY,IS) = REFCAPETNEW(PADD,IY) * RDAYS * 42. / 1000000.	! ONL: ethanol CCS        
...
!industrial cement
T111(176,IY,IS)=PBMINCL(3,1,IY)
T111(177,IY,IS)=PBMINCL(3,2,IY)
T111(178,IY,IS)=PBMINCL(3,3,IY)
T111(179,IY,IS)=PBMINCL(3,4,IY)
T111(180,IY,IS)=PBMINCL(3,5,IY)
T111(181,IY,IS)=PBMINCL(3,6,IY)
T111(182,IY,IS)=PBMINCL(3,7,IY)
T111(183,IY,IS)=PBMINCL(3,8,IY)
T111(184,IY,IS)=PBMINCL(3,9,IY)
T111(185,IY,IS)=PBMINCL(3,10,IY)
T111(186,IY,IS)=PBMINCL(3,11,IY)
T111(187,IY,IS)=PBMINCL(3,12,IY)
T111(188,IY,IS)=PBMINCL(3,13,IY)
T111(189,IY,IS)=PBMINCL(3,14,IY)
T111(190,IY,IS)=PBMINCL(3,15,IY)
T111(191,IY,IS)=PBMINCL(3,16,IY)
if (SUM(QBMINCL(3,1:16,IY)).ne.0.0) then
T111(192,IY,IS)=(PBMINCL(3,1,IY)*QBMINCL(3,1,IY)+PBMINCL(3,2,IY)*QBMINCL(3,2,IY)+ &
                 PBMINCL(3,3,IY)*QBMINCL(3,3,IY)+PBMINCL(3,4,IY)*QBMINCL(3,4,IY)+ &
                 PBMINCL(3,5,IY)*QBMINCL(3,5,IY)+PBMINCL(3,6,IY)*QBMINCL(3,6,IY)+ &
                 PBMINCL(3,7,IY)*QBMINCL(3,7,IY)+PBMINCL(3,8,IY)*QBMINCL(3,8,IY)+ &
                 PBMINCL(3,9,IY)*QBMINCL(3,9,IY)+PBMINCL(3,10,IY)*QBMINCL(3,10,IY)+ &
                 PBMINCL(3,11,IY)*QBMINCL(3,11,IY)+PBMINCL(3,12,IY)*QBMINCL(3,12,IY)+ &
                 PBMINCL(3,13,IY)*QBMINCL(3,13,IY)+PBMINCL(3,14,IY)*QBMINCL(3,14,IY)+ &
                 PBMINCL(3,15,IY)*QBMINCL(3,15,IY)+PBMINCL(3,16,IY)*QBMINCL(3,16,IY))/SUM(QBMINCL(3,1:16,IY))
                 else
                  T111(192,IY,IS)=0.0
                 endif
      ENDDO
...
!  private forestry residue
 ...
 !industrial cement       
        T112(176,IY,IS)=QBMINCL(3,1,IY)
        T112(177,IY,IS)=QBMINCL(3,2,IY)
        T112(178,IY,IS)=QBMINCL(3,3,IY)
        T112(179,IY,IS)=QBMINCL(3,4,IY)
        T112(180,IY,IS)=QBMINCL(3,5,IY)
        T112(181,IY,IS)=QBMINCL(3,6,IY)
        T112(182,IY,IS)=QBMINCL(3,7,IY)
        T112(183,IY,IS)=QBMINCL(3,8,IY)
        T112(184,IY,IS)=QBMINCL(3,9,IY)
        T112(185,IY,IS)=QBMINCL(3,10,IY)
        T112(186,IY,IS)=QBMINCL(3,11,IY)
        T112(187,IY,IS)=QBMINCL(3,12,IY)
        T112(188,IY,IS)=QBMINCL(3,13,IY)
        T112(189,IY,IS)=QBMINCL(3,14,IY)
        T112(190,IY,IS)=QBMINCL(3,15,IY)
        T112(191,IY,IS)=QBMINCL(3,16,IY)
        T112(192,IY,IS)=FSUM(T112(176,IY,IS),16)
      ENDDO
...
!        Fuel Expenses  (adjust for value of 45Q revenue that is revenue adjusted embedded in fuel prices)

         DO IY1 = 1 , LASTYR	! used in the NPV and cumulative calcs
            TMP(IY1) = Fuel_VOM(IR,IY1) - Carbon_Rev(IR,IY1) + &
            ((sum(CO2_CCS(0,1:7,2,IY1))/18000 * CCS_EOR_45Q(IY1)) + (CO2_CCS(0,-1,2,IY1)/18000 * CCS_SALINE_45Q(IY1)))/(1-0.24)		! ONL:   adjust for value of 45Q revenue that is revenue adjusted embedded in fuel prices

         END DO

         T116(7,IR,IY,IS) = (Fuel_VOM(IR,IY) - Carbon_Rev(IR,IY)) * SCALPR * 0.001 + &
            ((sum(CO2_CCS(0,1:7,2,IY))/18000 * CCS_EOR_45Q(IY)) + (CO2_CCS(0,-1,2,IY)/18000 * CCS_SALINE_45Q(IY))) * SCALPR/1000 / (1-0.24)		! ONL:   adjust for value of 45Q revenue that is revenue adjusted embedded in fuel prices
write(6,*)'45Qcred',IY,((sum(CO2_CCS(0,1:7,2,IY))/18000 * CCS_EOR_45Q(IY)) + (CO2_CCS(0,-1,2,IY)/18000 * CCS_SALINE_45Q(IY))) * SCALPR/1000,&
           ((sum(CO2_CCS(0,1:7,2,IY))/18000 * CCS_EOR_45Q(IY)) + (CO2_CCS(0,-1,2,IY)/18000 * CCS_SALINE_45Q(IY))) * SCALPR/1000 / (1-0.24)
         IF (IY .GT. CUMCAPADD - 1989) THEN
...
!        Total
         ...
         T116(10,IR,IY,IS) = (G_INST_ALL(IR,IY) + T_OVR(IR,IY) + RET_INV(IR,IY) + Total_FOM(IR,IY) + &
              CP_ADD(IR,IY) + Non_Fuel_VOM(IR,IY)+ T_DomEcon(IR,IY) + &    ! remove fuel costs here & add below now that eqn is long ONL MOD
              T_DomFirm(IR,IY) - T_IntExp(IR,IY) + T_IntImp(IR,IY) + Total_RCC(IR,IY) + &
              Total_RIC(IR,IY) + URPSPEN(IR,IY) + ECSTNREE(IR,IY)*1000.0 ) * SCALPR * 0.001 + T116(7,IR,IY,IS)	! ONL: added T116(7)
         IF (IY .GT. CUMCAPADD - 1989) THEN
         ...
         T116(30,IR,IY,IS) = (G_ANN(IR,IY) + T_ANN(IR,IY) + RET_Cst(IR,IY) + Total_FOM(IR,IY) + &
              CP_ADD(IR,IY) + Non_Fuel_VOM(IR,IY) + T_DomEcon(IR,IY) + &  ! ONL MOD remove fuel costs here & add below now that eqn is long
              T_DomFirm(IR,IY) - T_IntExp(IR,IY) + T_IntImp(IR,IY) + Total_RCC(IR,IY) + &
              Total_RIC(IR,IY) + URPSPEN(IR,IY) + ECSTNREE(IR,IY)*1000.0 ) * SCALPR * 0.001 + T116(27,IR,IY,IS) ! ONL: added T116(7)
         IF (IY .GT. CUMCAPADD - 1989) THEN
...
!   Carbon Dioxide emissions and allowance price
           T117(24,IY,IS) = em_elec(9,11,ICY) ! ONL: BECCS (9 is now total)
...
!     Compute average marginal hydrogen price over all uses &&&& ONL

        PQsum(IR,IY) = 0.00
        Qsum(IR,IY) = 0.0
		!MMBTU_HY = .1447   ! MMBTU /kg Hydrogen set up elsewhere 

         T125(2,IR,IY,IS) =  HMPRODSEQ(2,IR,IY)  !ng (sequestered)
         T125(1,IR,IY,IS) =  HMGSPRD(IY,IR)  !ng (unsequestered)
		 T125(4,IR,IY,IS) =  HMPRODSEQ(1,IR,IY) !coal (sequestered)
         T125(3,IR,IY,IS) =  HMCLPRD(IY,IR) !coal (unsequestered)
         T125(6,IR,IY,IS) =  HMELPRD(IY,IR)  ! electricity

         T125(7,IR,IY,IS) =  HMURPRD(IY,IR)   ! uranium

         T125(8,IR,IY,IS) =  HMPRODSEQ(3,IR,IY)  !BM(sequestered)
		 T125(5,IR,IY,IS) =  HMBIPRD(IY,IR)    !BM(unsequestered)

		 T125(13,IR,IY,IS) =  sum(T125(1:8,IR,IY,IS))  ! total hydrogen  &&&& need to do T125(1 - 8) first
!          
	    T125(42,IR,IY,IS) = QHMM(PHTDM_LOC,IR,IY)/MMBTU_HY !non fleet vehicles
	    T125(46,IR,IY,IS) = QHMM(PHTDM_CEN,IR,IY)/MMBTU_HY ! fleet vehicles
		T125(43,IR,IY,IS) = QHMM(PHIDM_LOC,IR,IY)/MMBTU_HY !industrial
		T125(44,IR,IY,IS) = QHMM(PHNGMM,IR,IY) /MMBTU_HY !ng blending
		T125(45,IR,IY,IS) = QHMM(PHLFMM,IR,IY) /MMBTU_HY  ! set to 0.0 before need to fix QHMM(PHLFMM,IR,IY) refinery
        T125(10,IR,IY,IS) = SUM(QHMM(PHEMM1:PHEMM3,IR,IY))/MMBTU_HY! sum seasons for electricity
		T125(56,IR,IY,IS) = (QHMM(PHCDM,IR,IY)+QHMM(PHIDM_CEN,IR,IY) + NH3EXPORT(IR,IY))/MMBTU_HY

! Conventional Hydrogen Production (million metric tonnes)

		T125(15,IR,IY,IS) = RFHCXH2IN(MNUMPR,IY) * 5.8 * RDAYS /MMBTU_HY !T125(16,IR,IY,IS) + T125(17,IR,IY,IS) ! Refinery input of hydrogen 
		T125(17,IR,IY,IS) = (hydrogen2ref(iy) * 5.8  +  QHMM(PHLFMM,IR,IY))/MMBTU_HY  !  Merchant from IDM and HMM

        T125(16,IR,IY,IS) = T125(15,IR,IY,IS)- T125(17,IR,IY,IS) !0.0  !  Onsite 
        hmotprd(33:mnumyr,11)=hmotprd(32,11) ! temp - done in main but adding here so ftab can be rerun
        T125(18,IR,IY,IS) = HMOTPRD(IY,11)*1000.0  ! 0.0 !Other Conventional Hydrogen

        T125(14,IR,IY,IS) = T125(15,IR,IY,IS)+ T125(18,IR,IY,IS)  !Conventional Hydrogen Production

!     Fuel Consumption

	    T125(47,IR,IY,IS) = QCLHM(IR,IY)  !coal
		T125(48,IR,IY,IS) = QNGHM(IR,IY)  !NG
		T125(49,IR,IY,IS) = QBMHM(IR,IY)  !BM
		T125(51,IR,IY,IS) = QELHM(IR,IY)  !EL
		T125(52,IR,IY,IS) = sum(T125(47:51,IR,IY,IS))   !total
! Prices
   !fill national numbers for PHMM
!   if (ir.eq.1) then
!     Do iii=1,10
!       if (QHMM(III,11,IY).ne.0.0) then
!         PHMM(III,11,IY)=0.0
!         do reg=1,9
!           PHMM(III,11,IY)=PHMM(III,11,IY)+SCALPR*PHMM(III,reg,IY)*QHMM(III,reg,IY)
!         enddo
!         PHMM(III,11,IY)=PHMM(III,11,IY)/QHMM(III,11,IY)
!       endif
!     enddo
!   endif
       if ((QHMM(PHTDM_LOC,IR,IY)+QHMM(PHTDM_CEN,IR,IY)).gt.0.0) &
          T125(55,IR,IY,IS) =SCALPR*((PHMM(PHTDM_LOC,IR,IY)*QHMM(PHTDM_LOC,IR,IY))+ (PHMM(PHTDM_CEN,IR,IY)*QHMM(PHTDM_CEN,IR,IY)))/(QHMM(PHTDM_LOC,IR,IY)+QHMM(PHTDM_CEN,IR,IY))!tran
	    T125(53,IR,IY,IS) = SCALPR*PHMM(PHIDM_LOC,IR,IY)!industrial
		T125(54,IR,IY,IS) = 0.0
		if ((SUM(QHMM(H2OFFSET+1: H2OFFSET+3,IR,IY)).gt.0.0)) then
			DO III = 1, 3   ! three seasons
				T125(54,IR,IY,IS) = T125(54,IR,IY,IS) + SCALPR*(PHMM(H2OFFSET+III,IR,IY)*QHMM(H2OFFSET+III,IR,IY))
			END DO
			T125(54,IR,IY,IS)= T125(54,IR,IY,IS)/SUM(QHMM(H2OFFSET+1: H2OFFSET+3,IR,IY))!Electricity
			...
			T125(54,IR,IY,IS) = SCALPR* SUM(PHMM(H2OFFSET+1: H2OFFSET+3,IR,IY))/3.
		ENDIF
!  other: now electricity		
! average of all sectoral prices			
         T125(57,IR,IY,IS) = 0.0
!		QHTOT = sum(QHMM(1:10,IR,IY))
		if (SUM(QHMM(1:10,IR,IY)).gt.0.0) then
			DO III = 1, 10   ! 10 sectors
				T125(57,IR,IY,IS) = T125(57,IR,IY,IS) + SCALPR*PHMM(III,IR,IY)*QHMM(III,IR,IY)
			END DO
			T125(57,IR,IY,IS)=  T125(57,IR,IY,IS)/sum(QHMM(1:10,IR,IY))!Electricity
		else 
			T125(57,IR,IY,IS) = SCALPR*(sum(PHMM(1:10,IR,IY)))/10.
        endif


!			T125(57,IR,IY,IS) = PHMM() !Avg marginal

!     Hydrogen Emissions

      T125(62,IR,IY,IS) =  co2capfuel(2,IR,IY)  !seq ngas   
      T125(66,IR,IY,IS) =  (QNGHM(IR,IY) * ENGEL(IY)* (44./12.)) - co2capfuel(2,IR,IY) !unseq ngas 
      T125(66,IR,IY,IS) =   (HMGSPRD(IY,IR) * MMBTU_HY * ENGEL(IY)* (44./12.))/1000      
      ! total demand
      T125(67,IR,IY,IS)=sum(T125(42:46,IR,IY,IS))+ T125(10,IR,IY,IS)+ T125(56,IR,IY,IS)
      T125(63,IR,IY,IS) =  co2capfuel(3,IR,IY)  !biomass
      T125(64,IR,IY,IS) =  co2capfuel(1,IR,IY)  !coal (sequestered)
!      T125(9,IR,IY,IS) =  (QCLHM(IR,IY) * ECLEL(IY) * (44./12.)) - co2capfuel(1,IR,IY) !coal (unsequestered)
      T125(9,IR,IY,IS) =   (HMCLPRD(IY,IR) * MMBTU_HY * ECLEL(IY)* (44./12.))/1000
! Total Capacity
       T125(65,IR,IY,IS) =sum(invcst(1:50,IR,IY))
...
! ONL begin: Direct Air Capture
         DO IY = 1,LASTYR
              T133(198,IY,IS)=Q_DAC_CS(11,IY)
         ENDDO
! ONL end: Direct Air Capture
...
! ONL begin: Direct Air Capture
       ...
       DO IY = 1,LASTYR
         DO IR=1,MAXNFR
             T133(((IR-1)*NUMREP)+199,IY,IS) = DACtoSAL(IR,IY)
           ENDDO
         ENDDO
        ...
        NUMREP=1                       ! equal to number of rows per IR 8
         DO IY = 1,LASTYR
           DO IR=1,8
              T133(((IR-1)*NUMREP)+223,IY,IS) = DACtoEOR(IR,IY)
         ENDDO
       ENDDO      
! ONL end: Direct Air Capture
...
! ONL begin: Ind CCS and DAC **TD check this section
!        T133(178,IY,IS) = INCO2_45QE(11,IY) + INCO2_45QS(11,IY) + & ! Cement CCS
!			(sum(OGCO245Q(MNLNP1,1:13,IY))-OGCO245Q(MNLNP1,7,IY)-OGCO245Q(MNLNP1,5,IY))/18000 !ONL MOD
     !   T133(178,IY,IS) = sum(OGCO245Q(MNLNP1,1:13,IY))/18000
     !   T133(178,IY,IS) = (co2toeor45q(3,IY)+co2toeor45q(6,IY)+co2toeor45q(9,IY)+co2tosaline45q(3,IY)+co2tosaline45q(6,IY)+co2tosaline45q(9,IY))*365/1000 !3 is eth, 6 is h2 and 9 is ngp
      !   T133(178,IY,IS) = sum(co2toeor45q(1:13,IY))*365/1000 + sum(co2tosaline45q(1:13,IY))*365/1000  &
      !                   + sum(OGCO245Q(MNLNP1,1:13,IY))/18000 - OGCO245Q(MNLNP1,7,IY)/18000  ! add ind still in OGSM and subtract power(7)
         T133(178,IY,IS) = sum(co2toeor45q(1:13,IY))*365/1000 + sum(co2tosaline45q(1:13,IY))*365/1000  &   ! LFMM industrial sources
              + INCO2_45QE(11,IY) + INCO2_45QS(11,IY) + I_CCS_Q(iy) + sum(CO2CAPFUEL(1:3,11,iy))  &       ! Cement and steel CCS from IDM, add HMM
              + OGCO245Q(MNLNP1,2,IY)/18000    ! Ammonica still in OGSM
         T133(231,IY,IS) = DAC_CO2_45Q(IY)
         T133(179,IY,IS) = T133(177,IY,IS) +  T133(178,IY,IS)  +  T133(231,IY,IS) 

!by DAC
         T133(232,IY,IS) = DAC_EOR_45Q(IY)
         T133(233,IY,IS) = DAC_SAL_45Q(IY)
         T133(238,IY,IS) = T133(232,IY,IS) +  T133(233,IY,IS)
         T133(239,IY,IS) = CCS_SAL_45Q(IY)
         T133(240,IY,IS) = ATM_SAL_45Q(IY)
         
!by storage type   next time for AEO2022 consider excluding power (7) from OGCO245Q
         T133(180,IY,IS) = (sum(OGCO245Q(MNLNP1,1:13,IY))-OGCO245Q(MNLNP1,7,IY)-OGCO245Q(MNLNP1,5,IY))/18000 & ! ONL MOD power (7) already included in OGCO245Q
							+ sum(CO2_CCS(0,1:7,2,IY))/18000 + INCO2_45QE(11,IY) ! Cement CCS to EOR
     !   T133(180,IY,IS) = sum(CO2_CCS(0,1:7,2,IY))/18000 + sum(OGCO245Q(MNLNP1,1:13,IY))/18000 
     !    T133(180,IY,IS) = sum(CO2_CCS(0,1:7,2,IY))/18000 + sum(OGCO245Q(MNLNP1,1:13,IY))/18000  & 
     !                    - OGCO245Q(MNLNP1,7,IY)/18000        & ! power(7) already included in OGCO245Q 
     !                    + sum(co2toeor45q(1:13,IY))*365/1000   ! industrial CCS in LFMM
     !   T133(181,IY,IS) = CO2_CCS(0,-1,2,IY)/18000
     !    T133(181,IY,IS) = CO2_CCS(0,-1,2,IY)/18000 + sum(co2tosaline45q(1:13,IY))*365/1000 + T133(233,IY,IS)

         T133(181,IY,IS) = CO2_CCS(0,-1,2,IY)/18000 + INCO2_45QS(11,IY) ! Cement CCS to Saline
...
!45Q Credit Payments ($ Billion)
!by plant type
! ONL begin: Ind CCS and DAC
      !   T133(183,IY,IS) = ((sum(CO2_CCS(0,1:7,2,IY))/18000 * CCS_EOR_45Q(IY)) + (CO2_CCS(0,-1,2,IY)/18000 * CCS_SALINE_45Q(IY))) * SCALPR/1000
     !   T133(184,IY,IS) = (sum(OGCO245Q(MNLNP1,1:13,IY))/18000 * CCS_EOR_45Q(IY)) * SCALPR
      !   T133(184,IY,IS) = ((sum(co2toeor45q(1:13,IY))*365/1000  &
       !                  + sum(OGCO245Q(MNLNP1,1:13,IY))/18000 - OGCO245Q(MNLNP1,7,IY)/18000) * CCS_EOR_45Q(IY) &  !subtract power
       !                  + sum(co2tosaline45q(1:13,IY))*365/1000 * CCS_SALINE_45Q(IY)) * SCALPR/1000 
!         T133(234,IY,IS) = (T133(232,IY,IS) * CCS_EOR_45Q(IY) + T133(233,IY,IS) * CCS_SALINE_45Q(IY)) * SCALPR/1000
!         T133(185,IY,IS) = T133(183,IY,IS) +  T133(184,IY,IS) + T133(234,IY,IS)
         T133(183,IY,IS) = ((sum(CO2_CCS(0,1:7,2,IY))/18000 * CCS_EOR_45Q(IY)) + (CO2_CCS(0,-1,2,IY)/18000 * CCS_SALINE_45Q(IY))) * SCALPR/1000 !ONL MOD
!         T133(184,IY,IS) = ((T133(178,IY,IS) - INCO2_45QS(11,IY)) * CCS_EOR_45Q(IY) + &
!							INCO2_45QS(11,IY) * CCS_SALINE_45Q(IY)) * SCALPR/1000 !ONL MOD
      T133(184,IY,IS) = ( sum(co2toeor45q(1:13,IY))*365/1000  + INCO2_45QE(11,IY)  &   ! LFMM and IDM sources to EOR
               + OGCO245Q(MNLNP1,2,IY)/18000 ) * CCS_EOR_45Q(IY) * SCALPR/1000  &  ! Ammonia still in OGSM
              + ( SUM(co2tosaline45q(1:13,IY))*365/1000 + INCO2_45QS(11,IY) + I_CCS_Q(iy) + sum(CO2CAPFUEL(1:3,11,iy))) * CCS_SALINE_45Q(IY) * SCALPR/1000    ! Ind CCS to saline, add HMM

!by storage type
     !   T133(186,IY,IS) = ((sum(CO2_CCS(0,1:7,2,IY))/18000 * CCS_EOR_45Q(IY))  + (sum(OGCO245Q(MNLNP1,1:13,IY))/18000 * CCS_EOR_45Q(IY))) * SCALPR/1000
         T133(186,IY,IS) = T133(180,IY,IS)* CCS_EOR_45Q(IY) * SCALPR/1000
     !   T133(187,IY,IS) = (CO2_CCS(0,-1,2,IY)/18000 * CCS_SALINE_45Q(IY)) * SCALPR
         T133(187,IY,IS) = T133(181,IY,IS)* CCS_SALINE_45Q(IY) * SCALPR/1000   
         T133(188,IY,IS) = T133(186,IY,IS) +  T133(187,IY,IS)
!by dac
         T133(235,IY,IS) = T133(232,IY,IS)* CCS_EOR_45Q(IY) * SCALPR/1000
!         T133(236,IY,IS) = T133(233,IY,IS)* CCS_SALINE_45Q(IY) * SCALPR   
         T133(241,IY,IS) = T133(239,IY,IS)* CCS_SALINE_45Q(IY) * SCALPR/1000   
         T133(242,IY,IS) = T133(240,IY,IS)* CCS_DAC_45Q(IY) * SCALPR/1000   
         T133(236,IY,IS) = T133(241,IY,IS) +  T133(242,IY,IS)   
         T133(237,IY,IS) = T133(235,IY,IS) +  T133(236,IY,IS)    
         T133(234,IY,IS) = T133(237,IY,IS) 
         T133(185,IY,IS) = T133(183,IY,IS) +  T133(184,IY,IS) + T133(234,IY,IS)
! ONL end: DAC
...
!         --- Carbon Emissions by Electric Generators
         ...
         T135( 9,IY,IS) = EMEL(3,1,IY)+EM_ELEC(8,11,IY+1989)		! ONL: BECCS
         T135(10,IY,IS) = EMEL(4,1,IY)
         T135(11,IY,IS) = sum(EMEL(1:4,1,IY))+EM_ELEC(8,11,IY+1989)	! ONL: BECCS
...
! Table 150 - Convergence Summary
      ...
      REAL I120(NTAB120,FTIEAYRS,14)  ! Note:  this table does not represent the regular ..,IY,IS concept
      REAL*8 REALNO
...
!        --- INDIGENOUS PRODUCTION
	  ...
	  I120( 1,I,11) =MTOECONV*WNCMSEL(IY,11)  
	  I120( 6,I,11) = (-1.)*MTOECONV* (WNCMSEL(IY,11))
	  I120( 9,I,11) = I120(10,I,11) + I120(12,I,11) + I120(14,I,11) - FSUM(I120( 1,I,11),8)
	  I120(17,I,11) =((WNGMSEL(IY,mnumnr))*.001) * IEAGROSS
...
!        --- INDIGENOUS PRODUCTION
      RDAYS = 365.
      IF (MOD(IYEAR(I),4).EQ.0) RDAYS = 366.
      I120( 1,I,10) =MTOECONV*(QBMEL(11,IY) +  (QMSEL(11,IY)-WNCMSEL(IY,11)) + &
      ...
      IF (CONEFF(IY) .NE. 0.0) I120( 1,I,10) = I120( 1,I,10) + &
            MTOECONV*(0.9751*CLLETHCD(11,IY) * RDAYS * CFBMQ(IY) * 42. / CONEFF(IY) / 1000000.)
      I120( 2,I,10) =(BIODIMP(11,IY) * CFBIOD(IY) + ETHIMP(11,IY) * CFPET)/1000. *MTOECONV*RDAYS*.001
      I120( 3,I,10) =(BIODEXP(11,IY) * CFBIOD(IY) + ETHEXP(11,IY) * CFPET)/1000. * RDAYS * .001 * MTOECONV * (-1.)
      I120( 6,I,10) = (-1.)*MTOECONV* (QBMEL(11,IY) + (QMSEL(11,IY) - WNCMSEL(IY,11)) + &
      ...
      I120( 8,I,10) = (-1.) * MTOECONV * &
              ...
              QBMRFBTL(11,IY)/1000. - RDAYS / 1000000. * &
                           (sum(BTLFRAC(1:4,MNUMPR,IY)) * CFBTLLIQ(IY) + &
                            sum(CBTLFRAC(2,1:4,MNUMPR,IY)) * CFCBTLLIQ(2,IY) + &
                            UBAVOL(MNUMPR,IY) * 5.763))
      IF (CONEFF(IY) .NE. 0.0) I120( 8,I,10) = I120( 8,I,10) + (-1.) * MTOECONV * &
              ((0.9751*CLLETHCD(11,IY) * RDAYS * (CFBMQ(IY) * 42. / CONEFF(IY) - CFPET)) / 1000000.)
      I120(10,I,10) =(QBMIN(11,IY) + QMSIN(11,IY) - &
      ...
      I120(12,I,10) =(0.9751*(CRNETHCD(11,IY)+CLLETHCD(11,IY)+OTHETHCD(11,IY))+ETHIMP(11,IY)-ETHEXP(11,IY))/1000. * RDAYS * .001 * CFPET * MTOECONV + &
      ...
      I120(13,I,10) = I120(12,I,10)  !  All transportation use is highway
      I120(14,I,10) = (QBMRS(11,IY) + QBMCM(11,IY) -(CGCOMMQ(11,IY,6) + &
                       CGCOMMQ(11,IY,7) + CGCOMMQ(11,IY,10))*.001) * MTOECONV
      I120(15,I,10) =  QBMRS(11,IY) * MTOECONV
!     I120(10,I,10) = I120(10,I,10) + I120(12,I,10) + I120(14,I,10) - FSUM(I120( 1,I,10),8)
      I120(17,I,10) =(UGNMSNR(1,mnumnr,IY) + UGNWDNR(1,mnumnr,IY)+ &
                      UGNMSNR(2,mnumnr,IY) + UGNWDNR(2,mnumnr,IY)+ &
                      (CGINDLGEN(11,IY,6,1) + CGINDLGEN(11,IY,6,2)+ &
                      CGINDLGEN(11,IY,7,1) + CGINDLGEN(11,IY,7,2)+ &
                      CGINDLGEN(11,IY,10,1) + CGINDLGEN(11,IY,10,2)+ &
      ...
      I120( 9,I,10) = I120(10,I,10) + I120(12,I,10) + I120(14,I,10) - FSUM(I120( 1,I,10),8)
! Keeping a constant 36% calculated efficiency for row 18
      I120(18,I,10) = (-.36 * I120(6,I,10) - I120(17,I,10) * .0859845228) / .02388
        I120(18,I,10) = (CGCOMMQ(11,IY,6) + CGINDLQ(11,IY,6) + CGNTQ(mnumnr,IY,6) + &
...
! U.S. Electric
       ...
       IEAGROSS = 1.07
       I120( 2,I,12) = (UTIMPF(mnumnr,IY) +UTIMPE(mnumnr,IY))*.0859845228*.001
       I120( 3,I,12) = (UTEXPF(mnumnr,IY) +UTEXPE(mnumnr,IY))*.0859845228*.001*(-1.)
       I120( 6,I,12) = (I120(17,I,1) + I120(17,I,3) + I120(17,I,4) + &
                        I120(17,I,5) + I120(17,I,6) + I120(17,I,7) + &
                        I120(17,I,8) + I120(17,I,9) + I120(17,I,10)) * .0859845228
       I120( 8,I,12) = (-1.) * (FSUM(I120( 1,I,12),6) - (QELAS(11,IY) * MTOECONV))
       I120(10,I,12) = QELIN(11,IY) * MTOECONV
       I120(12,I,12) = QELTR(11,IY) * MTOECONV
       I120(13,I,12) = TRQLDV(6,11,IY) * .001 * MTOECONV
       I120(14,I,12) = FSUM(I120(1,I,12),8) - I120(10,I,12) - I120(12,I,12)
       IF ((QELCM(11,IY)+QELRS(11,IY)) .NE. (0.0)) &
         I120(15,I,12) = I120(14,I,12) * (QELRS(11,IY) / (QELRS(11,IY) + QELCM(11,IY)))
       I120(15,I,12) = QELRS(11,IY) * MTOECONV
       I120( 9,I,12) = FSUM(I120( 1,I,12),8) - I120(10,I,12) - I120(12,I,12) - I120(14,I,12)

! U.S. Heat

       I120( 1,I,13) = (((RSHTRCON(IY,7) + RSCOOLCN(IY,2) + RSH2OCON(IY,5)) / &
             1000000000) + QSTCM(11,IY) + QPVCM(11,IY)) * MTOECONV
!        --- .02388 OFF IEA WORKSHEET FOR PJ TO MTOE CHECK
       I120( 6,I,13) = (I120(18,I,1) + I120(18,I,10) + &
                        I120(18,I,3) + I120(18,I,4)) * .02388
       IF ((QCLAS(11,IY) + QMCIN(11,IY)+QCIIN(11,IY)) .NE. 0.0) &
       I120( 8,I,13) = (-1.) * MTOECONV * ((((RSHTRCON(IY,7) + RSCOOLCN(IY,2) + &
       ...
       I120(14,I,13) = (((RSHTRCON(IY,7) + RSCOOLCN(IY,2) + RSH2OCON(IY,5)) / &
               1000000000) + QSTCM(11,IY) + QPVCM(11,IY)) * MTOECONV
       I120(15,I,13) = ((RSHTRCON(IY,7) + RSCOOLCN(IY,2) + RSH2OCON(IY,5)) / &
               1000000000) * MTOECONV
       I120(10,I,13) = FSUM(I120(1,I,13),8) - I120(12,I,13) - I120(14,I,13) - I120(16,I,13)

! U.S. Total

         DO III = 1,NTAB120
           I120(III,I,14)= I120(III,I, 1) + I120(III,I, 2) + I120(III,I, 3) + &
                           I120(III,I, 4) + I120(III,I, 5) + I120(III,I, 6) + &
                           I120(III,I, 7) + I120(III,I, 8) + I120(III,I, 9) + &
                           I120(III,I,10) + I120(III,I,11) + I120(III,I,12) + &
						   I120(III,I,13)
         ENDDO
      ENDDO
      ...
      DO IY = 1,FTIEAYRS
        ...
        WRITE(92,'(I4,a,a)') &
              ...
              '"Solar","Renew Combust","Non Rewnew Combust","Electric","Heat","U.S. Total"'
        TOTALROWS=1
        DO WHILE (CONTINUE120)
          ...
          ELSE IF (IROWS(TOTALROWS,120) .GT. 0) THEN
            ...
            WRITE(92,'(a,a,a,14(F14.6,a))') '"',trim(ROWLAB(TOTALROWS,120)),'",', &
                      ((I120(IROWS(TOTALROWS,120),IY,I),','),I=1,14)
          ENDIF
        ENDDO
      ENDDO
      CLOSE(UNIT=92)
...
!  THIS INCLUDE DEFINES THE PRICE AND QUANTITY VARIABLES
      ...
      include 'ab32'
      include 'rggi'
      include 'lfmmout'	! ONL Ind CCS
      include 'uecpout'	! ONL BECCS
      include 'hmmblk'
      REAL*4 FSUM
      EXTERNAL FSUM
      ...
      real indccs(mnumyr)	! CCS from IDM (only cement currently)
      INTEGER I,IR,IY,IS,ighg,ICY,itapstart, ikiln, IYY
...
! Covered CO2 is total covered emissions minus covered other ghg
          T118(65,IY,IS) = EMSOL(1,IY)-emmethane(1,IY) !ONL: for FECM -covered_oghg	! ONL: emmethane to replace covered_oghg
!!!!!!!   if(iy.le.17) T118(65,IY,IS) =T17(29,11,IY,IS)  ! 2006 and before: use co2 history override data from table 17
          T118(66,IY,IS) = emmethane(1,IY)  ! ONL: for FECM using this slot for natural gas methane since there are no covered_oghg
...
! Total ghg emissions: CO2 plus Other GHG
 ...
 !	Cement CCS data summed to national level
	do ikiln=1,MAX_KILNS
	  INDCCS(IY) = INDCCS(IY) + I_CPTY(ikiln)*I_CFR(ikiln,IY)/1e6	!MMT CO2/yr
	enddo
	do IYY=2,IY
            do ICY=1,9
                ...
                write(901,*) 'IY', IY, 'INDCCS', INDCCS(IY)
            enddo
	enddo
          T118(134,iy,is)= sum(EMCARBON(1:5,mnumnr,iy))+sum(ccs_pmm(1:4,mnumpr,iy))+&   ! ONL: BECCS: change in var for Total Carbon Capture and Storage
                   	   indccs(iy)+sum(CO2CAPFUEL(1:3,MNUMCR,iy))+I_CCS_Q(iy) ! ONL MOD Cement + BOF (steel) CCS and Hydrogen
...
! ONL begin: CO2 accounting modification by ONL for Ind CCS and BECCS
          T118(191,iy,is)= EMCARBON(5,mnumnr,iy)             !      Biomass          
          T118(190,iy,is)= sum(EMCARBON(1:5,mnumnr,iy))      !  Total captured from power
          T118(152,IY,IS)= (CO2TOEOR45q(6,iy)+CO2TOSALINE45q(6,iy)+CO2TOEOR245q(6,iy)+CO2TOSALINE245q(6,iy))*365/1000  ! 6=H2 in LFMM
          t118(151,iy,is)= (CO2TOEOR45q(3,iy)+CO2TOSALINE45q(3,iy)+CO2TOEOR245q(3,iy)+CO2TOSALINE245q(3,iy))*365/1000  ! 3=Eth
          T118(150,IY,IS)= (CO2TOEOR45q(9,iy)+CO2TOSALINE45q(9,iy)+CO2TOEOR245q(9,iy)+CO2TOSALINE245q(9,iy))*365/1000  ! 9=NGP
          t118(149,iy,is)= sum(ccs_pmm(1:4,mnumpr,iy))-T118(150,IY,IS)-T118(151,IY,IS)-T118(152,IY,IS)     !Other
          t118(192,iy,is)= sum(ccs_pmm(1:4,mnumpr,iy))       !  Total Refining (with new ind ccs structure) for check
          t118(193,iy,is)= Q_DAC_CS(11,IY) - Q_NG_CCS(11,IY)      !  Direct Air Capture (Net)
          t118(195,iy,is)= Q_DAC_CS(11,IY)       !  Direct Air Capture (Injected)
          ! Carbon Sequestered including Industry (Total CCS + Ammonia + Cement)
          T118(139,IY,IS)= T118(134,iy,is)+T25(16,IY,IS) !+INDCCS(IY) already in t118(134,x,x); !T25(19,IY,IS) ONL Mod to include total cement CCS instead
          T118(194,iy,is)= T118(134,iy,is)+Q_DAC_CS(11,IY)   !    Total CCS with DAC and cement CCS
          T118(196,iy,is)= indccs(iy) !ONL Cement CCS was 190
          !Hydrogen Carbon Sequestered
          T118(197,iy,is)= sum(CO2CAPFUEL(1:3,MNUMCR,iy)) !ONL Hydrogen: sum sequestered coal,gas and biomass
          T118(198,iy,is)= CO2CAPFUEL(1,MNUMCR,iy)
          T118(199,iy,is)= CO2CAPFUEL(2,MNUMCR,iy)
          T118(200,iy,is)= CO2CAPFUEL(3,MNUMCR,iy)
          T118(201,iy,is)= I_CCS_Q(iy) !ONL Iron/Steel CCS (combustion and process)
		  ! process emission captured that are in Covered Emissions (included in totals above)
		  T118(89,IY,IS) = I_CCS_Q(iy)*0.47 ! steel (line numbers repurpsed from below)
		  T118(94,IY,IS) = INCM_PREM(11,iy) * 44.0/12.0  ! cement convert to CO2
! ONL end
...
!          T118(89,IY,IS) = sum(GHG_BL(iy,11:14)) ! sum of F-Gases  ONL repurposing this line no.
          T118(90,IY,IS) = GHG_BL(iy,11)
          T118(91,IY,IS) = GHG_BL(iy,12)
          T118(92,IY,IS) = GHG_BL(iy,13)
          T118(93,IY,IS) = GHG_BL(iy,14)
!          T118(94,IY,IS) = sum(GHG_BL(iy,1:14)) ! sum of other GHG  ONL repurposing this line no.
! Abatement/Offsets
          ighg=95
...
! ***********************************************************
      ...
      EMSOL(1,:) = EMSOL(1,:) * C_CO2_FACTOR
		...
		EMMETHANE(1,:)= EMMETHANE(1,:) * C_CO2_FACTOR    ! ONL: added EMMETHANE
...
! emission factors only defined from 95 on, so use 95 for prior years
      iyy=max(iy,6)
      icy=iy+baseyr-1
      emis(1)=em_elec(9,11,icy)/qelas(11,iy)       ! elec note: qelas in quads here
...
! calculate electricity emissions on a census region basis
      ...
      do ir=1,4
        ...
        emis(1)=sum(em_elec(9,DS:DE,icy))/sum(qelas(DS:DE,iy))       ! elec note: qelas in quads here
