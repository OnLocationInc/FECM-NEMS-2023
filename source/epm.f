...
call blending_efacs !revise the emissions factors for hydrogen/ngas blending &ONL
if (curitr == 1) then
...
if (bank_flag .and. bank_startyr > ijumpcalyr .and. curiyr == firsyr .and. curitr == 1) then
    ...
    do iy = bank_priceyr, lastcalyr
        ...
        emetax(1,iy-1989)=emtax(iy-1989) ! ONL DAC: moved inside the loop
        emetax(2,iy-1989)=emtax(iy-1989) ! ONL DAC: moved inside the loop
    enddo
...
if ((tax_flag .or. permit_flag .or. market_flag) .and. bank_flag &
  .and. curiyr == firsyr .and. curitr == 1 .and. bank_startyr <= ijumpcalyr) then
    ...
    do iy=ip-2,lastyr ! ONL DAC: moved the starting point back 2 years
        asafety = ' '
    ...
    if (maxitr > 0) then  
        ...
        pbefore = emtax(io)
        emtax(1:ip-1) = 0.
        emetax(1,1:ip-1)=0. ! ONL DAC
        emetax(2,1:ip-1)=0. ! ONL DAC
...
!***********************************************************************
! ONL begin: methane
! Add up methane emissions to a national level for inclusion in cap.  ????
! Production by OGSM quantity, transportation by census
    FUNCTION EPM_ADDMETHANE()
    implicit none
	include 'parametr'
    include 'ncntrl'
	include 'ogsmout'
    ...
    COMMON/epm_out/iunit1, bank_onyr
	integer mthreg
	real T_72,epm_addmethane      ! total ghg equivalent of methane production  total
	real T13_3, T2_49,T2_52,T2_97,T2_116,T2_118,T2_120,T2_128 ! sectoral consumption gas variables
	real methprodfactor(mnl48a) ! methane emission per trill factors for each region need units to be consistent
	real ghg_equiv, methtrnsportfactor,alaskamethprodfactor
	integer decline_start_yr
	real decline_rate
	
	COMMON/epm_methfactor/methprodfactor,methtrnsportfactor,alaskamethprodfactor,ghg_equiv, decline_rate
	COMMON/epm_meth_dec/decline_start_yr


!sum production over ogsm region mnl48a is 10 regions  this is in tcf from table 72 T
	T_72 = 0.0
	do mthreg=1, mnl48a
		T_72 = T_72 + ogngprd(mthreg,curiyr) * .001 *methprodfactor(mthreg) 
	enddo
	
!alaska 	
	T_72 = T_72 + ogqngrep(8,curiyr)*.001 * alaskamethprodfactor
	
! Need total consumption. Note this is in quads not tcf and does adds in net exports
! taken from ftab_dac.f T2_49 means line 49 of table 2.

	T2_49 = QGPTR(MNUMCR,curiyr)
	T2_52 = 0.0
	T2_116 = QLPIN(MNUMCR,curiyr)
	T2_118 = QNGAS(MNUMCR,curiyr)  - QGTLRF(MNUMCR,curiyr) + DAC_NGC_CS(MNUMCR,curiyr)  !- QHYTR(MNUMCR,curiyr) / .7 remove 1000 as this  variable is not yet divided by 1000
	T2_120 = QGTLRF(MNUMCR,curiyr)
	T2_128 = QNGLQ(MNUMCR,curiyr)
! we will add exports back in not subtract 
	T13_3 = -1 * (OGQNGIMP(MNUMBX,curiyr)-OGQNGEXP(MNUMBX,curiyr))
!note all qarray is divided by 1000 at top of routine
	T2_97 = (T2_49 + T2_52 + T2_116 + T2_118 + T2_120 + T2_128 +T13_3)/1000.0
!multiply by emissions factor and equivalence	convert to trills here
	epm_addmethane = (T_72 +  T2_97 * methtrnsportfactor /1.037 ) * ghg_equiv * (12.0/44.0)   ! convert only to Carbon, not CO2, because total_emissions in units C
	! adjust for methane decline rate
	 write(666,*) 'decline',decline_rate,decline_start_yr,curiyr+1989,((1.0-decline_rate/100.0)**(curiyr-(decline_start_yr-1989)+1))
	 if (curiyr.ge.(decline_start_yr-1989)) epm_addmethane=epm_addmethane*((1.0-decline_rate/100.0)**(curiyr-(decline_start_yr-1989)+1))
	write(IUNIT1,*)'???? CH4 MT, ghg_equiv, T2_97 T_72, ogngprd, mnl48aT2_49 T13_3 T2_118 QNGAS', epm_addmethane, ghg_equiv, T2_97,T_72, ogngprd(1,curiyr),mnl48a, T2_49,T13_3, T2_118,QNGAS(MNUMCR,curiyr)
    return
    end
! ONL end: methane
...
if (curitr == 1) then
    ...
    IF (ISNAN(BASE_TAX).AND.(DAC_PMIN(J).GT.0.0)) THEN
        HIGH_TAX = DAC_PMIN(J) ! ONL: DAC
        LOW_TAX  = DAC_PMAX(J) ! ONL: DAC
    ELSE 
    HIGH_TAX = BASE_TAX
    LOW_TAX  = BASE_TAX
    ENDIF
...
! ONL begin: DAC
! Change tax based on DAC minimum price and slope                       
    IF ((NEW_TAX.GT.DAC_PMIN(J).AND.NEW_SUM.LT.100.0).AND.(DAC_PMIN(J).GT.0.0)) THEN
    LOW_TAX = DAC_PMAX(J)
            IF (DAC_SLOPE(J).GT.0.0) THEN 
            SLOPE = DAC_SLOPE(J)
            NEW_TAX = DAC_PMIN(J) + NEW_SUM/DAC_SLOPE(J) 
            ENDIF
    HIGH_TAX = DAC_PMIN(J)
    MIN_TAX(J) = DAC_PMIN(J)
    ENDIF
! ONL end: DAC
...
! Change tax based on DAC minimum price and slope                       
    ...
    LOW_TAX = DAC_PMAX(J)
            IF (DAC_SLOPE(J).GT.0.0) THEN 
            SLOPE = DAC_SLOPE(J)
            ! NEW_TAX = DAC_PMIN(J) + HIGH_SUM/DAC_SLOPE(J) 
            ENDIF
    HIGH_TAX = DAC_PMIN(J)
    MIN_TAX(J) = DAC_PMIN(J)
    ENDIF
...
if (curitr > 2) then
    if (low_sum * high_sum > 0) then
        ...
        elseif (low_sum > 0 .and. new_sum < 0) then
            HIGH_SUM = LOW_SUM ! ONL DAC
            HIGH_TAX = LOW_TAX ! ONL DAC
            low_sum = new_sum
            low_tax = new_tax
            bracket = .true.
...
! ONL begin: trap in case carbon content has not been assigned to a coal type
        DO I=1,NDRGG
           IF (CCLCLNR(I,J,K) .LT. 1.0) THEN
              CCLCLNR(I,J,K) = 25.0
           END IF
        ENDDO
! ONL end
...
real ccs(6,mnumnr) ! carbon captured and stored by 1:dist, 2:resid, 3:ng, 4:coal, and 5:total ONL: replace ccs(5,mnumnr) with BECCS
...
! ONL beging: methane
! additions for methane switch
real epm_addmethane ! function
integer add_methane  ! methane switch
parameter(add_methane = 1)  ! set switch to on
! ONL end: methane
...
EMINC(1,2,J) = (((QE2NGIN(11,J,1) - INQNGPF(11,J))* EGFIN(J) +  &
               ...
               QNGHM(11,J) * ENGHM(J)            + &  !NG
!					   (-1)* sum(CO2CAP_QTY(1:9,J))  	 + & capture quantity
!                      HCNSTECH(J,11,1)*(1.0 - PCFCP(J,1))*eNGhm(J) + &  ! Add in hydrogen sources
!                      HCNSTECH(J,11,2)*(1.0 - PCFCP(J,2))*eNGhm(J) + &
!                      HCNSTECH(J,11,8)*(1.0 - PGFCP(J,1))*eNGhm(J) + &
!                      HCNSTECH(J,11,10)*(1.0 - PFFCP(J,2))*eNGhm(J) + &                       
               QLPIN(11,J) * ELPIN(J)        + & ! QLPIN from ngtdm--not bifurcated
               QNGLQ(11,J) * ELPIN(J))/1000.)   & ! treat QNGLQ like QLPIN for starters
             - sum(co2capfuel(2,1:MNUMCR-2,J)) * 12./44. + &                                                                          
              !-CCS_PMM(3,mnumpr,J)) &! refinery carbon captured and storage, natural gas
              -OGCO2PUR(8,2,j)/18000./(44./12.)   & ! ONL ammonia to EOR (doesn't matter since EMINC91,2,J) not used elsewhere
              -(((CO2TOEOR45q(6,curiyr)+CO2TOSALINE45q(6,curiyr)+CO2TOEOR245q(6,curiyr)+CO2TOSALINE245q(6,curiyr))*0.365)/(44./12.)) &  !refinery CCS hydrogen ONL
              -(((CO2TOEOR45q(9,curiyr)+CO2TOSALINE45q(9,curiyr)+CO2TOEOR245q(9,curiyr)+CO2TOSALINE245q(9,curiyr))*0.365)/(44./12.))    !refinery CCS ng processing ONL

! ONL
EMINC(3,2,J) = (QE2CLIN(11,J,1) * ECLIN(J) + &
                  QE2MCIN(11,J,1) * EMCIN(J) + &
                  QCLHM(11,J) * ECLHM(J)   		 + & !CL HYDROGEN
!					   (-1)* sum(CO2CAP_QTY(1:9,J))  	 + & capture quantity
!                      HCNSTECH(J,11,3)*(1.0 - PCFCP(J,3))*eCLhm(J) + &  ! Add in hydrogen sources
!                      HCNSTECH(J,11,4)*(1.0 - PCFCP(J,4))*eCLhm(J) + &                      
                  QE2CIIN(11,J,1) * ECIIN(J))/1000. &
                    - sum(co2capfuel(1,1:MNUMCR-2,J)) * 12./44. - sum(co2capfuel(3,1:MNUMCR-2,J)) * 12./44. 	 + &  ! CL BM CO2 capture quantity
...
eminc(2, 2, j) = (qe2mgin(11, j, 1) * emgin(j) +                                                             &
                  ...
                  -((CO2TOEOR45q(3,curiyr)+CO2TOSALINE45q(3,curiyr)+CO2TOEOR245q(3,curiyr)+CO2TOSALINE245q(3,curiyr))*0.365)/(44./12.)  ! ONL: refinery CCS ethanol
...
! Now total
! ONL
EMINC(1,1,J) = (((QNGIN(11,J) - INQNGPF(11,J)+DAC_NGC_CS(11,j))* EGFIN(J) +  & ! ONL: Add DAC 
               INQNGPF(11,J) * ENQNGPF(J)      +  &
               QNGHM(11,J) * ENGHM(J)            + &  !NG  HYDROGEN 
!					   (-1)* sum(CO2CAP_QTY(1:9,J))  	 + & capture quantity
!                       HCNSTECH(J,11,1)*(1.0 - PCFCP(J,1))*eNGhm(J) + &  ! Add in hydrogen sources
!                       HCNSTECH(J,11,2)*(1.0 - PCFCP(J,2))*eNGhm(J) + &
!                       HCNSTECH(J,11,8)*(1.0 - PGFCP(J,1))*eNGhm(J) + &
!                       HCNSTECH(J,11,10)*(1.0 - PFFCP(J,2))*eNGhm(J) + &                       
               QLPIN(11,J) * ELPIN(J)        + & ! QLPIN from ngtdm--not bifurcated
               QNGLQ(11,J) * ELPIN(J))/1000.)   & ! treat QNGLQ like QLPIN for starters
               - sum(co2capfuel(2,1:MNUMCR -2,J)) * 12./44.  	 + &  ! capture quantity ng
               - Q_NG_CCS(11,j)/(44.0/12.0) & !less NG CCS 
              ! -CCS_PMM(3,mnumpr,J)) &                      ! refinery carbon captured and storage, natural gas (this is zero)
          -(((CO2TOEOR45q(6,curiyr)+CO2TOSALINE45q(6,curiyr)+CO2TOEOR245q(6,curiyr)+CO2TOSALINE245q(6,curiyr))*0.365)/(44./12.)) &  ! refinery CCS hydrogen ONL
          -(((CO2TOEOR45q(9,curiyr)+CO2TOSALINE45q(9,curiyr)+CO2TOEOR245q(9,curiyr)+CO2TOSALINE245q(9,curiyr))*0.365)/(44./12.)) &  ! refinery CCS ng processing ONL
          - INCM_CPTYF(1,j)  &                     ! Cement CCS
               - I_CCS_Q(j)*0.47/(44.0/12.0) &     ! Remove emissions captured by other industrial sources (BOF CCS)
               - OGCO2PUR(8,2,j)/18000./(44./12.)   ! ammonia to EOR converted to MMT C
   !write(iunit1,*) 'OGCO2inc', j,OGCO2PUR(8,2,j),OGCO2PUR(8,2,j)/18000./(44./12.), EMINC(1,1,J) 
...
!                     HCNSTECH(J,11,4)*(1.0 - PCFCP(J,4))*eCLhm(J) + &                      
              ...
              - sum(co2capfuel(1,1:MNUMCR - 2,J)) * 12./44.  - sum(co2capfuel(3,1:MNUMCR - 2,J)) * 12./44.	 + &  ! capture quantity  coal BM + & 
              -CCS_PMM(2,mnumpr,J) & ! refinery carbon captured and storage, coal
              - (INCM_CPTYF(2,j) + INCM_CPTYF(3,j) + INCM_CPTYF(5,j) + INCM_CPTYF(7,j) + &
               INCM_CPTYF(8,j) + INCM_CPTYF(9,j)) ! Cement CCS
...
eminc(2, 1, j) = (qmgin(11, j) * emgin(j) +                           &
                  ...
                  -CCS_PMM(4,mnumpr,J) & ! refinery carbon captured and storage, still gas
                  -((CO2TOEOR45q(3,curiyr)+CO2TOSALINE45q(3,curiyr)+CO2TOEOR245q(3,curiyr)+CO2TOSALINE245q(3,curiyr))*0.365)/(44./12.) & ! ONL: refinery CCS ethanol
                  - (INCM_CPTYF(4,j) + INCM_CPTYF(6,j)) ! Cement CCS
...
em_indy(4, 11, jcalyr) = qdsin(11, j) * edsin(j) - INCM_CPTYF(6,j)*1000.0  ! Distillate ONL MOD: cement CCS
...
em_indy(8, 11, jcalyr) = qpcin(11, j) * epcin(j) - INCM_CPTYF(4,j)*1000.0  ! Petroleum coke ONL MOD: cement CCS
! Still gas
em_indy(9, 11, jcalyr) = qsgin(11, j) * esgin(j) &
                         - ccs_pmm(4, mnumpr, j) * 1000. & ! Refinery carbon captured and storage
                         -((CO2TOEOR45q(3,curiyr)+CO2TOSALINE45q(3,curiyr)+CO2TOEOR245q(3,curiyr)+CO2TOSALINE245q(3,curiyr))*365.0)/(44./12.) ! ONL: refinery CCS ethanol
em_indy(10, 11, jcalyr) = qotin(11, j) * eotin(j)  ! Other petroleum
! Steam coal
em_indy(11,11,jcalyr)=qCLin(11,j)*eCLin(j) - &
        (INCM_CPTYF(2,j) + INCM_CPTYF(3,j) + INCM_CPTYF(7,j) + INCM_CPTYF(8,j) + INCM_CPTYF(9,j))*1000.0 + &   ! ONL: Cement CCS         
         QCLHM(11,J) * ECLHM(J) - (sum(co2capfuel(1,1:MNUMCR-2,J)) * 12./44. + sum(co2capfuel(3,1:MNUMCR-2,J)) * 12./44.)*1000.0 ! ONL: CL BM HYDROGEN less capture
em_indy(12, 11, jcalyr) = qmcin(11, j) * emcin(j) - INCM_CPTYF(5,j)*1000.0  ! Metallurgical coal ONL: cement CCS
! Net coal coke imports
em_indy(13, 11, jcalyr) = qciin(11, j) * eciin(j) &
                          - ccs_pmm(2, mnumpr, j) * 1000.  ! Refinery carbon captured and storage
! Natural gas fuel
! ONL
em_indy(14,11,jcalyr)=(qNGin(11,j)-inqNGpf(11,j)+DAC_NGC_CS(11,j))*eNGin(j) - 1000.0*Q_NG_CCS(11,j)/(44.0/12.0) + & ! ONL: Add DAC less NG CCS
        QNGHM(11,J) * ENGHM(J) -  &               !NG  HYDROGEN 
        sum(co2capfuel(2,1:MNUMCR-2,J))* 12./44.*1000.0 &  ! capture quantity
        !HCNSTECH(J,11,1)*(1.0 - PCFCP(J,1))*eNGhm(J) + &  ! Add in hydrogen sources
        !HCNSTECH(J,11,2)*(1.0 - PCFCP(J,2))*eNGhm(J) + &
        !HCNSTECH(J,11,8)*(1.0 - PGFCP(J,1))*eNGhm(J)  &
        !HCNSTECH(J,11,10)*(1.0 - PFFCP(J,2))*eNGhm(J)) &   ! -CCS_PMM(3,mnumpr,J)*1000.) &                ! refinery carbon captured and storage, natural gas
        -(((CO2TOEOR45q(6,curiyr)+CO2TOSALINE45q(6,curiyr)+CO2TOEOR245q(6,curiyr)+CO2TOSALINE245q(6,curiyr))*365.0)/(44./12.)) &  ! ONL: refinery CCS hydrogen
        -(((CO2TOEOR45q(9,curiyr)+CO2TOSALINE45q(9,curiyr)+CO2TOEOR245q(9,curiyr)+CO2TOSALINE245q(9,curiyr))*365.0)/(44./12.)) &  ! ONL: refinery CCS ng processing
        - OGCO2PUR(8,2,j)/18./(44./12.)  & ! ammonia to EOR converted to ktonnes C
        - INCM_CPTYF(1,j)*1000.0 & ! Cement CCS
        - I_CCS_Q(j)*0.47/(44.0/12.0)*1000.0 ! Remove emissions captured by other industrial sources (BOF CCS)
...
fl_indy(14, 1:11, jcalyr) = (qngin(1:11, j) - inqngpf(1:11, j)+DAC_NGC_CS(1:11,j)) ! ONL: Add DAC less NG CCS  ! Natural gas fuel
...
if ((qNGin(11,j)-inqNGpf(11,j)+DAC_NGC_CS(11,j)) /= 0. .and. curiyr <= msedyr) & ! ONL: Add DAC
...
!emhm(1:3, 1, j) = 0.
EMHM(1,1,J) = QNGHM(11,J) * ENGHM(J) ! ONL           !NG
EMHM(3,1,J) = QCLHM(11,J) * ECLHM(J) ! ONL           !Coal
!      write(iunit1,'(a,1i6,5f20.5)')'EPMDbg1-Y,Coal,SQCoal,eclhm,enghm: ',J,  &
!         HCNSTECH(CURIYR,11,3),HCNSTECH(CURIYR,11,4),ECLHM(J),ENGHM(J)
...
em_tran(8, 11, jcalyr) = qrhtr(11, j) * erhtr(j) + qRLtr(11,j)*eRLtr(j) ! ONL MOD: add qRLtr, which is Residual Fuel - low sulfur  ! High sulfur residual fuel
...
! ONL begin: Deep decarbonization (not part of ind CCUS, DAC, or BECCS)
! move any excess CO2 from biofuels from tran to industry
if(jcalyr.ge.2020 .and. fcrl.eq.1) write(iunit1,'(a,i5,5f12.3)')'B4biotoInd',jcalyr,&
   min(0.0,em_tran( 6,11,jcalyr)+em_tran( 7,11,jcalyr)),em_indy( 4,11,jcalyr),&
   em_tran( 7,11,jcalyr), -AllRenewable_Diesel*eDStr(j)
em_indy( 4,11,jcalyr)=em_indy( 4,11,jcalyr)+min(0.0,em_tran( 6,11,jcalyr)+em_tran( 7,11,jcalyr))
em_tran( 7,11,jcalyr)=em_tran( 7,11,jcalyr)-min(0.0,em_tran( 6,11,jcalyr)+em_tran( 7,11,jcalyr))
if(jcalyr.ge.2020 .and. fcrl.eq.1) write(iunit1,'(a,i5,5f12.3)')'bioCO2toInd',jcalyr,&
   min(0.0,em_tran( 6,11,jcalyr)+em_tran( 7,11,jcalyr)),em_indy( 4,11,jcalyr),&
   em_tran( 7,11,jcalyr), -AllRenewable_Diesel*eDStr(j)
! ONL end: biofuels
...
fl_tran(8, 1:11, jcalyr) = qrhtr(1:11, j) + qRLtr(1:11,j) ! ONL MOD: add qRLtr, which is Residual Fuel - low sulfur  ! High sulfur residual fuel
...
if ((qsRHtr(9,j)+qsRLtr(9,j)).ne. 0.0 .and. curiyr .le. MSEDYR) &
      fl_tran( 8,10,jcalyr)=(qsRHtr(10,j)+qsRLtr(10,j)) * (qRHtr(9,j)+qRLtr(9,j)) / (qsRHtr(9,j)+qsRLtr(9,j))  ! ONL MOD: add qRLtr, which is Residual Fuel - low sulfur
...
ccs(1:6,:)=0.  ! carbon captured and stored ONL: replace ccs(1:5,:) for BECCS
...
! ONL begin: BECCS
em_elec(8,11,jcalyr) = 0.0
em_elec(8,11,jcalyr) = em_elec(8,11,jcalyr) - UFLBMNR(MNUMNR,J) * EBMEL(J) * 0.001

DO I=1,MNUMNR         
 ccs(5,i) = ccs(5,i) + UFLBMNR(I,J) * EBMEL(J) * 0.001

!         Write(6,2901) I, J, curitr, jcalyr, EBMEL(J), em_elec(8,11,jcalyr), UFLBMNR(I,J), UFLBMNR(I,J)*EBMEL(J)*0.001, ccs(5,i), ccs(5,mnumnr)
! 2901    FORMAT(1X,"em_elec(8)",4(",",I4),6(",",F21.6))

enddo

!      Write(6,2902) MNUMNR, jcalyr, ccs(5,mnumnr), em_elec(8,11,jcalyr) 
!2902  FORMAT(1X,"ccs(5)",2(":",I4),2(":",F21.6))          
! ONL end: BECCS

! Eliminate small values due to rounding error
do k = 1, mnumnr
    do i = 1, 5 ! ONL: add one more step in the loop
...
do i = 1, mnumnr
    ccs(6,i)=sum(ccs(1:5,i))  ! total ONL BECCS: replace ccs(5,i)
enddo
ccs = ccs * 0.001

! emcarbon not previously used, but is convenient for holding this new variable
! for ftab.
emcarbon(1:6,:,j)=ccs(1:6,:) ! ONL: BECCS replace emcarbon(1:5,:,j)=ccs(1:5,:)
if (ncrl == 1) then
...
em_elec(9,11,jcalyr)=sum(em_elec(1:8,11,jcalyr)) ! ONL BECCS: replace em_elec(8,11,jcalyr), which is now assigned value above
...
fl_elec(8,1:11,jcalyr)=QBMEL(1:11,j) ! ONL BECCS: new line for the 8th slot
...
if (qsBMel(9,j) .ne. 0.0 .and. curiyr .le. MSEDYR) &
    fl_elec(8,10,jcalyr)=qsBMel(10,j) * qBMel(9,j) / qsBMel(9,j) ! ONL: new line

! Total electric power
do i = 1, 11
    fl_elec(9,i,jcalyr)=sum(fl_elec(1:8,i,jcalyr)) ! ONL BECCS: replace fl_elec(8,i,jcalyr)=sum(fl_elec(1:7,i,jcalyr))
enddo
...
nm_elec(8)='Biomass' ! ONL BECCS: insert biomass into 8th slot
nm_elec(9)='  Total Electric Power'    ! ONL BECCS: push this into 9th slot
...
allrenewablegasoline = (grn2mgqty(7, j) +                           &  ! Non ester renewable gasoline
                       ...
                       * 0.365 * cfnpq &   ! From million barrels per day to trill btu per year. Napthas
                       +RFBIOBUTERR(7,J)*.365 * CFBIOBUTE(J)      &   ! ONL MOD add total biobutanol produced by LFMM region; conv factor = 3.984 (no imports anyway)
                       + ethanol_blended
...
if (curiyr > msedyr) then
    ...
    em_tran(8, 10, curcalyr) = qrhtr(10, j) * erhtr(j) + qRLtr(10,j)*eRLtr(j) ! ONL MOD: add qRLtr, which is Residual Fuel - low sulfur
...
if (j <= base_yr) then
    ...
    em_elec(6, 11, iy) = em_elec(6, 11, iy) + (1000. * emishist(4, 5, j) - sum(em_elec(6:8,11,iy))) ! msw/geo/biomass  ONL BECCS: add biomass, replace sum(em_elec(6:7,11,iy))
...
do i = 1, 11
    em_elec(9,i,jcalyr) =sum(em_elec(1:8,i,jcalyr))        ! total electric sector emissions ONL BECCS: replace em_elec(8,i,jcalyr) =sum(em_elec(1:7,i,jcalyr)) 
    if (qelas(i, j) /= 0.) then
		! ONL begin: for BECCS replace em_elec(8) with em_elec(9)
        em_resd(6,i,jcalyr)=em_elec(9,i,jcalyr)*(qelrs(i,j)/qelas(i,j))
        em_comm(8,i,jcalyr)=em_elec(9,i,jcalyr)*(qelcm(i,j)/qelas(i,j))
        em_indy(17,i,jcalyr)=em_elec(9,i,jcalyr)*(qelin(i,j)/qelas(i,j))
        em_tran(12,i,jcalyr)=em_elec(9,i,jcalyr)*(qeltr(i,j)/qelas(i,j))
		! ONL end: for BECCS replace em_elec(8) with em_elec(9)
    else
        em_resd(6, i, jcalyr) = 0.
        em_comm(8, i, jcalyr) = 0.
        em_indy(17, i, jcalyr) = 0.
...
! ONL begin: DAC                 
       total_emissions(j)=total_emissions(j)-(Q_DAC_CS(11,j) - Q_NG_CCS(11,j))/(44.0/12.0) & ! dac is Co2, total_emissions is carbon
                            - INCM_PREM(11,j) & ! Remove process emissions captured by Cement CCS
                            - I_CCS_Q(j)*0.53/(44.0/12.0) ! Remove process emissions captured by other industrial sources (BOF CCS)

       write(666,*) 'debug DAC epm',total_emissions(j),Q_DAC_CS(11,j),j
       write(666,'(a,2i4,8f12.3)') 'Cbysect',j,curitr,sum(emrs(1:3,1,j)),sum(emcm(1:3,1,j)),sum( emtr(1:2,1,j))+ EMNT(11,1,J)/1000.,&
                     sum(eminc(1:3,1,j)),sum( emel(1:4,1,j)),Q_DAC_CS(11,j)/(44.0/12.0),Q_NG_CCS(11,j)/(44.0/12.0),total_emissions(j)
       write(iunit1,'(a,2i4,8f12.3)') 'Carb_sect',j,curitr,sum(emrs(1:3,1,j)),sum(emcm(1:3,1,j)),sum( emtr(1:2,1,j))+ EMNT(11,1,J)/1000.,&
                     sum(eminc(1:3,1,j)),sum( emel(1:4,1,j)),Q_DAC_CS(11,j)/(44.0/12.0),Q_NG_CCS(11,j)/(44.0/12.0),total_emissions(j)
! ONL end: DAC                 
...
! ONL begin: methane
! 	 add methane emission ???? Note these units are carbon, not CO2 (conversion occurs in ftab)
	  if (add_methane) then
          EMMETHANE(1,j)=epm_addmethane(j)
     	  total_emissions(j) = total_emissions(j) + EMMETHANE(1,j)
!     write(222,'(a,2i5,3f12.3)') 'Methane',curcalyr,curitr,total_emissions(j),epm_addmethane(j),EMMETHANE(1,j)                   

	  endif
      ! add BECCS biomass offset
      total_emissions(j)=total_emissions(j)+em_elec(8,11,j+1989)/1000.0
! ONL end: methane
...
4   format('"',a,'",,',-3p,<lastyr>(',',f10.3))
    ...
    write(iunit,4)'Other      ',((sum(em_elec(  6:8,i,iy))      )*44./12.,iy=2004,min(ijumpcalyr,lastcalyr)) ! ONL BECCS: replace sum(em_elec(  6:7
    write(iunit, 4) '  Total    ', ((sum(em_resd(1:5, i, iy)) + &
                                   sum(em_comm(1:7, i, iy)) +   &
                                   sum(em_indy(1:16, i, iy)) +  &
                                   sum(em_tran(1:11, i, iy)) +  &
                                   sum(em_elec(  1:8,i,iy))      )*44./12.,iy=2004,min(ijumpcalyr,lastcalyr)) ! ONL BECCS: replace sum(em_elec(  1:7
...
EMREV(3,J) =EMREV(3,J) + ((QNGIN(I,J)-INQNGPF(I,J)+DAC_NGC_CS(11,j))* ENGIN(J) - Q_NG_CCS(11,j)/(44.0/12.0)) * ETAXENDU !Add DAC less NG CCS ONL
...
allrenewablegasoline = 365. * grn2mgqty(mnumpr, j) / 1000 * cfnpq +                                    &  ! NER gasoline
                       ...
                       .365 * (RFBIOBUTECD(MNUMCR,J) * CFBIOBUTE(J)) +       &   ! ONL: biobutanol
                       ethanol_blended
...
! ONL begin: BECCS 
!     Biomass in plants with capture
!      WRITE(6,2903) CURIYR, CURITR, MNUMNR, J, EMREV(1,J), EMREV(2,J), EMREV(3,J), EMREV(4,J), EMREV(5,J), UFLBMNR(MNUMNR,J), EBMEL(J), ETAXELEC, EMREV(5,J)-UFLBMNR(MNUMNR,J)*EBMEL(J)*ETAXELEC, !(EMREV(5,J)-UFLBMNR(MNUMNR,J)*EBMEL(J)*ETAXELEC)*0.001
      EMREV(5,J) = EMREV(5,J) - UFLBMNR(MNUMNR,J) * EBMEL(J) * ETAXELEC
          EMREV(5,J) = EMREV(5,J) * 0.001
!2903  FORMAT(1X,"biomass_EMREV_adj",4(",",I4),9(",",F21.6))
! ONL: BECCS 
...
! ONL begin: methane
! 	  METHANE EMISSIONS FACTORS  COMMON
	  real methprodfactor(mnl48a) ! methane emission per trill factors for each region need units to be consistent
	  real methtrnsportfactor,alaskamethprodfactor,ghg_equiv
	  integer decline_start_yr
	  real decline_rate
...
if (iunit_epmdata > 0) then
    ...
    read(iunit_epmdata, *, err=120) ebmel(hist)
...
! ONL begin: methane
! read methane factors 
		call epmnext(iunit_epmdata)
		read(iunit_epmdata,*) (methprodfactor(J),J=1,MNL48a),alaskamethprodfactor, methtrnsportfactor,ghg_equiv,decline_start_yr,decline_rate
		write(iunit1,*) 'output read out methane factor ', methprodfactor,alaskamethprodfactor, methtrnsportfactor,ghg_equiv !,decline_start_yr,decline_rate
! ONL end: methane        
    ...
    do j = 1, lastyr
        ...
        epcel(j) = epcel(hist)
        emsel(j) = emsel(hist)
        EBMEL(J) = EBMEL(hist) ! ONL BECCS: biomass
...
120 write(6, *) 'ERROR : FAILED READ IN EPMDATA RESULTS INDETERMINATE'
...
! ONL begin: DAC
! Change tax based on DAC minimum price and slope                       
                        ! IF (NEW_TAX.GT.DAC_PMIN(IY)) THEN
                        ! LOW_TAX = DAC_PMAX(IY)
                                ! IF (DAC_SLOPE(IY).GT.0.0) THEN 
                                ! NEW_TAX = DAC_PMIN(IY) + HIGH_SUM/DAC_SLOPE(IY) 
                                ! ENDIF
                        ! HIGH_TAX = DAC_PMIN(IY)
                        ! MIN_TAX(IY) = DAC_PMIN(IY)
                        ! ENDIF
! ONL end: DAC
...
! ONL
subroutine blending_efacs
! revise the emissions factors for hydrogen and natural gas blending by the proportion of hydrogen in total natural gas
implicit none
include 'parametr'
include 'ncntrl'
include 'emeblk'
...
INTEGER BASE_YR   ! last year of history-overwrites (also, see HIST in EPM_READ)
real*4 NG_Frac   ! Fraction of natural gas in blended output, based on QHMM quantity and total gas
parameter(base_yr=31) ! 2020 Is last data year when reading epmdata history    

NG_Frac = 1.0 - QHMM(PHNGMM,MNUMCR,CURIYR)/max(QNGAS(MNUMCR,CURIYR), .001)
if (CURIYR .eq. 46) then 
  write(*,*) 'Blending adjustment CO2 parameters) curiyr,curitr,QHMM, QNGAS,  NG_Frac',Curiyr,curitr,QHMM(PHNGMM,MNUMCR,CURIYR),QNGAS(MNUMCR,CURIYR),NG_Frac
endif ! Curiyr
EGFRS(curiyr)= EGFRS(base_yr) * NG_Frac   !(Natural gas, firm, Residential)  
EGFCM(curiyr)= EGFCM(base_yr) * NG_Frac   !(Natural gas, firm, Commercial) 
EGFTR(curiyr)= EGFTR(base_yr) * NG_Frac   !(Natural gas, firm, Transportation) 
EGFIN(curiyr)= EGFIN(base_yr) * NG_Frac   !(Natural gas, firm, Industrial) 
EGFEL(curiyr)= EGFEL(base_yr) * NG_Frac   !(Natural gas, firm, Electric power)
EGIRS(curiyr)= EGIRS(base_yr) * NG_Frac   !(Natural gas, interruptible, Residential) 
EGICM(curiyr)= EGICM(base_yr) * NG_Frac   !(Natural gas, interruptible, Commercial) 
EGITR(curiyr)= EGITR(base_yr) * NG_Frac   !(Natural gas, interruptible, Transportation) 
EGIIN(curiyr)= EGIIN(base_yr) * NG_Frac   !(Natural gas, interruptible, Industrial) 
EGIEL(curiyr)= EGIEL(base_yr) * NG_Frac   !(Natural gas, interruptible, Electric power)
ENGRS(curiyr)= ENGRS(base_yr) * NG_Frac   !(Natural gas, Residential)  
ENGCM(curiyr)= ENGCM(base_yr) * NG_Frac   !(Natural gas, Commercial)  
ENGTR(curiyr)= ENGTR(base_yr) * NG_Frac   !(Natural gas, Transportation)  
ENGIN(curiyr)= ENGIN(base_yr) * NG_Frac   !(Natural gas, Industrial)  
ENGEL(curiyr)= ENGEL(base_yr) * NG_Frac   !(Natural gas, Electric power) 
EGPTR(curiyr)= EGPTR(base_yr) * NG_Frac   !(Natural gas, Pipeline)  
