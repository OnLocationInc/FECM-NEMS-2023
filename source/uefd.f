...
!
      SUBROUTINE ELCOST(IRG,IYR,JTYPE )
!
      IMPLICIT NONE
!
      include 'parametr'
      ...
      REAL*8    FACTE,VAL,BIO_CONS,RPSCST
      ...
      INTEGER*4 F,K,IRG,IYR,ITYPE,IBTP,INCT,IPHASE,NOXYR,FULLYR, FL_RG_25,JTYPE
      INTEGER SLICE
...
!     FACTE = (2000.0 / 2204.0) / 1000000.0
      ...
      UFLBMNR(IRG,CURIYR) = 0.0    !ONL BECCS
      IF (IRG .EQ. 1) THEN
         UFLBMNR(MNUMNR,CURIYR) = 0.0
      END IF
...
!
			   ...
			   STOCST_RPSADJ(IGRP,ISP) = 0.0    !ONL BECCS
			   RPSCST = 0.0                     !ONL BECCS
               DO 20 IFP = 1 , EIFPLT
...
!                 ENDIF
                  ...
                  IF (ECFSHR(N,IFP) .GT. 0.0000000001) THEN
                     ...
                     UFRCO2(UIWD,IFLRG) = EBMEL(CURIYR) * 2 * 44.0/12.0    !ONL BECCS
...
!                    ACCOUNT FOR SUBSIDY, IF ANY, FOR BIOMASS COFIRING (CONVERT FROM MILLS)
                     ...
                     ELSE
                        CFSUB = 0.0
                     END IF
					 RPSCST = EPRPSPR(CURIYR) * (UPRNWBND(CURIYR) - UPRNWSHR(IECPT))  ! ONL BECCS
					 IF (UPRNWBAS(IECPT) .GT. 0.0) RPSCST = 0.0
					 STOCST_RPSADJ(IGRP,ISP) = RPSCST
                     STORAGE_CST(IGRP,ISP) = STORAGE_CST(IGRP,ISP) + ECFSHR(N,IFP) * (0.001 * ULHTRT_EFD(IGRP,ISP) * &
                        (UPFUEL(IFLTP,IFLRG) - EOR_REV + ACIFUEL - CFSUB - ECSEQS(N) * CRBPRC(IFLTP)) + ECOMR(N,IFP) + ACIOM) + &
						STOCST_RPSADJ(IGRP,ISP)

                        BM_CAR_REV = 0.0                !ONL BECCS
                        ...
                        IF (ECSEQS(N) .GT. 0.0 .AND. ECFSHR(N,IFP) .GT. 0.0) THEN
                           IF (IFLTP .EQ. UIWD) THEN 
                              BM_CAR_REV = ECFSHR(N,IFP) * ECSEQS(N)  * (0.001 * ULHTRT_EFD(IGRP,ISP)) * (EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR))
                              STORAGE_CST(IGRP,ISP) = STORAGE_CST(IGRP,ISP) - BM_CAR_REV
                           END IF
                        END  IF
                      IF (FCRL .EQ. 1) THEN
 ...
 1068                format(1x,'ELCOST ECFSHR ',3I6,F10.5,I4)
                     ...
                     IF (ECSEQS(N) .GT. 0.0 .AND. ECFSHR(N,IFP) .GT. 0.0) THEN
                        ...
                        IF (IFLTP .EQ. UIWD) THEN       !ONL BECCS
                            BIO_CONS = (FUEL - FUEL_ALT) * ECSEQS(N) + FUEL_ALT * UPPCEF_MIN(IECPT)
                            BM_CAR_REV = BIO_CONS * 0.001 * EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR)
                            UFLBMNR(IRG,CURIYR) = UFLBMNR(IRG,CURIYR) + BIO_CONS
                            UFLBMNR(MNUMNR,CURIYR) = UFLBMNR(MNUMNR,CURIYR) + BIO_CONS
                        END IF
                        IF (FCRL .EQ. 1 .OR. CURITR .GT. MAXITR) WRITE(18,2317) CURIRUN, CURCALYR, CURITR, N, ISP, IRG, IECPT, IFP, IFLTP, IFLRG, FL_RG_25, I_45Q, ITYPE, &
                           (ECFSHR(N,IFP) * UFRCO2(IFLTP,IFLRG) * ((FUEL - FUEL_ALT) * ECSEQS(N) + FUEL_ALT * UPPCEF_MIN(IECPT))), &
                           ECFSHR(n,IFP), UFRCO2(IFLTP,IFLRG), FUEL, (FUEL - FUEL_ALT), FUEL_ALT, ECSEQS(N), UPPCEF_MIN(IECPT), BIO_CONS, UFLBMNR(IRG,CURIYR), UPPCEF(IECPT), &
                           BIO_CONS, BM_CAR_REV, JCLEL(CURIYR), EBMEL(CURIYR), ECLEL(CURIYR)
 2317                   FORMAT(1X,"BTU_CCS_INFO",13(",",I5),16(",",F21.6))

                     END IF
					 UFRCO2(UIWD,IFLRG) = 0.0
 ...
 !                       IF (CURIYR.EQ.27)WRITE(6,4924)'uqfgenc1 ', CURIYR,IFLTP,ICR,N,IFP,UQFGENC(IFLTP,ICR),  GEN, ECFSHR(N,IFP)
   4924   Format(A25,1x,5(I4,1x),3(F12.3,1x))
                     ...
                     ERTFL(IFOWN) = ERTFL(IFOWN) + &
                        (FUEL -FUEL_ALT) * (UPFUEL(IFLTP,IFLRG) - EOR_REV + ACIFUEL - CRBCST(IFLTP) - CFSUB - ECSEQS(N) * CRBPRC(IFLTP)) * 0.001 - BM_CAR_REV
                     ERTFL(IFOWN) = ERTFL(IFOWN) + &
...
!                    Add EOR Revenues to ULREVS()  - ONL BECCS
...
!
      ...
      INTEGER IECP
      REAL*8  PSFAC,GEN,NOXFAC,FUEL,PRPS,BRPS,SRPS,CO2LB,CAPFAC,SPRPS,SBRPS,SSRPS,CST_RPS,CST_RPS_ST,RPSCST
...
!              STORAGE_CST(IGRP,ISP) = STORAGE_CST(IGRP,ISP) + EHMFSH(IRNW,IFP) * 0.001 * ((ULHTRT_EFD(IGRP,ISP) * UPFUEL(IFL,IFLRG)) + EHVOMR(IRNW))
			   ...
			   RPSCST = EPRPSPR(CURIYR) * (UPRNWBND(CURIYR) - UPRNWSHR(IECP))  ! ONL BECCS
...
!1234 format(1h ,'!egenwn',i4,i3,2f10.5)
      ...
      RETURN
      END
! Hydrogen retrieve from consumption rows
      SUBROUTINE EDOH2(IRG,IS)
      ...
      REAL*8 SOLVAL(5)
      CHARACTER*16  H2COL
      CHARACTER*8 COLSOL /'ASLUP   '/
      CHARACTER*2 STAT
	  CHARACTER*2 BR,CL_CD,GS_CD,OL_CD,CA_CD
      INTEGER*4 IREG, IS, IRG, IRET, HRG
	  REAL*8 QVAL
!	  Integer*4 H2OFFSET
!	  Parameter (H2OFFSET = 3) ! electricity quantity (seasonal) for QHMM start at index 4	
! find census region from fuel region
! Col mask and retrieve retrieve transport vector
      efdsub='EDOH2'
! &&&& ONL need QELHMM(HRG,CURIYR,IS) for varlistrec.txt
! set up row and column 
! H2OFFSET is sector,which for electricity is by season. So we collect it by season	  
	  HRG = EPCSMP(IRG)  !  census regions	 
	  WRITE(BR,'(I2.2)') HRG  !  converted to census string 
	  IF (HRG .GT. 0) THEN
		H2COL = 'HTUS' // BR // CHCOD(IS); call makmsk(col_mask,':H2US:' , BR, CHCOD(IS))	
		CALL DWFSCOL(H2COL,COLSOL,STAT,SOLVAL,COLUMN_mask,IRET)
		QHMM(H2OFFSET + IS,HRG,CURIYR) =  SOLVAL(1)
        QELHMM(HRG,CURIYR,IS) = SOLVAL(1)
		if ((curiyr .eq. 41).or. (CURIYR .eq. 61)) then
			write(6,*)'H2 consumption Fuel region, Census region, season, colname, QHMM, solval ',  IRG,' ', BR, ' ',IS ,' ',  H2COL, QHMM(H2OFFSET + IS,HRG,CURIYR),  SOLVAL(1)
		endif ! curiyr
	  ELSE
		if (curiyr .eq. 41) then
			write(6,*) ' EFD Fuel region Census Region ', IRG, HRG 
		endif ! curiyr
	  ENDIF
      RETURN
      END
...
!
      ...
      CHARACTER*2 BR,CL_CD,GS_CD,OL_CD,CA_CD
!  hydrogen variables  
	  INTEGER I_ICE, IFLTP, NUMSTEP, ISTEP
      CHARACTER*2 H2_CD
      CHARACTER*16 H2ROW, H2COL, H2ROWP
      
      REAL*8 NEG1/-1.0D0/
      COMMON/TEST_HYDROGEN/H2_TEST   ! test hydrogen switch
      INTEGER H2_TEST
      efdsub='ED$BTU'
...
!   coal
       ...
       DO IECP = 1, UIIS
            ...
            IF (CPFLECP(IECP,ORG,CRG,GRG) .GT. 0.0) THEN
               ...
               IF (EFDMAT .EQ. 0) THEN
                 CALL DROWTYPE(BTUROW,'E       ',BTUROW_mask)
...
!       FLCST = MAX(FLCST,0.001)
        ...
        FLCST = 0.001
		  FLCST = CLSH * JCLCLNR(CURIYR,IECP) * (1.0 - UPPCEF(IECP))   !ONL BECCS
		  IF (UPPCEF(IECP) .GT. 0.0) THEN     !ONL BECCS
			 FLCST = FLCST - (1.0 * CFSH * JCLCLNR(CURIYR,IECP) * EBMEL(CURIYR) / ECLEL(CURIYR) * UPPCEF(IECP))
		  END IF
...
!   account for sequestration
        IF (SEQRT .GT. 0.0) THEN
          COEFF = SEQRT * UFRCAR(IECP,CRG) * (1.0 / 2204.0) * CLSH
			 COEFF = COEFF + EGFEL(CURIYR) * 0.001 * GASSH * SEQRT   !ONL BECCS
			 COEFF = COEFF + EDSEL(CURIYR) * 0.001 * OLSH * SEQRT
			 COEFF = COEFF + SEQRT * UFRCAR(IECP,CRG) * (1.0 / 2204.0) * (CFSH * EBMEL(CURIYR) / ECLEL(CURIYR))
			 WRITE(6,3912) CURIRUN, CURCALYR, CURITR, IECP, CRG, UPSTYR, USW_ECPCF, COL, ROWCAR, ROWSEQ, COEFF, SEQRT, CLSH, GASSH, OLSH, CFSH, FLCST, &
				JCLCLNR(CURIYR,IECP), UFRCAR(IECP,CRG), EGFEL(CURIYR), EDSEL(CURIYR), EBMEL(CURIYR), ECLEL(CURIYR), UPPCEF(IECP)
	3912                FORMAT(1X,"CL_TRANS_BTU_UEFD",7(",",I5),3(",",A16),14(",",F21.6))
...
!           FLCST = MAX(FLCST,0.001)
            FLCST = 0.001
		  FLCST = CLSH * JCLCLNR(CURIYR,IECP) * (1.0 - UPPCEF(IECP))
		  IF (UPPCEF(IECP) .GT. 0.0) THEN     !ONL BECCS
			 FLCST = FLCST - (1.0 * CFSH * JCLCLNR(CURIYR,IECP) * EBMEL(CURIYR) / ECLEL(CURIYR) * UPPCEF(IECP))
		  END IF
...
!   account for sequestration
            IF (SEQRT .GT. 0.0) THEN    !ONL BECCS
              COEFF = SEQRT * UFRCAR(IECP,CRG) * (1.0 / 2204.0) * CLSH
              ! IF (COEFF .GT. EFD_MIN) then
...
!  fuel cost = extra transportation cost for cofiring step
            ! FLCST = CFSH * UPCFSTEP(ICOF,CFLV)
            ! FLCST = MAX(FLCST,0.001)
		  ! Return carbon cost due to biomass use in units with capture (Biomass production removes carbon then returns it when burned except if it is captured)
			FLCST = CLSH * JCLCLNR(CURIYR,IECP) * (1.0 - UPPCEF(IECP))
			IF (UPPCEF(IECP) .GT. 0.0) THEN            !ONL BECCS
			   FLCST = FLCST - (1.0 * CFSH * JCLCLNR(CURIYR,IECP) * EBMEL(CURIYR) / ECLEL(CURIYR) * UPPCEF(IECP))
			END IF
			FLCST = FLCST + CFSH * UPCFSTEP(ICOF,CFLV)
...
! Separate loop for hydrogen H2 turbine (ICE) because we are special  
! Use census regions and three seasons 
! 	  Set plant code to EFD number, this should be fixed
	  I_ICE= 14
	  IFLTP = 3  ! fuel three 
	  DO FRG = 1, UNFRGN
		Do IS = 1, EENSP  
!
			HRG = EPCSMP(FRG)  !  census regions
!   SET h2 row  	 
			WRITE(BR,'(I2.2)') HRG
			H2_CD = BR
			H2ROW = 'H2' // BR // CHCOD(IS); call makmsk(H2ROW_mask,':H2:' , BR, CHCOD(IS))	
!   Set up HTUS column which which is the transport
!   
			H2COL = 'HTUS' // BR // CHCOD(IS); call makmsk(COLUMN_mask,':H2US:' , BR, CHCOD(IS))
!   SET UP b COLUMN  for ICE only call it fuel 3!
			COL = 'B'//EPFLCD(FRG)//EPPLCD(I_ICE)//CHCOD(IS)//CHCOD(IFLTP)//'X'; COL_mask='B(*)(***)(*)(**)'
!   set up  BTU balance  needed for intersection
			BTUROW = 'BTU'//EPFLCD(FRG)//EPPLCD(I_ICE)//CHCOD(IS); call makmsk(BTUROW_mask,':BTU:',EPFLCD(FRG),EPPLCD(I_ICE),CHCOD(IS))
!   intersection with BTU
			CALL DVAL(COL,BTUROW,NEG1,COL_mask,BTUROW_mask,'ED$BTU,84')
!   intersection  H2 supply balance row, 
			CALL DVAL(COL,H2ROW,DBLE(1.0),COL_mask,H2ROW_mask,'ED$BTU,85')
!   conversion factor in OBJ
			FLCST = 0.001  ! like all others just a conversion factor
			CALL DVAL(COL,EFDOBJ,DBLE(FLCST),COL_mask,efdobj,'ED$BTU,86')
! 	reset cost of fueltypes 1 and  2 to 100 so they will never be used
			IFLTP = 1
			FLCST = 100.0  ! like all others just a conversion factor
			COL = 'B'//EPFLCD(FRG)//EPPLCD(I_ICE)//CHCOD(IS)//CHCOD(IFLTP)//'X'; COL_mask='B(*)(***)(*)(**)'
			CALL DVAL(COL,EFDOBJ,DBLE(FLCST),COL_mask,efdobj,'ED$BTU,87')
			IFLTP = 1			
			FLCST = 100.0  ! like all others just a conversion factor
			COL = 'B'//EPFLCD(FRG)//EPPLCD(I_ICE)//CHCOD(IS)//CHCOD(IFLTP)//'X'; COL_mask='B(*)(***)(*)(**)'
			CALL DVAL(COL,EFDOBJ,DBLE(FLCST),COL_mask,efdobj,'ED$BTU,88')
!   set up intersection HTUS column with 	H2ROW, transferring fuel price
			CALL DVAL(H2COL,H2ROW,NEG1,COLUMN_mask,efdobj,'ED$BTU,89')
!   set up intersection HTUS column with 	OBJ		
		    FLCST = 0.001 ! like everybody else
			CALL DVAL(H2COL,EFDOBJ,DBLE(FLCST),COL_mask,efdobj,'ED$BTU,90')	
! 	set up production row 
			H2ROWP = 'HPUS' // BR // CHCOD(IS); call makmsk(H2ROW_mask,':HPUS:' , BR, CHCOD(IS))	
! 	set up intersection from transport to HP row
			CALL DVAL(H2COL,H2ROWP, NEG1,COLUMN_mask,H2ROW_mask,'ED$BTU,91')
!   set up production columns first 1 step, no limit at this time
			NUMSTEP = 1
			DO ISTEP =1, NUMSTEP
				H2COL = 'HP' // BR // CHCOD(IS) // CHCOD(ISTEP); call makmsk(COLUMN_mask,':H2US:' , BR, CHCOD(IS), CHCOD(ISTEP) )				
!   set up intersection with HPUS row
				CALL DVAL(H2COL,H2ROWP,DBLE(1.0),COLUMN_mask,H2ROW_mask,'ED$BTU,92')	
!  add tested h2 price   $.25 /kg * 134510 BTU/kg HHV	= $1.85/MMBTU  = $.77/MMBTU  $87 	
				IF (HRG .GT. 0) THEN
!					FLCST = DBLE(0.01)    !take this out it is a dummy number!!!!
                    IF (H2_TEST .GT. 0) FLCST = 0.5   ! test of  H2 model
					FLCST = PHMM(H2OFFSET + IS,HRG,CURIYR)
					if ((CURIYR .eq. 36) .and. (CURITR .eq. 1)) then
						write(*,*) 'HMM IS HRG CURIYR  FLCST', IS,HRG,CURIYR,FLCST
					ENDIF
				ELSE
					FLCST = 0.001
				ENDIF
! 	set up intersection with OBJ
				CALL DVAL(H2COL,EFDOBJ,DBLE(FLCST),COLUMN_mask,efdobj,'ED$BTU,93')	
			ENDDO ! ISTEP
		ENDDO   ! EENSP
	  ENDDO   ! FRG
...
!     lf Pump Storage Units are in this region set up rows to consume electricity in off-peak slices
      ...
      INTEGER*4  N,IRG,ITYP,IS,IVSL,IP,IECP,IGRP,TFR,IFR,IST,STA,CRG,IGS,JVLS,CAR,IGR,I_NOCCS,I_NOCCS_EFD,IEFD, N_last
      ...
      REAL*8 DRAT,VAL,VALCF,GEN,OBJVAL,BTUREQ,NOXEM,GENBMS,VAL_SR,DRAT_MIN,DRAT_MAX,MAX_SP_LOAD,MIN_SP_LOAD,GEN_MIN,GEN_MAX,BTUREQ_MIN,BTUREQ_MAX,DRAT_ORG,SP_HOURS, COFCON
      REAL*8 COFCON_ALT, OBJVAL_ALT       !ONL BECCS
...
!3232 format(1h ,'!efdbsl',4I6,a10,a3,8f10.3)
               COFCON = 0.0        !ONL BECCS
               COFCON_ALT = 0.0
               IF (UPPCEF(IECP) .GT. 0.0) THEN          
                  BTUREQ = GEN * HTRT * HTRT_ADJ * 0.000001  !  trill BTU per GW
                  COFCON = UPPCEF(IECP) * CFSH * BTUREQ * EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR)
                  COFCON_ALT = UPPCEF_MIN(IECP) * CFSH * BTUREQ * EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR)
               END IF
              OBJVAL = (VOM + RPSCST + GPSCST - PTC - COFCON) * GEN * 0.001   ! units million $ / GW BECCS obj
...
3231      FORMAT(1x,A10,5I6,F15.2,4F15.5,F15.2)               
               ...
               OBJVAL_ALT = (VOM + RPSCST + ALT_GPSCST - PTC - COFCON_ALT) * GEN * 0.001   ! units million $ / GW
               IF (ADJ_FAC .NE. 1.0) CALL DVAL(COL_ALT,EFDOBJ,OBJVAL_ALT,COL_ALT_mask,EFDOBJ,'OPBSLD,24')

               IF (UPPCEF(IECP) .GT. 0.0 .AND. N .NE. N_last .AND. CFSH .GT. 0.0) THEN
                  WRITE(6,137)  CURIRUN, CURIYR, CURIYR+1989, IRG, IECP, WIIS, N, COL, EBMEL(CURIYR), ECLEL(CURIYR), JCLEL(CURIYR), UPPCEF(IECP), &
                      PTC, BTUREQ, GEN, UPWDCFR(IECP,EPCLMP(TFR)), CFSH, HTRT, HTRT_ADJ, COFCON, VOM, RPSCST, GPSCST, OBJVAL, COFCON_ALT, OBJVAL_ALT, ALT_GPSCST, UPPCEF_MIN(IECP)
  137             FORMAT(1X,"BECCS_obj_2",7(",",I4),1(",",A8),20(",",F12.4))
               END IF
               IF (SR_CREDIT(IECP) .GT. 0.0) THEN
                  BTUREQ_MIN = GEN_MIN * HTRT * HTRT_ADJ_MIN * 0.000001  !  trill BTU per GW
                  COFCON = UPPCEF(IECP) * CFSH * BTUREQ_MIN * EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR)
                  VAL_MIN_SR = (VOM + RPSCST + GPSCST_MIN - PTC - COFCON) * GEN_MIN * 0.001   ! units million $ / GW
                  CALL DVAL(COL_MIN_SR,EFDOBJ,VAL_MIN_SR,COL_MIN_SR_mask,EFDOBJ,'OPBSLD,25')
                  COFCON_ALT = UPPCEF_MIN(IECP) * CFSH * BTUREQ_MIN * EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR)
                  VAL_MIN_SR = (VOM + RPSCST + ALT_GPSCST_MIN - PTC - COFCON_ALT) * GEN_MIN * 0.001   ! units million $ / GW
                  ...
                  COFCON = UPPCEF(IECP) * CFSH * BTUREQ_MAX * EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR)
                  VAL_MAX_SR = (VOM + RPSCST + GPSCST_MAX - PTC - COFCON) * GEN_MAX * 0.001   ! units million $ / GW
                  CALL DVAL(COL_MAX_SR,EFDOBJ,VAL_MAX_SR,COL_MAX_SR_mask,EFDOBJ,'OPBSLD,27')
                  COFCON_ALT = UPPCEF_MIN(IECP) * CFSH * BTUREQ_MAX * EBMEL(CURIYR) * JCLEL(CURIYR) / ECLEL(CURIYR)
                  VAL_MAX_SR = (VOM + RPSCST + ALT_GPSCST_MAX - PTC - COFCON_ALT) * GEN_MAX * 0.001   ! units million $ / GW
...
!                    MAKE CAPTURED CO2 FROM COAL UNITS AVAILABLE TO EOR PROJECTS
                     ...
                     IF (UPPCEF(IECP) .GT. 0.0) THEN
                        ...
                        ENDIF
                        VAL_CAP_CO2 = UPPCEF(IECP) * BTUREQ * (CFSH*EBMEL(CURIYR) + (1-CFSH)*ECLEL(CURIYR)) * 0.001 * (44.0 / 12.0) !!! ONL BECCS
                        CALL DVAL(COL,ROW_EOR_CO2,VAL_CAP_CO2,COL_mask,ROW_EOR_CO2_mask,'OPBSLD,77')

                        ALT_VAL = UPPCEF_MIN(IECP) * BTUREQ * (CFSH*EBMEL(CURIYR) + (1-CFSH)*ECLEL(CURIYR)) * 0.001 * (44.0 / 12.0)                        
                        IF (ADJ_FAC .NE. 1.0) CALL DVAL(COL_ALT,ROW_EOR_CO2,ALT_VAL,COL_ALT_mask,ROW_EOR_CO2_mask,'OPBSLD,78')

                        IF (SR_CREDIT(IECP) .GT. 0.0) THEN
                           VAL_MIN_SR = UPPCEF(IECP) * BTUREQ_MIN * (CFSH*EBMEL(CURIYR) + (1-CFSH)*ECLEL(CURIYR)) * 0.001 * (44.0 / 12.0)
                           CALL DVAL(COL_MIN_SR,ROW_EOR_CO2,VAL_MIN_SR,COL_MIN_SR_mask,ROW_EOR_CO2_mask,'OPBSLD,79')
                           ALT_VAL = UPPCEF_MIN(IECP) * BTUREQ_MIN * (CFSH*EBMEL(CURIYR) + (1-CFSH)*ECLEL(CURIYR)) * 0.001 * (44.0 / 12.0)
                           IF (ADJ_FAC .NE. 1.0) CALL DVAL(COL_MIN_SR_ALT,ROW_EOR_CO2,ALT_VAL,COL_MIN_SR_ALT_mask,ROW_EOR_CO2_mask,'OPBSLD,80')

                           VAL_MAX_SR = UPPCEF(IECP) * BTUREQ_MAX * (CFSH*EBMEL(CURIYR) + (1-CFSH)*ECLEL(CURIYR)) * 0.001 * (44.0 / 12.0)
                           CALL DVAL(COL_MAX_SR,ROW_EOR_CO2,VAL_MAX_SR,COL_MAX_SR_mask,ROW_EOR_CO2_mask,'OPBSLD,81')

                           ALT_VAL = UPPCEF_MIN(IECP) * BTUREQ_MAX * (CFSH*EBMEL(CURIYR) + (1-CFSH)*ECLEL(CURIYR)) * 0.001 * (44.0 / 12.0)
                           IF (ADJ_FAC .NE. 1.0) CALL DVAL(COL_MAX_SR_ALT,ROW_EOR_CO2,ALT_VAL,COL_MAX_SR_ALT_mask,ROW_EOR_CO2_mask,'OPBSLD,82')
                        END IF

!                       WRITE(6,9317) CURIRUN, CURIYR+1989, CURITR, COL, ROW_EOR_CO2, IRG, TFR, IECP, BTUREQ, CFSH, &
!                          VAL_CAP_CO2, UPPCEF(IECP), ECLEL(CURIYR), EBMEL(CURIYR)
!9317                   FORMAT(1X,"BECCS_test_2",3(":",I4),2(":",A16),3(":",I4),6(":",F15.3))
