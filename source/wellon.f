...
!  ONL MOD - Read dynamic CO2 input data
          CALL DATE_AND_TIME(VALUES=tmp_time)
		  CALL READ_CO2
          CALL SETUP
          write(OGBUG1,*) 'end reading dynamic CO2 input data'
          CALL DATE_AND_TIME(TIME=curtime, VALUES=cur_time)
          CALL ELAPSED_TIME(tmp_time,cur_time)
...
! Map Price Bin to Industrial Source Indexes, this excludes Natural, Power and XTLs
       ...
       ! THESE THREE TYPES OF CONSTRAINTS ARE ONLY CALLED ONCE
       ! 1) DRILLING
       ! 2) CAPITAL
       ! 4) GAS DEMAND
       IF (.NOT. ONCE) THEN
...
!        call set_base_co2_volumes_available    ! ONL MOD - moved to outside ONCE loop which covers the constraints 1, 2, 4
...
!  STORE THE TOTAL REGIONAL GAS DEMAND - FOR REPORTING PURPOSES
 ...
 15    format (a30,4(2x,f10.3))
       ...
       ONCE = .TRUE.
       ENDIF   ! END OF THE BLOCK THAT IS ONLY CALLED ONCE AT THE BEGINNING OF THE RUN
               ! 1) DRILLING
               ! 2) CAPITAL
               ! 4) GAS DEMAND

       ! ONL MOD
       ! Contraint #3 CO2 is now moved to CTUS.F as a subroutine
...
!  ASSIGN WELLS to NEMS OUTPUT VARIABLES
      ...
      r=1
      do isrc=1, max_src
          write(6780,*)'OGTO curiyr=', curiyr+1989,' ireg=',r,' isrc=',isrc,' cregpryrS=',cregpryrS(r,ISRC,:)
      enddo
      write(6780,*)'==============================='
      isrc=5
      do r=1, max_reg-1
