...
'copy %xlsx% %cname% > nul                                                                          ', &
'set xlsx=%cname%                                                                                   ', &
'echo Unzipping %xlsx% into folder %folder%                                                         ', &
'mkdir %folder%                                                                                     ', &
'n:/default/aeo20/scripts/winrar.exe X -o+ %xlsx% .\%folder%  > nul                                 ', &
...
! reads non-record-formatted XML file and writes a new one with tags on new lines
	...
	character(len=:),allocatable :: xmlfile
	...
	iret=0
    open(funit,file=original,status='old',form='unformatted',access='STREAM',iostat=istat)
	inquire(file=original, size=filesize)
	call unitunopened(100,999,w_unit)
    ...
    lline=0
    oldchar=' '
	if (istat==0) then !file open
	  allocate( character(len=filesize) :: xmlfile)
      read(funit,pos=1,iostat=istat) xmlfile
	  close(funit)
	  if (istat==0) then !file read	  
...
900       ByteLoc=ByteLoc+1
          ...
          if(bchar.eq.lf .or.  (bchar.eq.'<'.and.oldchar.eq.'>')) then
             ...
             if (byteloc.lt.filesize) goto 900  ! go read the next character
          endif
 ...
 ! add the character to the current line, increment character count, and save to check for cr/lf combo
          ...
          if (byteloc.lt.filesize) goto 900
