...
!...Local variable dictionary
    ...
    REAL :: annual_gwh_ev(mnumyr)
    REAL :: annual_gwh_phev(mnumyr)    
				...
				if((ILDV.ge.4.and.ILDV.le.7).or.ILDV.eq.15)then
					do IVTYP=1,maxvtyp
						if(COUNT(avg_kwh(IVTYP,ILDV,:,ic+1989).gt.0.0).eq.0.0)then
							annual_gwh(ic) = 0.0
                            annual_gwh_ev(ic)=0.0
                            annual_gwh_phev(ic) = 0.0
						elseif(IVTYP.eq.1)then		! Uses average kWh per vehicle, averaged over all size classes
                            ...
                            if ((ILDV.eq.5).or. (ILDV.eq.6) ) then
                               annual_gwh_phev(ic) = annual_gwh_phev(ic) + trldsalc(ILDV,11,ic)*SUM(avg_kwh(IVTYP,ILDV,:,ic+1989))/ &
											 COUNT(avg_kwh(IVTYP,ILDV,:,ic+1989).gt.0.0)
					          
                            else
                               annual_gwh_ev(ic) = annual_gwh_ev(ic) + trldsalc(ILDV,11,ic)*SUM(avg_kwh(IVTYP,ILDV,:,ic+1989))/ &
											 COUNT(avg_kwh(IVTYP,ILDV,:,ic+1989).gt.0.0)
                            endif
						else
							annual_gwh(ic) = annual_gwh(ic) + trldsalt(ILDV,11,ic)*SUM(avg_kwh(IVTYP,ILDV,:,ic+1989))/ &
											 COUNT(avg_kwh(IVTYP,ILDV,:,ic+1989).gt.0.0)
                               if ((ILDV.eq.5).or. (ILDV.eq.6) ) then
                               annual_gwh_phev(ic) = annual_gwh_phev(ic) + trldsalt(ILDV,11,ic)*SUM(avg_kwh(IVTYP,ILDV,:,ic+1989))/ &
											 COUNT(avg_kwh(IVTYP,ILDV,:,ic+1989).gt.0.0)
					          
                            else
                               annual_gwh_ev(ic) = annual_gwh_ev(ic) + trldsalt(ILDV,11,ic)*SUM(avg_kwh(IVTYP,ILDV,:,ic+1989))/ &
											 COUNT(avg_kwh(IVTYP,ILDV,:,ic+1989).gt.0.0)
                            endif
						endif
					enddo ! maxvtyp
		...
		do ILDV=1,maxldv
			if((ILDV.ge.4.and.ILDV.le.7).or.ILDV.eq.15)then
				do ICL=1,MAXCLASS
                    ...
                    if ((ILDV.eq.5).or. (ILDV.eq.6) ) then
                      annual_gwh_phev(n) = annual_gwh_phev(n) + avg_kwh(1,ILDV,ICL,yrs)*NCSTECH(11,ICL,ILDV,n)
					  annual_gwh_phev(n) = annual_gwh_phev(n) + avg_kwh(2,ILDV,ICL,yrs)*NLTECH(11,ICL,ILDV,n)
                    else
                      annual_gwh_ev(n) = annual_gwh_ev(n) + avg_kwh(1,ILDV,ICL,yrs)*NCSTECH(11,ICL,ILDV,n)
					  annual_gwh_ev(n) = annual_gwh_ev(n) + avg_kwh(2,ILDV,ICL,yrs)*NLTECH(11,ICL,ILDV,n)
                    endif
				enddo ! MAXCLASS
...
!		Align curve with historical data
	...
	if(n.eq.MNUMYR.and.FCRL.eq.1)then
		...
		write(21,*) "ic annual_gwh_phev annual_gwh_ev annual_gwh cumulative_gwh li_ion_cost (2020USD ILDV=4:7 and 15)"
		do ic=6,n
			write(21, '(I5,", ", 7(F12.3,", "))') ic+1989, annual_gwh_phev(ic),annual_gwh_ev(ic),annual_gwh(ic), cumulative_gwh(ic), li_ion_cost(4:7,ic+1989)/ MC_JPGDP(1) * MC_JPGDP(31), &
