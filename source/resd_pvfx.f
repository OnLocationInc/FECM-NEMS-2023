...
!NEW PV MODEL VARIABLES (use dynamic allocations due to ZIP code dimension for arrays, ~30,000 ZIPs)    !PVPen
    ...
    Real*4, allocatable:: CumUnits_L(:)             ! ONL MOD For constraining penetration; Initial Value for Iteration Control (OnLoc; note that AEO22 may solve this a diff way)
...
!  NO CALCULATIONS PRIOR TO RECSYear+1
      ...
      !Beginning of Dynamic Array Allocations for new Econometric Model
        ...
        if(allocated(CumUnits_L)) deallocate(CumUnits_L); allocate(CumUnits_L(numzips)) !ONL MOD
...
!             CumUnits(i) = 0.0 ! ONL MOD: remove since this keeps it from accumulating within each zip code
             ...
             ! Set Inputs to Equations:
                IF(curitr.EQ.1 .AND. CurCalYr.GT.EstYear) THEN  !Subsequent to the estimation year update lag variables on first iteration
                  ...
                  CumUnits_L(i)=CumUnits(i)      ! ONL MOD
             ...
             !Compute hurdle model additions to PV households with a test for overflows
               ...
               ELSE
                 ...
                 CumUnits(i)=CumUnits_L(i)+xtemp !ONL MOD also accumulate units within a ZIP code for testing penetration level
              !If penetration exceeds a particular value back out additional units
                IF(CumUnits(i)/Households(i) .GT. 0.80) THEN !80% is limit on share of households that can have PV in each ZIP code
                  xUnits=xUnits-xTemp
                  CumUnits(i)=CumUnits_L(i)  ! ONL MOD don't back out xtemp from cumulative
                  xTemp=0.
                  ProjectedInstalls(i)=0     ! ONL MODzero out lag variable also so doesn't keep growing
!                    if(fcrl .eq.1) write(9,'(a,5i6,5f15.4)') 'HH>80%aft',curcalyr,curitr,i,zipcode(i),CenDiv(i),xtemp,xunits,CumUnits(i),CumUnits_L(i),Households(i)
                ENDIF
                   if(idiv .eq.9.and. curcalyr .ge.2025) write(9,'(a,5i6,5f15.4)') 'HHall9',curcalyr,curitr,i,zipcode(i),CenDiv(i),xtemp,xunits,CumUnits(i),CumUnits_L(i),Households(i)
