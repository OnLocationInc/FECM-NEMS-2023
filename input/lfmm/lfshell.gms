...
if(errorlevel=0,
  ...
  execute_load PriorBuilds,ETHCO2_PriorBuilds,H2CO2_PriorBuilds,NGPCO2_PriorBuilds ;
...
if(errorlevel=0,
  put_utility 'gdxin' / '%LFMM_BUILDS%' ;
  execute_load CO2_PriorBuilds,H2CO2_PriorBuilds(H2ID,t) ;
*  display '%LFMM_BUILDS% found!' ;
else
  CO2_PriorBuilds(ETHID,t) = 0 ;
  display '%LFMM_BUILDS% NOT found!' ;
);
...
if(errorlevel=0,
  put_utility 'gdxin' / '%LFMM_BUILDS%' ;
*  execute_load H2CO2_PriorBuilds ;
  H2CO2_PriorBuilds(H2ID,t) = 0 ;
*  display '%LFMM_BUILDS% found!' ;
else
  H2CO2_PriorBuilds(H2ID,t) = 0 ;
  display '%LFMM_BUILDS% NOT found!' ;
...
*Set for credit year
  Credityear(t) = no;
  if( RunYears.val <= TCS45Q_I_45Q_LYR_NEW + TCS45Q_I_45Q_DURATION - 1,
*    Credityear(t)$(t.val <= min(RunYears.val, TCS45Q_I_45Q_LYR_NEW) - 1 and t.val >= max(RunYears.val, TCS45Q_I_45Q_LYR_NEW) - (TCS45Q_I_45Q_DURATION)) = yes ;
    Credityear(t)$(t.val <= min(RunYears.val, TCS45Q_I_45Q_LYR_NEW) - 1 and t.val >= max(TCS45Q_I_45Q_SYR, min(RunYears.val, TCS45Q_I_45Q_LYR_NEW) - 12)) = yes ;
      else
      Credityear(t) = no;
    );
...
*Set for ETHID
  ETHIDR(ETHID) = no ;
  ETHIDR(ETHID)$(sum(t$(Ord(t)< CurYrIdx),ETHCO2_PriorBuilds(ETHID,t))>0) = yes ;
  ETHIDN(ETHID) = not ETHIDR(ETHID);

*Set for H2ID
  H2IDR(H2ID) = no ;
  H2IDR(H2ID)$(sum(t$(Ord(t)< CurYrIdx),H2CO2_PriorBuilds(H2ID,t))>0) = yes ;
  H2IDN(H2ID) = not H2IDR(H2ID);

*Set for NGPID
  NGPIDR(NGPID) = no;
  NGPIDR(NGPID)$(sum(t$(Ord(t)<CurYrIdx),NGPCO2_PriorBuilds(NGPID,t))>0)=yes;
  NGPIDN(NGPID) = not NGPIDR(NGPID);

* ETH 45q Retrofit ID
    ETH45Q_StartYr(ETHID,t)$(ETHCO2_PriorBuilds(ETHID,t)$(t.val<=TCS45Q_I_45Q_LYR_NEW)) = t.val + 1 ;
    ETH45Q_EndYr(ETHID,t)$(ETHCO2_PriorBuilds(ETHID,t)$(t.val<=TCS45Q_I_45Q_LYR_NEW)) = t.val + TCS45Q_I_45Q_DURATION ;
    ETH45QID(ETHID,t) = no ;
    ETH45QID(ETHID,t)$(t.val >= min(NCNTRL_CURCALYR, smax(tt,ETH45Q_StartYr(ETHID,tt))) and t.val <= smax(tt,ETH45Q_EndYr(ETHID,tt))) = yes ;
    ETH45QCap(ETHID,Runyears)$ETH45QID(ETHID,Runyears) = smax(t,ETHCO2_PriorBuilds(ETHID,t));
    ETH45QID_p(ETHID,Period) = no ;
    loop((t,Period,ETHID)$(t.val >= 2020),
*        put loopie /;
*        put loopie 'ETHID: ', ETHID.tl, ' t ', t.val, ' Period ', Period.tl, ' map ',tPer(t,Period), ' value ', ETH45QID_p(ETHID,Period)/;
        if((tPer(t,Period) and ETH45QID(ETHID,t)),
            ETH45QID_p(ETHID,Period) = yes;
        );
    );

* H2 45q Retrofit ID
    H245Q_StartYr(H2ID,t)$(H2CO2_PriorBuilds(H2ID,t)$(t.val<=TCS45Q_I_45Q_LYR_NEW)) = t.val + 1;
    H245Q_EndYr(H2ID,t)$(H2CO2_PriorBuilds(H2ID,t)$(t.val<=TCS45Q_I_45Q_LYR_NEW)) = t.val + TCS45Q_I_45Q_DURATION ;
    H245QID(H2ID,t) = no ;
    H245QID(H2ID,t)$(t.val >= min(NCNTRL_CURCALYR, smax(tt,H245Q_StartYr(H2ID,tt))) and t.val <= smax(tt,H245Q_EndYr(H2ID,tt))) = yes ;
    H245QCap(H2ID,Runyears)$H245QID(H2ID,Runyears) = smax(t,H2CO2_PriorBuilds(H2ID,t));
    H245QID_p(H2ID,Period) = no ;
    loop((t,Period,H2ID)$(t.val >= 2020),
       if((tPer(t,Period) and H245QID(H2ID,t)),
            H245QID_p(H2ID,Period) = yes;
        );
    );

* NGP 45q Retrofit ID
    NGP45Q_StartYr(NGPID,t)$(NGPCO2_PriorBuilds(NGPID,t)$(t.val<=TCS45Q_I_45Q_LYR_NEW)) = t.val + 1 ;
    NGP45Q_EndYr(NGPID,t)$(NGPCO2_PriorBuilds(NGPID,t)$(t.val<=TCS45Q_I_45Q_LYR_NEW)) = t.val + TCS45Q_I_45Q_DURATION ;
    NGP45QID(NGPID,t) = no ;
    NGP45QID(NGPID,t)$(t.val >= min(NCNTRL_CURCALYR, smax(tt,NGP45Q_StartYr(NGPID,tt))) and t.val <= smax(tt,NGP45Q_EndYr(NGPID,tt))) = yes ;
    NGP45QCap(NGPID,Runyears)$NGP45QID(NGPID,Runyears) = smax(t,NGPCO2_PriorBuilds(NGPID,t));
    NGP45QID_p(NGPID,Period) = no ;
    loop((t,Period,NGPID)$(t.val >= 2020),
       if((tPer(t,Period) and NGP45QID(NGPID,t)),
            NGP45QID_p(NGPID,Period) = yes;
        );
    );
...
* NPV of cost parameters inflated to NOMINAL dollars
  ...
  npv_ETHCO2RETopCost(ETHID,RefReg,ActivePeriod)                         = npv1(ETHCO2RETopCost(ETHID,RefReg),Rate(t));
  npv_H2CO2RETopCost(H2ID,RefReg,ActivePeriod)                           = npv1(H2CO2RETopCost(H2ID,RefReg),Rate(t));
  npv_NGPCO2RETopCost(NGPID,RefReg,ActivePeriod)                         = npv1(NGPCO2RETopCost(NGPID,RefReg),Rate(t));
...
* revenue adjustment of credit choosing region 2 because it's midwest. *
  npv_CO2creditEOR(ActivePeriod)                                         = npv1(CO2creditEOR(t)/(1 - EffectiveTax('2_RefReg')),Rate(t));
  npv_CO2creditSal(ActivePeriod)                                         = npv1(CO2creditSal(t)/(1 - EffectiveTax('2_RefReg')),Rate(t));

  npv_ETHnocreditEOR(ETHID,ActivePeriod)                                 = npv1(ETHnocreditEOR(ETHID,t),Rate(t));
  npv_ETHnocreditSal(ETHID,ActivePeriod)                                 = npv1(ETHnocreditSal(ETHID,t),Rate(t));

  npv_H2nocreditEOR(H2ID,ActivePeriod)                                   = npv1(H2nocreditEOR(H2ID,t),Rate(t));
  npv_H2nocreditSal(H2ID,ActivePeriod)                                   = npv1(H2nocreditSal(H2ID,t),Rate(t));

  npv_NGPnocreditEOR(NGPID,ActivePeriod)                                 = npv1(NGPnocreditEOR(NGPID,t),Rate(t));
  npv_NGPnocreditSal(NGPID,ActivePeriod)                                 = npv1(NGPnocreditSal(NGPID,t),Rate(t));
...
* npv_EthBrazilSup(EthStream,Step,ActivePeriod)                   = npv2(EthBrazilSup(EthStream,Step,t),Rate(t));
  ...
  npv_STEO_Penalty(Step,ActivePeriod)                             = npv1(STEO_Penalty(Step,t),Rate(t));
...
*---------------------------------npv of CO2RET capital cost and fixed OM cost--------------------------------------

*Convert over night cost to annuity
  ETHID_CAPEX(ETHID,RefReg) = ETHID_CAPEX(ETHID,RefReg)*CRF('EDMCCS',RefReg);
  npv_ETHCO2RETBuildCost(ETHID,RefReg,Period)$ActivePeriod(Period)= npv3(ETHID_FOMCOST(ETHID,RefReg),ETHID_CAPEX(ETHID,RefReg),Rate(t));

  H2CAPEX(H2ID,RefReg) =  H2CAPEX(H2ID,RefReg)*CRF('H2PCCS',RefReg);
  npv_H2CO2RETBldCost(H2ID,RefReg,Period)$ActivePeriod(Period) = npv3(H2FOMCOST(H2ID,RefReg),H2CAPEX(H2ID,RefReg),Rate(t));
*need to change crf for ngp
  NGPCAPEX(NGPID,RefReg) =  NGPCAPEX(NGPID,RefReg)*CRF('H2PCCS',RefReg);
  npv_NGPRETBldCost(NGPID,RefReg,Period)$ActivePeriod(Period) = npv3(NGPFOMCOST(NGPID,RefReg),NGPCAPEX(NGPID,RefReg) ,Rate(t));
*Debug2
*npv_NGPCO2RETopCost(NGPID,RefReg,Period) = 0.0000001;
*npv_NGPRETBldCost(NGPID,RefReg,Period) = 0.0000001;
...
* In last year of 45q credit and after sending CO2 to credit variables in future periods is not allowed
    if (RunYears.val >= TCS45Q_I_45Q_LYR_NEW,
        CO2toEOR.fx(FuelRegion,OGCO2Reg,CO2_SourceCCS,FuturePeriods(Period)) = 0 ;
        CO2toSALINE.fx(FuelRegion,CO2_SourceCCS,FuturePeriods(Period)) = 0 ;
    );
...
* Bound the operations of existing capacity to the amount previously built
  OPERATECAP.up(RefReg,RefType,Process,ActivePeriod)$(not SameAs(Process,'CO2ETHRET2') and not SameAs(Process,'CO2H2RET2') and not SameAs(Process,'CO2NGPRET2') ) =
    AvailCap(RefReg,RefType,Process)$(AvailCap(RefReg,RefType,Process)>0) ;

****Ethanol carbon capture capacity Bound******************************************************
  ETHCO2RET_OPERATE.up(ETHID,Period)= sum(t$(ord(t)<CurYrIdx),ETHCO2_PriorBuilds(ETHID,t));
  H2CO2RET_OPERATE.up(H2ID,Period) =sum(t$(ord(t)<CurYrIdx),H2CO2_PriorBuilds(H2ID,t));
  NGPCO2_OPERATE.up(NGPID,Period) =sum(t$(ord(t)<CurYrIdx),NGPCO2_PriorBuilds(NGPID,t));

*Must store CO2 PriorBuilds process

  ETHCO2RET_Builds.up(ETHIDN,ActivePeriod) = sum(RefReg$ETHID_RefReg(ETHIDN,RefReg), ETHID_TotalCO2(ETHIDN,RefReg));
  ETHCO2RET_Builds.fx(ETHIDR,ActivePeriod) = 0 ;

  H2CO2RET_Builds.up(H2IDN,ActivePeriod) = sum(RefReg$H2RefReg(H2IDN,RefReg), H2TotCO2Control(H2IDN,RefReg));
  H2CO2RET_Builds.fx(H2IDR,ActivePeriod) = 0 ;

  NGPCO2_BUILDS.up(NGPIDN,ActivePeriod) = sum(RefReg$NGPRefReg(NGPIDN,RefReg), NGPTotCO2Control(NGPIDN,RefReg));
  NGPCO2_BUILDS.fx(NGPIDR,ActivePeriod) = 0 ;

*  NGP_BuildsCap(RefReg,Period) =
*         max(0,(sum(OGCO2Reg$RefReg_2_OGCO2Reg(OGCO2Reg,RefReg),
*         npv_CO2_Avail(OGCO2Reg,'NG_PROCESSING',Period))-AvailCap(RefReg,'COKING','CO2NGPRET2')));
$ontext
...
* Restrict corn ethanol builds before 2015
  if(NCNTRL_CURCALYR<=2013,
    BUILDS.fx(RefReg,RefType,'IBA',BldStep,ActivePeriod)    = 0 ;
    BUILDS.fx(RefReg,RefType,'EDM',BldStep,ActivePeriod)    = 0 ;
    BUILDS.fx(RefReg,RefType,'EDH',BldStep,ActivePeriod)    = 0 ;
    BUILDS.fx(RefReg,RefType,'EDMNEW',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'EDHNEW',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'AET',BldStep,ActivePeriod)    = 0 ;
    BUILDS.fx(RefReg,RefType,'NCE',BldStep,ActivePeriod)    = 0 ;
    BUILDS.fx(RefReg,RefType,'H2PNEW',BldStep,ActivePeriod) = 0 ;
  ) ;

* Restrict CCS corn ethanol builds before 2025
  if(NCNTRL_CURCALYR<=2023,
    BUILDS.fx(RefReg,RefType,'EDMCCS',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'EDHCCS',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'CO2ETHRET',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'H2PCCS',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'CO2H2RET',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'CO2NGPRET',BldStep,ActivePeriod) = 0 ;
  ) ;

* Do not allow CBTL to build
    BUILDS.fx(RefReg,RefType,'CBL',BldStep,ActivePeriod)    = 0 ;
    BUILDS.fx(RefReg,RefType,'CBLCCS',BldStep,ActivePeriod) = 0 ;

* Do not allow SEW to build
    BUILDS.fx(RefReg,RefType,'SEW',BldStep,ActivePeriod)    = 0 ;

* Don't let these build, should build EDHNEW and EDMNEW instead
    BUILDS.fx(RefReg,RefType,'EDH',BldStep,ActivePeriod)     = 0 ;
    BUILDS.fx(RefReg,RefType,'EDM',BldStep,ActivePeriod)     = 0 ;
    BUILDS.fx(RefReg,RefType,'CO2ETHRET2',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'H2P',BldStep,ActivePeriod)     = 0 ;
    BUILDS.fx(RefReg,RefType,'CO2H2RET2',BldStep,ActivePeriod) = 0 ;
    BUILDS.fx(RefReg,RefType,'CO2NGPRET2',BldStep,ActivePeriod) = 0

* Restrict GTL builds before 2020
  if(NCNTRL_CURCALYR<=2018,
    BUILDS.fx(RefReg,RefType,'GTL',BldStep,ActivePeriod)    = 0 ;
  ) ;
...
$ontext
CO2Cap_Credit(FuelRegion,'EDMCCS',RefReg) =
  sum(Credityear, ProcessTable('CO2CCS','EDMCCS','EDMCCS_CD2') * PriorBuilds(RefReg,'ETH_REF','EDMCCS',Credityear)
CO2Cap_Credit('EDHCCS',RefReg) =
  sum(Credityear, ProcessTable('CO2CCS','EDHCCS','EDHCCS_CD2') * PriorBuilds(RefReg,'ETH_REF','EDHCCS',Credityear)
CO2Cap_Credit('CO2ETHRET2',RefReg) =
  sum(Credityear, ProcessTable('CO2CCS','CO2ETHRET2','CO2ETHRET2_CD3') * PriorBuilds(RefReg,'ETH_REF','CO2ETHRET2',Credityear)+
CO2Cap_Credit('H2PCCS',RefReg) =
  sum(Credityear, ProcessTable('CO2CCS','H2PCCS','H2PCCS_NGS2')* PriorBuilds(RefReg,'COKING','H2PCCS',Credityear)+
CO2Cap_Credit('CO2H2RET2',RefReg) =
  sum(Credityear, ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3') *PriorBuilds(RefReg,'COKING','CO2H2RET2',Credityear)
CO2Cap_Credit('CO2NGPRET2',RefReg) =
  sum(Credityear, ProcessTable('CO2CCS','CO2NGPRET2','CO2NGPRET2_CD3') *PriorBuilds(RefReg,'COKING','CO2NGPRET2',Credityear));

CO2Cap_Credit('ETHANOL',FuelRegion) =
    ProcessTable('CO2CCS','CO2ETHRET2','CO2ETHRET2_CD3')
    * sum((Credityear, RefReg, ETHIDR)$(ETHID_FuelReg(ETHIDR,FuelRegion) and RefReg_2_FuelRegion(RefReg,FuelRegion) and ETHID_TotalCO2(ETHIDR,RefReg) ge 0.25), CO2_PriorBuilds(ETHIDR, Credityear)) +
    ProcessTable('CO2CCS','EDHCCS','EDHCCS_CD2')
    * sum((Credityear, RefReg, ETHIDR)$(RefReg_2_FuelRegion(RefReg,FuelRegion) and ETHID_RefReg(ETHIDR,RefReg) and ETHID_TotalCO2(ETHIDR,RefReg) ge 0.25), PriorBuilds(RefReg,'ETH_REF','EDHCCS',Credityear)) +
    ProcessTable('CO2CCS','EDMCCS','EDMCCS_CD2')
    * sum((Credityear, RefReg, ETHIDR)$(RefReg_2_FuelRegion(RefReg,FuelRegion) and ETHID_RefReg(ETHIDR,RefReg) and ETHID_TotalCO2(ETHIDR,RefReg) ge 0.25),  PriorBuilds(RefReg,'ETH_REF','EDMCCS',Credityear)) ;

CO2Cap_Credit('HYDROGEN_REF',FuelRegion) =
    ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3')
    * sum((Credityear, RefReg, H2IDR)$(H2FuelReg(H2IDR,FuelRegion) and RefReg_2_FuelRegion(RefReg,FuelRegion) and H2TotCO2Control(H2IDR,RefReg) ge 0.25), H2CO2_PriorBuilds(H2IDR, Credityear)) +
    ProcessTable('CO2CCS','H2PCCS','H2PCCS_NGS2')
    * sum((Credityear, RefReg, H2IDR)$(RefReg_2_FuelRegion(RefReg,FuelRegion) and H2RefReg(H2IDR,RefReg) and H2TotCO2Control(H2IDR,RefReg) ge 0.25), PriorBuilds(RefReg,'COKING','H2PCCS',Credityear)) ;

CO2Cap_Credit('NG_PROCESSING',FuelRegion) =
    ProcessTable('CO2CCS','CO2NGPRET2','CO2NGPRET2_CD3')
    * sum((Credityear, NGPIDR)$NGPFuelReg(NGPIDR,FuelRegion), sum(RefReg$(NGPRefReg(NGPIDR,RefReg) and NGPTotCO2Control(NGPIDR,RefReg) ge 0.25), NGPCO2_PriorBuilds(NGPIDR, Credityear))) ;
$offtext

CO2Cap_Credit('ETHANOL',FuelRegion) =
    ProcessTable('CO2CCS','CO2ETHRET2','CO2ETHRET2_CD3') * sum(ETHIDR$ETHID_FuelReg(ETHIDR,FuelRegion), ETH45QCap(ETHIDR,Runyears)) +
    ProcessTable('CO2CCS','EDHCCS','EDHCCS_CD2') * sum((Credityear, RefReg)$RefReg_2_FuelRegion(RefReg,FuelRegion), PriorBuilds(RefReg,'ETH_REF','EDHCCS',Credityear)) +
    ProcessTable('CO2CCS','EDMCCS','EDMCCS_CD2') * sum((Credityear, RefReg)$RefReg_2_FuelRegion(RefReg,FuelRegion),  PriorBuilds(RefReg,'ETH_REF','EDMCCS',Credityear)) ;

CO2Cap_Credit('HYDROGEN_REF',FuelRegion) =
    ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3') * sum(H2IDR$H2FuelReg(H2IDR,FuelRegion), H245QCap(H2IDR,Runyears)) +
    ProcessTable('CO2CCS','H2PCCS','H2PCCS_NGS2') * sum((Credityear, RefReg)$RefReg_2_FuelRegion(RefReg,FuelRegion), PriorBuilds(RefReg,'COKING','H2PCCS',Credityear)) ;

CO2Cap_Credit('NG_PROCESSING',FuelRegion) =
    ProcessTable('CO2CCS','CO2NGPRET2','CO2NGPRET2_CD3') * sum(NGPIDR$NGPFuelReg(NGPIDR,FuelRegion), NGP45QCap(NGPIDR,Runyears)) ;

*$sum(ActivePeriod,PROCMODE.l(RefReg,"COKING","H2PCCS",ProcessMode,ActivePeriod)*tPer(Credityear,ActivePeriod)>0 ),
...
*   Post-process ethanol capture retrofits to make them 100% built if model
*    chooses to build above some fraction of capacity.
    ETHCO2RET_BUILDS.l(ETHID,BldPeriod)$(ETHCO2RET_BUILDS.l(ETHID,BldPeriod) >= PercentageBuilds*ETHCO2RET_BUILDS.up(ETHID,BldPeriod)) =
      ETHCO2RET_BUILDS.up(ETHID,BldPeriod) ;
    ETHCO2RET_BUILDS.l(ETHID,BldPeriod)$(ETHCO2RET_BUILDS.l(ETHID,BldPeriod) < PercentageBuilds*ETHCO2RET_BUILDS.up(ETHID,BldPeriod)) = 0 ;

    H2CO2RET_BUILDS.l(H2ID,BldPeriod)$(H2CO2RET_BUILDS.l(H2ID,BldPeriod) >= PercentageBuilds*H2CO2RET_BUILDS.up(H2ID,BldPeriod)) =
      H2CO2RET_BUILDS.up(H2ID,BldPeriod) ;
    H2CO2RET_BUILDS.l(H2ID,BldPeriod)$(H2CO2RET_BUILDS.l(H2ID,BldPeriod) < PercentageBuilds*H2CO2RET_BUILDS.up(H2ID,BldPeriod)) = 0 ;

    NGPCO2_BUILDS.l(NGPID,BldPeriod)$(NGPCO2_BUILDS.l(NGPID,BldPeriod) >= PercentageBuilds*NGPCO2_BUILDS.up(NGPID,BldPeriod)) =
      NGPCO2_BUILDS.up(NGPID,BldPeriod) ;
    NGPCO2_BUILDS.l(NGPID,BldPeriod)$(NGPCO2_BUILDS.l(NGPID,BldPeriod) < PercentageBuilds*NGPCO2_BUILDS.up(NGPID,BldPeriod)) = 0 ;
...
*   Save build period capacity expansion decisions
    ...
    PriorBuilds(RefReg,EthRefType,'EDM',RunYears) =
      -(1/ProcessTable('CO2ETH','EDM','EDM_CD1'))*sum((BldPeriod, ETHID)$ETHID_RefReg(ETHID,RefReg), ETHCO2RET_Builds.l(ETHID, BldPeriod));
    PriorBuilds(RefReg,EthRefType,'CO2ETHRET2',RunYears) = sum((BldPeriod,ETHID)$ETHID_RefReg(ETHID,RefReg),ETHCO2RET_Builds.l(ETHID, BldPeriod));
    PriorBuilds(RefReg,EthRefType,'CO2ETHRET',RunYears)  = 0 ;
    PriorBuilds(RefReg,EthRefType,'EDMRET',RunYears)  = -1*PriorBuilds(RefReg,EthRefType,'EDM',RunYears);

    PriorBuilds(RefReg,PetRefType,'H2P',RunYears) =
      -(1/ProcessTable('CO2H2P','H2P','H2P_NGS'))*sum((BldPeriod, H2ID)$H2RefReg(H2ID,RefReg), H2CO2RET_Builds.l(H2ID, BldPeriod));
    PriorBuilds(RefReg,PetRefType,'CO2H2RET2',RunYears) = sum((BldPeriod,H2ID)$H2RefReg(H2ID,RefReg),H2CO2RET_Builds.l(H2ID, BldPeriod));
    PriorBuilds(RefReg,PetRefType,'CO2H2RET',RunYears)  = 0 ;
    PriorBuilds(RefReg,PetRefType,'H2PRET',RunYears)  = -1*PriorBuilds(RefReg,PetRefType,'H2P',RunYears);

    PriorBuilds(RefReg,PetRefType,'CO2NGPRET2',RunYears) = sum((BldPeriod,NGPID)$NGPRefReg(NGPID,RefReg),NGPCO2_BUILDS.l(NGPID, BldPeriod));
    PriorBuilds(RefReg,PetRefType,'CO2NGPRET',RunYears)  = 0 ;

    ETHCO2_PriorBuilds(ETHID,RunYears) = sum(Period$BldPeriod(Period),ETHCO2RET_BUILDS.l(ETHID,Period));
    H2CO2_PriorBuilds(H2ID,RunYears) = sum(Period$BldPeriod(Period),H2CO2RET_BUILDS.l(H2ID,Period));
    NGPCO2_PriorBuilds(NGPID,RunYears) = sum(Period$BldPeriod(Period),NGPCO2_BUILDS.l(NGPID,Period));

*   CO2 tax credits cap
    Execute_Unload 'LFMM_Builds.gdx', PriorBuilds, ETHCO2_PriorBuilds,H2CO2_PriorBuilds,NGPCO2_PriorBuilds ;
...
*CO2CCSBalance_H2.m, CO2CCSBalance_CTL.m, CO2CCSBalance_ETH.m,
      ProdImpBalance.m, ProdImpSupCurve.m, ProdExpBalance.m,
*, StateBiodiesel.m
     ProdExpSupCurve.m ;
...
*    chooses to build above some fraction of capacity.
    ...
    BUILDS.l(RefReg,EthRefType,'CO2ETHRET','STEP01',BldPeriod) =
      sum(ETHID$ETHID_RefReg(ETHID,RefReg), ETHCO2RET_BUILDS.l(ETHID,BldPeriod)) ;

    BUILDS.l(RefReg,PetRefType,'CO2H2RET','STEP01',BldPeriod) =
      sum(H2ID$H2RefReg(H2ID,RefReg), H2CO2RET_BUILDS.l(H2ID,BldPeriod)) ;

    BUILDS.l(RefReg,PetRefType,'CO2NGPRET','STEP01',BldPeriod) =
      sum(NGPID$NGPRefReg(NGPID,RefReg), NGPCO2_BUILDS.l(NGPID,BldPeriod)) ;
...
*    CO2_PriorBuilds(ETHID,t)$(ord(t)<>CurYrIdx) = 0 ;
*    PriorBuilds(RefReg,RefType,'CO2RET',t)$(ord(t)<>CurYrIdx) = 0 ;
