...
** NEMS SEDS historical data in TRILLS per year
     ...
     EMEBLK_EAGTR(MNUMYR)
     EMEBLK_ENGIN(MNUMYR)
* Define Hydrogen Quantiy and PHMM parameters
	 HMMBLK_PHMM(M10,MNUMCR,MNUMYR)	  Price of hydrogen
	 HMMBLK_QHMM(M10,MNUMCR,MNUMYR)	  Quantity of hydrogen

     INDOUT_INQLGHP(MNUMCR,MNUMYR)    Industrial LPG heat and power feedstock
     INDOUT_HYDROGEN2REF(MNUMYR)      Industrial sourced hydrogen to refinery
...
$offtext
     ...
     PMMOUT_L_RG(M1000)                    Region (NEMS or Fuel region?) of new build in ECP
     PMMOUT_L_RY(M1000)                    Year plant built in ECP
     PMMOUT_L_IGRP(M1000)                  W_IGRP index for plant bult in ECP
     PMMOUT_L_PLTS                         Number of new builds from ECP
     PMMOUT_L_CFR(M1000)                   Capacity Factor of new CCS build
     PMMOUT_L_HRAT(M1000)                  Heatrate of new CCS build
     PMMOUT_L_CPTY(M1000)                  Capacity  of new CCS build
     PMMOUT_L_PTP(M1000)                   Plant type of new CCS build
     PMMOUT_L_OPCAP(M1000,MNUMYR)          Operating Capacity of new CCS build                                                                              
...
* NEMS conversion factors
     ...
     MPBLK_PETTR(MNUMCR,MNUMYR)       Ethanol - Transportation
...
* for ftab table 21
     ...
     LFMMOUT_REFPRODETRET(MNUMPR,MNUMYR)          Refinery production: ethanal retrofit
     LFMMOUT_REFPRODETNEW(MNUMPR,MNUMYR)          Refinery production: ethanal new
     LFMMOUT_REFCAPETRET(MNUMPR,MNUMYR)           Refinery capacity: ethanal retrofit
     LFMMOUT_REFCAPETNEW(MNUMPR,MNUMYR)           Refinery capacity: ethanal new
* INTOUT variables
...
* UECPOUT variables
     ...
     TCS45Q_CCS_EOR_45Q(MNUMYR)                             Carbon Tax credit for EOR 2016$
     TCS45Q_CCS_SALINE_45Q(MNUMYR)                             Carbon Tax credit for Saline 2016$
...
*** PMMOUT tables
     ...
     PMMOUT_DCRDWHP(MNUMOR,MNUMYR)            Domestic Crude Wellhead Price
...
*** AB32 tables
     ...
     OGSMOUT_OGCO2QLFs(M8,M13,MNUMYR)         CO2 quantity by source from LFMM(mmcf)

*** 45 tax credit
     TCS45Q_I_45Q_Duration                   45Q tax credit duration
     TCS45Q_I_45Q_LYR_NEW                    Last year of 45Q tax credit(no 45Q tax credit after this year)
     TCS45Q_I_45Q_SYR                    	First year of 45Q tax credit(no 45Q tax credit before this year)

     LFMMOUT_CO2TOEOR45q(M13,MNUMYR)         CO2 quantity to EOR by source from LFMM(mmcf)
     LFMMOUT_CO2TOSALINE45q(M13,MNUMYR)      CO2 quantity to SALINE by source from LFMM(mmcf)

     LFMMOUT_CO2TOEOR245q(M13,MNUMYR)         CO2 quantity to EOR by source from LFMM(mmcf)
     LFMMOUT_CO2TOSALINE245q(M13,MNUMYR)      CO2 quantity to SALINE by source from LFMM(mmcf)
;
...
$Load CRSTEP,CISTEP,MCSTEP,MNPROD,MNCRUD,MNUMOR,OGDIST, PRDEXP
$Load INSTEP, INTREG

$Load COALEMM_RCLCLNR,COALEMM_PLNT_EMF,
$Load PMMOUT_L_RG,PMMOUT_L_RY,PMMOUT_L_IGRP,PMMOUT_L_PLTS,PMMOUT_L_CFR
$Load PMMOUT_L_HRAT,PMMOUT_L_CPTY,PMMOUT_L_PTP,PMMOUT_L_OPCAP
...
$Load LFMMOUT_REFPRODETRET, LFMMOUT_REFPRODETNEW, LFMMOUT_REFCAPETRET, LFMMOUT_REFCAPETNEW
...
$Load OGSMOUT_OGCO2QLF,OGSMOUT_OGCO2PLF,OGSMOUT_OGCO2QLFs
...
$Load PMMOUT_DCRDWHP
$Load LFMMOUT_RFPRDDIESEL,LFMMOUT_CO2TOEOR45q,LFMMOUT_CO2TOSALINE45q,LFMMOUT_CO2TOEOR245q,LFMMOUT_CO2TOSALINE245q
...
$Load INDOUT_INQLGHP
$Load INDOUT_HYDROGEN2REF
* Load HMM parameters
$Load HMMBLK_PHMM, HMMBLK_QHMM
...
$Load TCS45Q_I_45Q_Duration, TCS45Q_I_45Q_LYR_NEW, TCS45Q_I_45Q_SYR
