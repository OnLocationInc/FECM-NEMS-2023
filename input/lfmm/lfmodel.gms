...
**** OL JR ****
****CO2 Transport variables****
   CO2toEOR(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)             XTL CO2 transported from FuelRegion to OGSM region for EOR in M tons CO2 per day(with co2 credit)
   CO2toSALINE(FuelRegion,CO2_SourceCCS,Period)                   XTL CO2 sent to saline storage in M tons CO2 per day(with co2 credit)
   CO2toEOR2(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)            XTL CO2 transported from FuelRegion to OGSM region for EOR in M tons CO2 per day (without co2 credit)
   CO2toSALINE2(FuelRegion,CO2_SourceCCS,Period)                  XTL CO2 sent to saline storage in M tons CO2 per day (without co2 credit)
*******************************
...
****CO2 Capture variables******
   ETHCO2RET_OPERATE(ETHID,Period)                                   Ethanol CO2 retrofit operate
   ETHCO2RET_BUILDS(ETHID,Period)                                    Ethanol CO2 retrofit bulid

   H2CO2RET_OPERATE(H2ID,Period)                                  Hydrogen CO2 retrofit operate
   H2CO2RET_BUILDS(H2ID,Period)                                   Hydrogen CO2 retrofit bulid

   NGPCO2_OPERATE(NGPID,Period)                                   NGP CO2 retrofit operate
   NGPCO2_BUILDS(NGPID,Period)                                    NGP CO2 retrofit build

   ETHCO2RET_OpCapture(ETHID,CaptureMode,Period)                                     Ethanol CO2 retrofit operate capture
   H2CO2RET_OpCapture(H2ID,CaptureMode,Period)                                    Hydrogen CO2 retrofit operate capture
   NGPCO2RET_OpCapture(NGPID,CaptureMode,Period)                                  NGP CO2 retrofit operate capture
*******************************
;
...
**** OL JR ****
**** CO2 Balance to EOR/Saline Contraints ****
*   CO2CCSBalance(FuelRegion,Period)                                 XTL captured CO2 must go to either EOR or Saline storage
   CO2CCSBalance_CTL(FuelRegion,Period)                             CTL captured CO2 must go to either EOR or Saline storage
   CO2CCSBalance_ETH(FuelRegion,Period)                             ETH captured CO2 must go to either EOR or Saline storage
   CO2CCSBalance_H2(FuelRegion,Period)                              Hydrogen captured CO2 must go to either EOR or Saline storage
   CO2CCSBalance_NGP(FuelRegion,Period)                             NGP captured CO2 must go to either EOR or Saline storage
*********************************************
...
****Carbon Capture constraints**************************
   ETHCO2RET_OP(RefReg, Period)                                       CO2 Ethanol carbon capture retrofit operate volume
   ETHCO2RET_BUILD(RefReg,Period)                                     CO2 Ethanol carbon capture  retrofit capacity
   ETHCO2RET_MAXBUILDS(ETHID)                                         Maximum ethanol capacity that may be retrofit over all periods in a given run year
   CO2Creditcap(FuelRegion,CO2_SourceCCS,Period)                   Maximum CO2 cap for CO2 credit

   H2CO2RET_OP(RefReg, Period)                                     Hydrogen process CO2 capture retrofit operate volume
   H2CO2RET_BUILD(RefReg,Period)                                   Hydrogen porcess CO2 capture retrofit capacity
   H2CO2RET_MAXBUILDS(H2ID)                                        Maximum H2 capacity that may be retrofit over all periods in a given run year

   NGPCO2RET_OP(RefReg,Period)                                     NGP process CO2 capture retrofit operate volume
   NGPCO2_BUILD(RefReg,Period)                                     NGP process capture retrofit capacity
*   NGPCO2_BUILDAvail(RefReg,Period)                               NGP process capture retrofit capacity based on available CO2 from OGSM
   NGPCO2_MAXBUILDS(NGPID)                                         Maximum NGP capacity that may be retrofitted overa ll periods in a given run year
*   NGPCO2RET_OPAvail(RefReg,Period)                               NGP process CO2 capture retrofit operate volume based on available CO2 from OGSM
********************************************************

**** OL JR ****
**** Updated 45Q constraints ****************************
*    Balance_ETH_45Q(FuelRegion,Period)                             CO2 captured for 45q credit from ETH must go to EOR-Saline credit variables
*    Balance_H2_45Q(FuelRegion,Period)                              CO2 captured for 45q credit from H2 must go to EOR-Saline credit variables
*    Balance_NGP_45Q(FuelRegion,Period)                             CO2 captured for 45q credit from NGP must go to EOR-Saline credit variables
    CapacityBalanceCCS(RefReg,RefType,Process,Period)               Retrofit OPERATECAP = PROCMODE
*    EOR_Saline_npv(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)
    ETHID_CaptureBalance(RefReg, Period)                                                        CO2 Capture Balance from Ethanol
    H2HID_CaptureBalance(RefReg, Period)                                                        CO2 Capture Balance from H2 plants
    NGPID_CaptureBalance(RefReg, Period)
    ETHID_Balance(ETHID,Period)
    H2ID_Balance(H2ID,Period)
    NGPID_Balance(NGPID,Period)
;

set
    PROCtoOPCAPeq(Process)                                          Set of specific Processes in which to set PROCMODE = OPERATECAP
                    /CO2ETHRET
                     CO2ETHRET2
                     CO2H2RET
                     CO2H2RET2
...
OBJ..
  - TotalCost +
  sum((ETHID,ActivePeriod),sum(RefReg,npv_ETHCO2RETBuildCost(ETHID,RefReg,ActivePeriod))*ETHCO2RET_BUILDS(ETHID,ActivePeriod) ) +
  sum((H2ID,ActivePeriod),sum(RefReg,npv_H2CO2RETBldCost(H2ID,RefReg,ActivePeriod))*H2CO2RET_BUILDS(H2ID,ActivePeriod) ) +
  sum((NGPID,ActivePeriod),sum(RefReg,npv_NGPRETBldCost(NGPID,RefReg,ActivePeriod))*NGPCO2_BUILDS(NGPID,ActivePeriod) ) +
  sum((ETHID,ActivePeriod),sum(RefReg,npv_ETHCO2RETopCost(ETHID,RefReg,ActivePeriod))*ETHCO2RET_OPERATE(ETHID,ActivePeriod))+
  sum((H2ID,ActivePeriod),sum(RefReg,npv_H2CO2RETopCost(H2ID,RefReg,ActivePeriod))*H2CO2RET_OPERATE(H2ID,ActivePeriod))+
  sum((NGPID,ActivePeriod),sum(RefReg,npv_NGPCO2RETopCost(NGPID,RefReg,ActivePeriod))*NGPCO2_OPERATE(NGPID,ActivePeriod))+
  sum((ETHIDR,ActivePeriod), npv_ETHnocreditSal(ETHIDR,ActivePeriod)*ETHCO2RET_OpCapture(ETHIDR,'capture',ActivePeriod)*ProcessTable('CO2CCS','CO2ETHRET2','CO2ETHRET2_CD3') ) +
  sum((ETHIDN,ActivePeriod), npv_ETHnocreditSal(ETHIDN,ActivePeriod)*ETHCO2RET_BUILDS(ETHIDN,ActivePeriod)*ProcessTable('CO2CCS','CO2ETHRET','CO2ETHRET_CD3') ) +
  sum((H2IDR,ActivePeriod), npv_H2nocreditSal(H2IDR,ActivePeriod)*H2CO2RET_OpCapture(H2IDR,'capture',ActivePeriod)*ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3') ) +
  sum((H2IDN,ActivePeriod), npv_H2nocreditSal(H2IDN,ActivePeriod)*H2CO2RET_BUILDS(H2IDN,ActivePeriod)*ProcessTable('CO2CCS','CO2H2RET','CO2H2RET_NGS3') ) +
  sum((NGPIDR,ActivePeriod), npv_NGPnocreditSal(NGPIDR,ActivePeriod)*NGPCO2RET_OpCapture(NGPIDR,'capture',ActivePeriod)*ProcessTable('CO2CCS','CO2NGPRET2','CO2NGPRET2_CD3') ) +
  sum((NGPIDN,ActivePeriod), npv_NGPnocreditSal(NGPIDN,ActivePeriod)*NGPCO2_BUILDS(NGPIDN,ActivePeriod)*ProcessTable('CO2CCS','CO2NGPRET','CO2NGPRET_CD3') ) +
...
* NOTE:  ERROR ABOVE is corrected below
  sum((ActivePeriod), -80. * ToSPECBLEND('7_RefReg','COKING','DSU','RDH',ActivePeriod)$StreamSpecProd('RDH','DSU') )    +
  sum((ActivePeriod), -80. * ToSPECBLEND('4_RefReg','COKING','DSU','RDH',ActivePeriod)$StreamSpecProd('RDH','DSU') )    +

* incentivize renewD production using 60.0 $(nom)/bbl revenue (for AEO2022)
  sum((ActivePeriod), -80. * ToRECIPEBLEND('7_RefReg','COKING','RCP_carbDSU2','RDH',ActivePeriod)$RecipeBlending('RCP_carbDSU2','RDH') ) +
  sum((ActivePeriod), -80. * ToRECIPEBLEND('4_RefReg','COKING','RCP_carbDSU2','RDH',ActivePeriod)$RecipeBlending('RCP_carbDSU2','RDH') ) +
  sum((ActivePeriod), -80. * ToRECIPEBLEND('7_RefReg','COKING','RCP_DSU2','RDH',ActivePeriod)$RecipeBlending('RCP_DSU2','RDH') ) +
  sum((ActivePeriod), -80. * ToRECIPEBLEND('4_RefReg','COKING','RCP_DSU2','RDH',ActivePeriod)$RecipeBlending('RCP_DSU2','RDH') ) +
...
*    npv_CO2TranStoreCost(DomRefReg,ActivePeriod) * PROCMODE(DomRefReg,RefType,Process,ProcessMode,ActivePeriod) * ProcessTable('CO2CCS',Process,ProcessMode) ) +
  sum((DomRefReg,RefType,Process,ProcessMode,ActivePeriod)
    $(ProcessTable('CO2',Process,ProcessMode) and
     ...
     not SameAs(Process,'EDMNEW') and not SameAs(Process,'EDMCCS') and
     not SameAs(Process,'CO2RET') and not SameAs(Process,'EDHNEW') and
     not SameAs(Process,'EDHCCS') and not SameAs(Process,'CO2RET2') and
     not SameAs(Process,'SEW') and not SameAs(Process,'AET') and
     RefTypProc(RefType,Process) and
     SupTypMode(DomRefReg,RefType,Process,ProcessMode)),
  ...
  sum((DomRefReg,RefType,Process,ProcessMode,ActivePeriod)
    $((ProcessTable('CO2CCS',Process,ProcessMode) or
     SameAs(Process,'EDMCCS') or SameAs(Process,'CO2RET') or
     SameAs(Process,'EDHCCS') or SameAs(Process,'CO2RET2') or
     SameAs(Process,'H2PCCS') or SameAs(Process,'CO2H2RET') or
     SameAs(Process,'CO2H2RET2')or SameAs(Process,'CO2NGPRET') or
     SameAs(Process,'CO2NGPRET2'))and RefTypProc(RefType,Process) and
     SupTypMode(DomRefReg,RefType,Process,ProcessMode) ),
    npv_CO2EmissionTax(ActivePeriod) * PROCMODE(DomRefReg,RefType,Process,ProcessMode,ActivePeriod) * ProcessTable('CO2CCS',Process,ProcessMode) ) -
  ...
  sum((FuelRegion,CO2_SourceCCS,ActivePeriod),
    (npv_FRtoSalineCost(FuelRegion,ActivePeriod)- npv_CO2creditSal(ActivePeriod)) * CO2toSALINE(FuelRegion,CO2_SourceCCS,ActivePeriod) ) +
  sum((FuelRegion,CO2_SourceCCS,ActivePeriod),
    npv_FRtoSalineCost(FuelRegion,ActivePeriod) * CO2toSALINE2(FuelRegion,CO2_SourceCCS,ActivePeriod) ) +
  sum((FuelRegion,OGCO2Reg,CO2_SourceCCS,ActivePeriod),
    (npv_FRtoEORCost(FuelRegion,OGCO2Reg,ActivePeriod) - npv_CO2creditEOR(ActivePeriod)) * CO2toEOR(FuelRegion,OGCO2Reg,CO2_SourceCCS,ActivePeriod) ) +
  sum((FuelRegion,OGCO2Reg,CO2_SourceCCS,ActivePeriod),
    npv_FRtoEORCost(FuelRegion,OGCO2Reg,ActivePeriod) * CO2toEOR2(FuelRegion,OGCO2Reg,CO2_SourceCCS,ActivePeriod) ) +
...
StreamBalance(RefReg,RefType,NonCrudeUtil,ActivePeriod)$(not SameAs(NonCrudeUtil,'CO2NGP') and not SameAs(NonCrudeUtil,'CO2NGP2') )..
...
CapacityBalance(RefReg,RefType,Process,Period)$(ActivePeriod(Period) and not PROCtoOPCAPeq(Process))..
...
CapacityBalanceCCS(RefReg,RefType,Process,Period)$(ActivePeriod(Period) and PROCtoOPCAPeq(Process))..
   sum(ProcessMode, CapFactor(Process,ProcessMode)*PROCMODE(RefReg,RefType,Process,ProcessMode,Period) ) -
   StreamFactors(Process)*OPERATECAP(RefReg,RefType,Process,Period) -
   StreamFactors(Process)*sum((BldStep,Period2)$(ord(Period2)<=ord(Period) and ord(Period2)>=BldPerIdx),
                            BUILDS(RefReg,RefType,Process,BldStep,Period2))
...
MaxLTE(RefReg,PetRefType,Period)$FuturePeriods(Period)..
   ...
   1.10 * ExistingCap(RefReg,PetRefType,'LTE','2008')
   =L= 0 ;
...
OGSM_CO2Demand(OGCO2Reg,ActivePeriod)..
   ...
   sum((FuelRegion,CO2_SourceCCS), CO2toEOR(FuelRegion,OGCO2Reg,CO2_SourceCCS,ActivePeriod) +
   CO2toEOR2(FuelRegion,OGCO2Reg,CO2_SourceCCS,ActivePeriod) ) +
...
OGSM_CO2TranBal(OGCO2Reg,ActivePeriod)..
   sum(CO2_Source$(not SameAs(CO2_Source,'PLANNED_POWER')),
    CO2_PURCH(OGCO2Reg,CO2_Source,ActivePeriod) ) -
   sum(OGCO2Reg2, CO2_TRAN(OGCO2Reg,OGCO2Reg2,ActivePeriod) )
   =E= 0 ;
$ontext
CO2CCSBalance(FuelRegion,Period)$ActivePeriod(Period)..
*   sum((DomRefReg,RefType,Process('CO2RET'),ProcessMode)$RefReg_2_FuelRegion(DomRefReg,FuelRegion),
*     PROCMODE(DomRefReg,RefType,Process,ProcessMode,ActivePeriod)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,RefType,Process('CTLCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum(ETHIDR$ETHID_FuelReg(ETHIDR,FuelRegion),
     ETHCO2RET_OPERATE(ETHIDR,Period) * ProcessTable('CO2CCS','CO2RET2','CO2RET2_CD3') ) +
   sum(ETHIDN$ETHID_FuelReg(ETHIDN,FuelRegion),
      sum(Period2$(ord(Period2)<=ord(Period) and ord(Period2)>=BldPerIdx),
        ETHCO2RET_BUILDS(ETHIDN,Period2)* ProcessTable('CO2CCS','CO2ETHRET','CO2ETHRET_CD3')) ) +
   sum(H2IDR$H2FuelReg(H2IDR,FuelRegion),
     H2CO2RET_OPERATE(H2IDR,Period) * ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3') ) +
   sum(H2IDN$H2FuelReg(H2IDN,FuelRegion),
      sum(Period2$(ord(Period2)<=ord(Period) and ord(Period2)>=BldPerIdx),
        H2CO2RET_BUILDS(H2IDN,Period2)* ProcessTable('CO2CCS','CO2H2RET','CO2H2RET_NGS3')) ) +
   sum((DomRefReg,RefType,Process('H2PCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,EthRefType,Process('EDHCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,EthRefType,Process('EDMCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,EthRefType,Process('EDHNEW'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,EthRefType,Process('EDMNEW'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,RefType,Process('CBLCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) -
   sum(OGCO2Reg, CO2toEOR(FuelRegion,OGCO2Reg,Period) ) -
   CO2toSALINE(FuelRegion,Period)-
   sum(OGCO2Reg, CO2toEOR2(FuelRegion,OGCO2Reg,Period) ) -
   CO2toSALINE2(FuelRegion,Period)
   =E= 0 ;
$offtext

*EOR_Saline_npv(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)..
*    CO2toEOR(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period) + CO2toSALINE(FuelRegion,CO2_SourceCCS,Period) =L= 100*npv_CO2creditSal(Period)  ;
* and H245QID_p(H2IDR,Period)
CO2CCSBalance_H2(FuelRegion,Period)$ActivePeriod(Period)..
   sum(H2IDR$(H2FuelReg(H2IDR,FuelRegion)),
     H2CO2RET_OpCapture(H2IDR,'capture',Period) * ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3') ) +
   sum(H2IDN$H2FuelReg(H2IDN,FuelRegion),
      sum(Period2$(ord(Period2)<=ord(Period) and ord(Period2)>=BldPerIdx),
        H2CO2RET_BUILDS(H2IDN,Period2)* ProcessTable('CO2CCS','CO2H2RET','CO2H2RET_NGS3')) ) +
   sum((DomRefReg,PetRefType,Process('H2PCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and not NoCaptureSet(Process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,PetRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) -
   sum(OGCO2Reg,
    CO2toEOR(FuelRegion,OGCO2Reg,'HYDROGEN_REF',Period) ) -
    CO2toSALINE(FuelRegion,'HYDROGEN_REF',Period)-
   sum(OGCO2Reg,
    CO2toEOR2(FuelRegion,OGCO2Reg,'HYDROGEN_REF',Period) ) -
    CO2toSALINE2(FuelRegion,'HYDROGEN_REF',Period)
   =E= 0 ;

CO2CCSBalance_CTL(FuelRegion,Period)$ActivePeriod(Period)..
*   sum((DomRefReg,RefType,Process('CO2RET'),ProcessMode)$RefReg_2_FuelRegion(DomRefReg,FuelRegion),
*     PROCMODE(DomRefReg,RefType,Process,ProcessMode,ActivePeriod)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,PetRefType,Process('CTLCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,PetRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode))+
   sum((DomRefReg,PetRefType,Process('CBLCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,PetRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) )-
   sum(OGCO2Reg,
     CO2toEOR(FuelRegion,OGCO2Reg,'PLANNED_CTL',Period) ) -
     CO2toSALINE(FuelRegion,'PLANNED_CTL',Period)-
   sum(OGCO2Reg,
     CO2toEOR2(FuelRegion,OGCO2Reg,'PLANNED_CTL',Period) ) -
     CO2toSALINE2(FuelRegion,'PLANNED_CTL',Period)
   =E= 0 ;

* and ETH45QID_p(ETHIDR,Period)
CO2CCSBalance_ETH(FuelRegion,Period)$ActivePeriod(Period)..
   sum(ETHIDR$(ETHID_FuelReg(ETHIDR,FuelRegion)),
     ETHCO2RET_OpCapture(ETHIDR,'capture',Period) * ProcessTable('CO2CCS','CO2ETHRET2','CO2ETHRET2_CD3') ) +
   sum(ETHIDN$ETHID_FuelReg(ETHIDN,FuelRegion),
      sum(Period2$(ord(Period2)<=ord(Period) and ord(Period2)>=BldPerIdx),
        ETHCO2RET_BUILDS(ETHIDN,Period2)* ProcessTable('CO2CCS','CO2ETHRET','CO2ETHRET_CD3')) ) +
   sum((DomRefReg,EthRefType,Process('EDHCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and not NoCaptureSet(Process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,EthRefType,Process('EDMCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and not NoCaptureSet(Process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,EthRefType,Process('EDHNEW'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
   sum((DomRefReg,EthRefType,Process('EDMNEW'),ProcessMode)$(ProcProcMode(process,ProcessMode) and RefReg_2_FuelRegion(DomRefReg,FuelRegion)),
     PROCMODE(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) -
   sum(OGCO2Reg,
     CO2toEOR(FuelRegion,OGCO2Reg,'ETHANOL',Period) ) -
     CO2toSALINE(FuelRegion,'ETHANOL',Period)-
   sum(OGCO2Reg,
     CO2toEOR2(FuelRegion,OGCO2Reg,'ETHANOL',Period) ) -
     CO2toSALINE2(FuelRegion,'ETHANOL',Period)
   =E= 0 ;
* and NGP45QID_p(NGPIDR,Period)
CO2CCSBalance_NGP(FuelRegion,Period)$ActivePeriod(Period)..
   sum(NGPIDR$(NGPFuelReg(NGPIDR,FuelRegion)),
         NGPCO2RET_OpCapture(NGPIDR,'capture',Period) * ProcessTable('CO2CCS','CO2NGPRET2','CO2NGPRET2_CD3') ) +
   sum(NGPIDN$NGPFuelReg(NGPIDN,FuelRegion),
       sum(Period2$(ord(Period2) <= ord(Period) and ord(Period2) >= BldPerIdx),
         NGPCO2_BUILDS(NGPIDN,Period2) * ProcessTable('CO2CCS','CO2NGPRET','CO2NGPRET_CD3')) ) -
   sum(OGCO2Reg,
    CO2toEOR(FuelRegion,OGCO2Reg,'NG_PROCESSING',Period) ) -
    CO2toSALINE(FuelRegion,'NG_PROCESSING',Period) -
   sum(OGCO2Reg,
    CO2toEOR2(FuelRegion,OGCO2Reg,'NG_PROCESSING',Period) ) -
    CO2toSALINE2(FuelRegion,'NG_PROCESSING',Period)
   =E= 0;

ETHID_Balance(ETHIDR,Period)$ActivePeriod(Period)..
    sum(CaptureMode,ETHCO2RET_OpCapture(ETHIDR,CaptureMode,Period)) - ETHCO2RET_OPERATE(ETHIDR,Period)
    =E= 0;

H2ID_Balance(H2IDR,Period)$ActivePeriod(Period)..
    sum(CaptureMode,H2CO2RET_OpCapture(H2IDR,CaptureMode,Period)) - H2CO2RET_OPERATE(H2IDR,Period)
    =E= 0;

NGPID_Balance(NGPIDR,Period)$ActivePeriod(Period)..
    sum(CaptureMode,NGPCO2RET_OpCapture(NGPIDR,CaptureMode,Period)) - NGPCO2_OPERATE(NGPIDR,Period)
    =E= 0;

ETHID_CaptureBalance(DomRefReg,Period)$ActivePeriod(Period)..
    sum(EthRefType,
        PROCMODE(DomRefReg,EthRefType,'CO2ETHRET2','CO2ETHRET2_CD3',Period)) -
    sum(ETHIDR$(ETHID_RefReg(ETHIDR,DomRefReg)),
        ETHCO2RET_OpCapture(ETHIDR,'capture',Period))
    =E= 0;

H2HID_CaptureBalance(DomRefReg,Period)$ActivePeriod(Period)..
    sum(PetRefType,
        PROCMODE(DomRefReg,PetRefType,'CO2H2RET2','CO2H2RET2_NGS3',Period)) -
    sum(H2IDR$(H2RefReg(H2IDR,DomRefReg)),
        H2CO2RET_OpCapture(H2IDR,'capture',Period))
    =E= 0;

NGPID_CaptureBalance(DomRefReg,Period)$ActivePeriod(Period)..
    sum(PetRefType,
        PROCMODE(DomRefReg,PetRefType,'CO2NGPRET2','CO2NGPRET2_CD3',Period)) -
    sum(NGPIDR$(NGPRefReg(NGPIDR,DomRefReg)),
        NGPCO2RET_OpCapture(NGPIDR,'capture',Period))
    =E= 0;

$ontext
Balance_H2_45Q(FuelRegion,Period)$ActivePeriod(Period)..
    sum(H2IDR$(H2FuelReg(H2IDR,FuelRegion) and H245QID_p(H2IDR,Period)),
        H2CO2RET_OPERATE(H2IDR,Period) * ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3') ) +
    sum(H2IDN$(H2FuelReg(H2IDN,FuelRegion)),
        sum(Period2$(ord(Period2)<=ord(Period) and ord(Period2)>=BldPerIdx),
            H2CO2RET_BUILDS(H2IDN,Period2)* ProcessTable('CO2CCS','CO2H2RET','CO2H2RET_NGS3')) ) -
    sum(OGCO2Reg,CO2toEOR(FuelRegion,OGCO2Reg,'HYDROGEN_REF',Period) ) -
    CO2toSALINE(FuelRegion,'HYDROGEN_REF',Period)
    =E= 0 ;

Balance_ETH_45Q(FuelRegion,Period)$ActivePeriod(Period)..
    sum(ETHIDR$(ETHID_FuelReg(ETHIDR,FuelRegion) and ETH45QID_p(ETHIDR,Period)),
     ETHCO2RET_OPERATE(ETHIDR,Period) * ProcessTable('CO2CCS','CO2RET2','CO2RET2_CD3') ) +
    sum(ETHIDN$(ETHID_FuelReg(ETHIDN,FuelRegion)),
        sum(Period2$(ord(Period2)<=ord(Period) and ord(Period2)>=BldPerIdx),
            ETHCO2RET_BUILDS(ETHIDN,Period2)* ProcessTable('CO2CCS','CO2ETHRET','CO2ETHRET_CD3')) ) -
...
Balance_NGP_45Q(FuelRegion,Period)$ActivePeriod(Period)..
    sum(NGPIDR$(NGPFuelReg(NGPIDR,FuelRegion) and NGP45QID_p(NGPIDR,Period)),
          NGPCO2_OPERATE(NGPIDR,Period) * ProcessTable('CO2CCS','CO2NGPRET2','CO2NGPRET2_CD3') ) +
    sum(NGPIDN$(NGPFuelReg(NGPIDN,FuelRegion)),
        sum(Period2$(ord(Period2) <= ord(Period) and ord(Period2) >= BldPerIdx),
          NGPCO2_BUILDS(NGPIDN,Period2) * ProcessTable('CO2CCS','CO2NGPRET','CO2NGPRET_CD3')) ) -
    sum(OGCO2Reg,
...
CO2Creditcap(FuelRegion,CO2_SourceCCS,PrcPeriod)..
   sum(OGCO2Reg, CO2toEOR(FuelRegion,OGCO2Reg,CO2_SourceCCS,PrcPeriod)) +
   CO2toSALINE(FuelRegion,CO2_SourceCCS,PrcPeriod) -
   CO2Cap_Credit(CO2_SourceCCS,FuelRegion)
   =L=0;
...
*******************Carbon Capture constraints******************************
ETHCO2RET_OP(RefReg, ActivePeriod)..
   sum(EthRefType, Operatecap(RefReg,EthRefType,'CO2ETHRET2',ActivePeriod)) -
   sum(ETHIDR$ETHID_RefReg(ETHIDR,RefReg),ETHCO2RET_OPERATE(ETHIDR,ActivePeriod))
   =E=0;

ETHCO2RET_BUILD(RefReg,ActivePeriod)..
   sum((EthRefType,BldStep), BUILDS(RefReg,EthRefType,'CO2ETHRET',BldStep,ActivePeriod))-
   sum(ETHID$ETHID_RefReg(ETHID,RefReg),ETHCO2RET_BUILDS(ETHID,ActivePeriod))
   =E=0;

ETHCO2RET_MAXBUILDS(ETHIDN)..
   sum(ActivePeriod$(not PrcPeriod(ActivePeriod)), ETHCO2RET_BUILDS(ETHIDN,ActivePeriod))
   =L=
   sum(RefReg$ETHID_RefReg(ETHIDN,RefReg), ETHID_TotalCO2(ETHIDN,RefReg));
*- sum(t$(ord(t)<CurYrIdx), CO2_PriorBuilds(ETHID,t)) ;

*Hydrogen constraints
H2CO2RET_OP(RefReg, ActivePeriod)..
   sum(PetRefType, Operatecap(RefReg,PetRefType,'CO2H2RET2',ActivePeriod)) -
   sum(H2IDR$H2RefReg(H2IDR,RefReg),H2CO2RET_OPERATE(H2IDR,ActivePeriod))
   =E=0;

H2CO2RET_BUILD(RefReg, ActivePeriod)..
   sum((PetRefType,BldStep), BUILDS(RefReg,PetRefType,'CO2H2RET',BldStep,ActivePeriod))-
   sum(H2ID$H2RefReg(H2ID,RefReg),H2CO2RET_BUILDS(H2ID,ActivePeriod))
   =E=0;

H2CO2RET_MAXBUILDS(H2IDN)..
   sum(ActivePeriod$(not PrcPeriod(ActivePeriod)), H2CO2RET_BUILDS(H2IDN,ActivePeriod))
   =L=
   sum(RefReg$H2RefReg(H2IDN,RefReg), H2TotCO2Control(H2IDN,RefReg)) ;

*NGP Constraints---
NGPCO2RET_OP(RefReg,ActivePeriod)..
  sum(PetRefType,Operatecap(RefReg,PetRefType,'CO2NGPRET2',ActivePeriod)) -
  sum(NGPIDR$NGPRefReg(NGPIDR,RefReg),NGPCO2_OPERATE(NGPIDR,ActivePeriod))
  =E=0;

*NGPCO2RET_OPAvail(RefReg,ActivePeriod)..
*  sum(NGPIDR$NGPRefReg(NGPIDR,RefReg),NGPCO2_OPERATE(NGPIDR,ActivePeriod))-
*  sum(OGCO2Reg$RefReg_2_OGCO2Reg(OGCO2Reg,RefReg),npv_CO2_Avail(OGCO2Reg,'NG_PROCESSING',ActivePeriod))
*  =L=0;

NGPCO2_BUILD(RefReg,ActivePeriod)..
  sum((PetRefType,BldStep),BUILDS(RefReg,PetRefType,'CO2NGPRET',BldStep,ActivePeriod))-
  sum(NGPID$NGPRefReg(NGPID,RefReg),NGPCO2_BUILDS(NGPID,ActivePeriod))
  =E=0;

*NGPCO2_BUILDAvail(RefReg,ActivePeriod)..
*  sum(NGPID$NGPRefReg(NGPID,RefReg),NGPCO2_BUILDS(NGPID,ActivePeriod))
*  =L= NGP_BuildsCap(RefReg,ActivePeriod);

NGPCO2_MAXBUILDS(NGPIDN)..
   sum(ActivePeriod$(not PrcPeriod(ActivePeriod)), NGPCO2_BUILDS(NGPIDN,ActivePeriod))
   =L= sum(RefReg$NGPRefReg(NGPIDN,RefReg), NGPTotCO2Control(NGPIDN,RefReg)) ;
