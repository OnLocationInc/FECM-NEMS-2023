...
******** Generate NEMS variables for pass back
  if(NCNTRL_CURCALYR=2016,
    PMMOUT_L_RG(M1000)   = 0 ;
    PMMOUT_L_RY(M1000)   = 0 ;
    PMMOUT_L_IGRP(M1000) = 0 ;
    PMMOUT_L_PLTS        = 0 ;
    PMMOUT_L_CFR(M1000)  = 0 ;
    PMMOUT_L_HRAT(M1000) = 0 ;
    PMMOUT_L_CPTY(M1000) = 0 ;
    PMMOUT_L_PTP(M1000)  = 0 ;
    PMMOUT_L_OPCAP(M1000,MNUMYR) = 0 ;                                      
  );
...
********************************** HMM Reporting JR ONL ****************************************
*   Quantity of hydrogen purchased by refineries from the HMM for census division
    HMMBLK_QHMM('10_M10','07_West_South_Central',MNUMYR) =
           REFPURCH.l('4_Refreg','H2','Step02',Period)*1000*(5.8*10**6)/(10**12)*365;
*   National quantity of hydrogen purchased by refineries from the HMM
    HMMBLK_QHMM('10_M10','11_United_States',MNUMYR) = 
           HMMBLK_QHMM('10_M10','07_West_South_Central',MNUMYR);
...
$ontext
    PMMRPT_CRNETHCD('11_United_States',MNUMYR) =
      sum((DomRefReg,EthRefType,CornProcess,ProcessMode),PROCMODE.l(DomRefReg,EthRefType,CornProcess,ProcessMode,Period) );
$offtext

   LFMMOUT_REFPRODETRET(MNUMPR,MNUMYR) =
      sum((DomRefReg,EthRefType,ProcessMode)$(ProcessTable('ETHCRN','EDMRET',ProcessMode) and RefReg2MNUMPR(MNUMPR,DomRefReg)),
         ProcessTable('ETHCRN','EDMRET',ProcessMode)*PROCMODE.l(DomRefReg,EthRefType,'EDMRET',ProcessMode,Period) ) +
      sum((DomRefReg,EthRefType,ProcessMode)$(ProcessTable('ETHCRNexp','EDMRET',ProcessMode)and RefReg2MNUMPR(MNUMPR,DomRefReg)),
         ProcessTable('ETHCRNexp','EDMRET',ProcessMode)*PROCMODE.l(DomRefReg,EthRefType,'EDMRET',ProcessMode,Period)) ;

   LFMMOUT_REFPRODETRET('10_MNUMPR',MNUMYR) =
      sum(MNUMPR$(ord(MNUMPR)<9), LFMMOUT_REFPRODETRET(MNUMPR,MNUMYR) ) ;

   LFMMOUT_REFPRODETNEW(MNUMPR,MNUMYR) =
      sum((DomRefReg,EthRefType,ProcessMode)$(ProcessTable('ETHCRN','EDHCCS',ProcessMode) and RefReg2MNUMPR(MNUMPR,DomRefReg)),
         ProcessTable('ETHCRN','EDHCCS',ProcessMode)*PROCMODE.l(DomRefReg,EthRefType,'EDHCCS',ProcessMode,Period) ) +
      sum((DomRefReg,EthRefType,ProcessMode)$(ProcessTable('ETHCRNexp','EDHCCS',ProcessMode) and RefReg2MNUMPR(MNUMPR,DomRefReg)),
         ProcessTable('ETHCRNexp','EDHCCS',ProcessMode)*PROCMODE.l(DomRefReg,EthRefType,'EDHCCS',ProcessMode,Period)) +
      sum((DomRefReg,EthRefType,ProcessMode)$(ProcessTable('ETHCRN','EDMCCS',ProcessMode) and RefReg2MNUMPR(MNUMPR,DomRefReg)),
         ProcessTable('ETHCRN','EDMCCS',ProcessMode)*PROCMODE.l(DomRefReg,EthRefType,'EDMCCS',ProcessMode,Period) ) +
      sum((DomRefReg,EthRefType,ProcessMode)$(ProcessTable('ETHCRNexp','EDMCCS',ProcessMode) and RefReg2MNUMPR(MNUMPR,DomRefReg)),
         ProcessTable('ETHCRNexp','EDMCCS',ProcessMode)*PROCMODE.l(DomRefReg,EthRefType,'EDMCCS',ProcessMode,Period))  ;

  LFMMOUT_REFPRODETNEW('10_MNUMPR',MNUMYR) =
      sum(MNUMPR$(ord(MNUMPR)<9), LFMMOUT_REFPRODETNEW(MNUMPR,MNUMYR) ) ;
...
* (MMbbl/cd)
    ...
    LFMMOUT_REFCAPETRET(MNUMPR,MNUMYR) =
      sum(DomRefReg$RefReg2MNUMPR(MNUMPR,DomRefReg),AvailCap(DomRefReg,'ETH_REF','CO2ETHRET2')) ;

    LFMMOUT_REFCAPETRET('10_MNUMPR',MNUMYR) =
      sum(MNUMPR$(ord(MNUMPR)<9), LFMMOUT_REFCAPETRET(MNUMPR,MNUMYR) ) ;

    LFMMOUT_REFCAPETNEW(MNUMPR,MNUMYR) =
      sum(DomRefReg$RefReg2MNUMPR(MNUMPR,DomRefReg),(AvailCap(DomRefReg,'ETH_REF','EDHCCS')  +
                                                               AvailCap(DomRefReg,'ETH_REF','EDMCCS')) ) ;

    LFMMOUT_REFCAPETNEW('10_MNUMPR',MNUMYR) =
      sum(MNUMPR$(ord(MNUMPR)<9), LFMMOUT_REFCAPETNEW(MNUMPR,MNUMYR) ) ;
...
*  ** total ethanol consumed by CD via E85
    ...
    PMMFTAB_CRNCAPCD('11_United_States',MNUMYR) =
      sum((DomRefReg,EthRefType,CornProcess), AvailCap(DomRefReg,EthRefType,CornProcess) );
...
* MM bfoe/cd, by LFMM region
    ...
    PMMFTAB_RFHCXH2IN(MNUMPR,MNUMYR) =
      (sum((DomRefReg,RefType,Process('H2P'),ProcessMode('H2P_NGS'))$RefReg2MNUMPR(MNUMPR,DomRefReg),
        abs(ProcessTable('NGS',Process,ProcessMode))*PROCMODE.l(DomRefReg,RefType,Process,ProcessMode,Period))+
      sum((DomRefReg,RefType,Process('H2P'),ProcessMode('H2P_NGSnoCCS'))$RefReg2MNUMPR(MNUMPR,DomRefReg),
        abs(ProcessTable('NGS',Process,ProcessMode))*PROCMODE.l(DomRefReg,RefType,Process,ProcessMode,Period))+
      sum((DomRefReg,RefType,Process('H2PCCS'),ProcessMode('H2PCCS_NGS2'))$RefReg2MNUMPR(MNUMPR,DomRefReg),
        abs(ProcessTable('NGS',Process,ProcessMode))*PROCMODE.l(DomRefReg,RefType,Process,ProcessMode,Period))+
      sum((DomRefReg,RefType,Process('H2PNEW'),ProcessMode('H2PNEW_NGS'))$RefReg2MNUMPR(MNUMPR,DomRefReg),
        abs(ProcessTable('NGS',Process,ProcessMode))*PROCMODE.l(DomRefReg,RefType,Process,ProcessMode,Period))+
      sum((DomRefReg,RefType,Process('H2PRET'),ProcessMode('H2PRET_NGS2'))$RefReg2MNUMPR(MNUMPR,DomRefReg),
...
* adding hydrogen from the HMM (REFPURCH - step 2)
* ASSUMED to go into RefReg 4 only
    ...
    PMMFTAB_RFHCXH2IN('04_MNUMPR',MNUMYR) = PMMFTAB_RFHCXH2IN('04_MNUMPR',MNUMYR) +
                                            ...
                                            REFPURCH.l('4_Refreg','H2','Step02',Period) /1000;
...
* CO2 captured by xTLs in MM tonnes C per year
    EMISSION_CCS_PMM('2_M5',MNUMPR,MNUMYR) =
      sum((RefReg,RefType)$RefReg2MNUMPR(MNUMPR,RefReg),
        sum((Process('CTLCCS'),ProcessMode),
          PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        (8.04/11.37)*sum((Process('CBLCCS'),ProcessMode),
          PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) ) * (12/44) * 365 / 1000 ;

    EMISSION_CCS_PMM('1_M5',MNUMPR,MNUMYR) =
      sum((RefReg,RefType)$RefReg2MNUMPR(MNUMPR,RefReg),
        (3.33/11.37)*sum((Process('CBLCCS'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('EDHCCS'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('EDMCCS'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('CO2ETHRET'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('CO2ETHRET2'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('H2PCCS'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('CO2H2RET'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('CO2H2RET2'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('CO2NGPRET'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) ) +
        sum((Process('CO2NGPRET2'),ProcessMode),
           PROCMODE.l(RefReg,RefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) )
        ) * (12/44) * 365 / 1000 ;
    ...
    OGSMOUT_OGCO2QLFs(M8,M13,MNUMYR) =
      sum((FuelRegion,OGCO2Reg,CO2_SourceCCS)$(OGCO2Reg_2_M8(OGCO2Reg,M8) and CO2_Source_2_M13(CO2_SourceCCS,M13)),
        CO2toEOR.l(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period) + CO2toEOR2.l(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)) * 18 * 365 ;

    OGSMOUT_OGCO2QLFs('8_M8',M13,MNUMYR) =
      sum(M8$(ord(M8)<8), OGSMOUT_OGCO2QLFs(M8,M13,MNUMYR) ) ;


    LFMMOUT_CO2TOEOR45q(M13,MNUMYR) =
       sum((FuelRegion,OGCO2Reg,CO2_SourceCCS)$(CO2_Source_2_M13(CO2_SourceCCS,M13)),CO2toEOR.l(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)) ;

    LFMMOUT_CO2TOSALINE45q(M13,MNUMYR) =
       sum((FuelRegion,CO2_SourceCCS)$(CO2_Source_2_M13(CO2_SourceCCS,M13)),CO2toSALINE.l(FuelRegion,CO2_SourceCCS,Period)) ;

    LFMMOUT_CO2TOEOR245q(M13,MNUMYR) =
       sum((FuelRegion,OGCO2Reg,CO2_SourceCCS)$(CO2_Source_2_M13(CO2_SourceCCS,M13)),CO2toEOR2.l(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)) ;

    LFMMOUT_CO2TOSALINE245q(M13,MNUMYR) =
       sum((FuelRegion,CO2_SourceCCS)$(CO2_Source_2_M13(CO2_SourceCCS,M13)),CO2toSALINE2.l(FuelRegion,CO2_SourceCCS,Period)) ;

    OGSMOUT_OGCO2QLF(M8,MNUMYR) =
      sum((FuelRegion,OGCO2Reg,CO2_SourceCCS)$OGCO2Reg_2_M8(OGCO2Reg,M8),
        CO2toEOR.l(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period) + CO2toEOR2.l(FuelRegion,OGCO2Reg,CO2_SourceCCS,Period)) * 18 * 365 ;
...
* Set the CTL CO2 price to just below the marginal CO2 price
    OGSMOUT_OGCO2PLF(M8,MNUMYR) =
      sum((OGCO2Reg,BldPeriod)$OGCO2Reg_2_M8(OGCO2Reg,M8),
        0.85 * OGSM_CO2Demand.m(OGCO2Reg,BldPeriod) ) * GDP('2008') / 18 / GDP(t) / Discount(Rate(t)) ;

*    OGSMOUT_OGCO2PLF(M8,MNUMYR) = 1 ;
...
plant_counter = PMMOUT_L_PLTS;
CO2_OPERATE(M1000,t) = sum(MNUMYR$tMNUM(t,MNUMYR), PMMOUT_L_OPCAP(M1000,MNUMYR));                                                                                 
loop((DomRefReg,Period(BldPeriod),FuelRegion)$RefReg_2_FuelRegion(DomRefReg,FuelRegion),
  ...
  if(sum((PetRefType,BldStep), BUILDS.l(DomRefReg,PetRefType,'CTLCCS',BldStep,Period))>0,
    ...
    PMMOUT_L_RG(M1000)$(ord(M1000)=plant_counter)   = FuelRegion.val ;
    PMMOUT_L_RY(M1000)$(ord(M1000)=plant_counter)   = NCNTRL_CURCALYR+1 ;
    PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = 20000 + plant_counter ;
    PMMOUT_L_PLTS                                   = plant_counter ;
    PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('CTLCCS') ;
    PMMOUT_L_HRAT(M1000)$(ord(M1000)=plant_counter) = abs(ProcessTableCrude('COA','all','CTLCCS','CTLCCS_CDSCCS')) * 1000000 /
                                                         abs(ProcessTableCrude('KWGrid','all','CTLCCS','CTLCCS_CDSCCS')) ;
    PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) = sum((PetRefType,BldStep), BUILDS.l(DomRefReg,PetRefType,'CTLCCS',BldStep,Period)) *
                                                         abs(ProcessTableCrude('KWGrid','all','CTLCCS','CTLCCS_CDSCCS')) * 365 / 8760 ;
    PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 60 ;
...
*    plant_counter = plant_counter + 1 ;
  ...
  if(sum((PetRefType,BldStep), BUILDS.l(DomRefReg,PetRefType,'CBLCCS',BldStep,Period))>0,
    ...
    PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = 20000 + plant_counter ;
    PMMOUT_L_PLTS                                   = plant_counter ;
    PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('CBLCCS') ;
    PMMOUT_L_HRAT(M1000)$(ord(M1000)=plant_counter) = abs(ProcessTableCrude('COA','all','CBLCCS','CBLCCS_CBJECCS')+ProcessTableCrude('ECR','all','CBLCCS','CBLCCS_CBJECCS')) * 1000000 /
                                                         abs(ProcessTableCrude('KWGrid','all','CBLCCS','CBLCCS_CBJECCS')) ;
    PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) = sum((PetRefType,BldStep), BUILDS.l(DomRefReg,PetRefType,'CBLCCS',BldStep,Period)) *
                                                         abs(ProcessTableCrude('KWGrid','all','CBLCCS','CBLCCS_CBJECCS')) * 365 / 8760 ;
    PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 61 ;


  );
*ETHCCS and EDMCCS report
  if(sum((RefType,BldStep), BUILDS.l(DomRefReg,RefType,'EDHCCS',BldStep,Period))>0,
    plant_counter = plant_counter + 1 ;
    PMMOUT_L_RG(M1000)$(ord(M1000)=plant_counter)   = FuelRegion.val ;
    PMMOUT_L_RY(M1000)$(ord(M1000)=plant_counter)   = NCNTRL_CURCALYR+1 ;
    PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = 30000 + plant_counter ;
    PMMOUT_L_PLTS                                   = plant_counter ;
    PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('EDHCCS') ;
    PMMOUT_L_HRAT(M1000)$(ord(M1000)=plant_counter) = 1;
    PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) =
      sum((RefType,BldStep), BUILDS.l(DomRefReg,RefType,'EDHCCS',BldStep,Period) * ProcessTable('CO2CCS','EDHCCS','EDHCCS_CD2') * 365/1000 );
    PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 62 ;


  );

  if(sum((RefType,BldStep), BUILDS.l(DomRefReg,RefType,'EDMCCS',BldStep,Period))>0,
    plant_counter = plant_counter + 1 ;
    PMMOUT_L_RG(M1000)$(ord(M1000)=plant_counter)   = FuelRegion.val ;
    PMMOUT_L_RY(M1000)$(ord(M1000)=plant_counter)   = NCNTRL_CURCALYR+1 ;
    PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = 30000 + plant_counter ;
    PMMOUT_L_PLTS                                   = plant_counter ;
    PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('EDMCCS') ;
    PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) =
      sum((RefType,BldStep), BUILDS.l(DomRefReg,RefType,'EDMCCS',BldStep,Period) * ProcessTable('CO2CCS','EDMCCS','EDMCCS_CD2') * 365/1000 );
    PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 63 ;


  );

  if(sum((RefType,BldStep), BUILDS.l(DomRefReg,RefType,'H2PCCS',BldStep,Period))>0,
    plant_counter = plant_counter + 1 ;
    PMMOUT_L_RG(M1000)$(ord(M1000)=plant_counter)   = FuelRegion.val ;
    PMMOUT_L_RY(M1000)$(ord(M1000)=plant_counter)   = NCNTRL_CURCALYR+1 ;
    PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = 35000 + plant_counter ;
    PMMOUT_L_PLTS                                   = plant_counter ;
    PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('H2PCCS') ;
    PMMOUT_L_HRAT(M1000)$(ord(M1000)=plant_counter) = 1;
    PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) =
      sum((RefType,BldStep), BUILDS.l(DomRefReg,RefType,'H2PCCS',BldStep,Period) * ProcessTable('CO2CCS','H2PCCS','H2PCCS_NGS2') * 365/1000 );
    PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 66 ;

  );
);
loop((DomRefReg,Period(PrcPeriod),FuelRegion)$RefReg_2_FuelRegion(DomRefReg,FuelRegion),
  loop(M1000$(ord(M1000) le plant_counter and PMMOUT_L_IGRP(M1000)=ord(M1000) + 30000),
    CO2_OPERATE(M1000,t)$(t.val=NCNTRL_CURCALYR and sum(RefType, OPERATECAP.l(DomRefReg,RefType,'EDHCCS',Period))>0) =
      sum((EthRefType,Process('EDHCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and not NoCaptureSet(Process,ProcessMode)),
           PROCMODE.l(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) * 365/1000 );
  );

  loop(M1000$(ord(M1000) le plant_counter and PMMOUT_L_IGRP(M1000)=ord(M1000) + 30000),
    CO2_OPERATE(M1000,t)$(t.val=NCNTRL_CURCALYR and sum(RefType, OPERATECAP.l(DomRefReg,RefType,'EDMCCS',Period))>0) =
      sum((EthRefType,Process('EDMCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and not NoCaptureSet(Process,ProcessMode)),
           PROCMODE.l(DomRefReg,EthRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) * 365/1000 );
  );

  loop(M1000$(ord(M1000) le plant_counter and PMMOUT_L_IGRP(M1000)=ord(M1000) + 35000),
    CO2_OPERATE(M1000,t)$(t.val=NCNTRL_CURCALYR and sum(RefType, OPERATECAP.l(DomRefReg,RefType,'H2PCCS',Period))>0) =
      sum((PetRefType,Process('H2PCCS'),ProcessMode)$(ProcProcMode(process,ProcessMode) and not NoCaptureSet(Process,ProcessMode)),
           PROCMODE.l(DomRefReg,PetRefType,Process,ProcessMode,Period)*ProcessTable('CO2CCS',Process,ProcessMode) * 365/1000 );
  );
);

loop((ETHID,FuelRegion,Period(BldPeriod))$(ETHID_FuelReg(ETHID,FuelRegion) and (ETHCO2RET_BUILDS.l(ETHID,Period)>0)),
  plant_counter = plant_counter + 1 ;
  PMMOUT_L_RG(M1000)$(ord(M1000)=plant_counter)   = FuelRegion.val ;
  PMMOUT_L_RY(M1000)$(ord(M1000)=plant_counter)   = NCNTRL_CURCALYR+1 ;
  PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = ETHID.val - 50000 ;
  PMMOUT_L_PLTS                                   = plant_counter ;
  PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('CO2ETHRET') ;
  PMMOUT_L_HRAT(M1000)$(ord(M1000)=plant_counter) = 1;
  PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) = ETHCO2RET_BUILDS.l(ETHID,Period)*ProcessTable('CO2CCS','CO2ETHRET','CO2ETHRET_CD3')*365/1000 ;
  PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 64 ;
);
loop((M1000,ETHID,Period(PrcPeriod))$(ord(M1000) le plant_counter and ETHCO2RET_OpCapture.l(ETHID,'capture',PrcPeriod)>0 and PMMOUT_L_IGRP(M1000)=ETHID.val - 50000),
  CO2_OPERATE(M1000,t)$(t.val=NCNTRL_CURCALYR)
   = ETHCO2RET_OpCapture.l(ETHID,'capture',PrcPeriod)*ProcessTable('CO2CCS','CO2ETHRET2','CO2ETHRET2_CD3')*365/1000 ;
);

loop((H2ID,FuelRegion,Period(BldPeriod))$(H2FuelReg(H2ID,FuelRegion) and (H2CO2RET_BUILDS.l(H2ID,Period)>0)),
    plant_counter = plant_counter + 1 ;
  PMMOUT_L_RG(M1000)$(ord(M1000)=plant_counter)   = FuelRegion.val ;
  PMMOUT_L_RY(M1000)$(ord(M1000)=plant_counter)   = NCNTRL_CURCALYR+1 ;
  PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = H2ID.val - 50000 ;
  PMMOUT_L_PLTS                                   = plant_counter ;
  PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('CO2H2RET') ;
  PMMOUT_L_HRAT(M1000)$(ord(M1000)=plant_counter) = 1;
  PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) = H2CO2RET_BUILDS.l(H2ID,Period)*ProcessTable('CO2CCS','CO2H2RET','CO2H2RET_NGS3')*365/1000 ;
  PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 67 ;
);
loop((M1000,H2ID,Period(PrcPeriod))$(ord(M1000) le plant_counter and H2CO2RET_OpCapture.l(H2ID,'capture',PrcPeriod)>0 and PMMOUT_L_IGRP(M1000)=H2ID.val - 50000),
  CO2_OPERATE(M1000,t)$(t.val=NCNTRL_CURCALYR)
   = H2CO2RET_OpCapture.l(H2ID,'capture',PrcPeriod)*ProcessTable('CO2CCS','CO2H2RET2','CO2H2RET2_NGS3')*365/1000 ;
  );

loop((NGPID,FuelRegion,Period(BldPeriod))$(NGPFuelReg(NGPID,FuelRegion) and (NGPCO2_BUILDS.l(NGPID,Period)>0)),
  plant_counter = plant_counter + 1 ;
  PMMOUT_L_RG(M1000)$(ord(M1000)=plant_counter)   = FuelRegion.val ;
  PMMOUT_L_RY(M1000)$(ord(M1000)=plant_counter)   = NCNTRL_CURCALYR+1 ;
  PMMOUT_L_IGRP(M1000)$(ord(M1000)=plant_counter) = NGPID.val - 50000 ;
  PMMOUT_L_PLTS                                   = plant_counter ;
  PMMOUT_L_CFR(M1000)$(ord(M1000)=plant_counter)  = StreamFactors('CO2NGPRET') ;
  PMMOUT_L_HRAT(M1000)$(ord(M1000)=plant_counter) = 1;
  PMMOUT_L_CPTY(M1000)$(ord(M1000)=plant_counter) = NGPCO2_BUILDS.l(NGPID,Period)*ProcessTable('CO2CCS','CO2NGPRET','CO2NGPRET_CD3')*365/1000 ;
  PMMOUT_L_PTP(M1000)$(ord(M1000)=plant_counter)  = 68 ;
);
loop((M1000,NGPID,Period(PrcPeriod))$(ord(M1000) le plant_counter and NGPCO2RET_OpCapture.l(NGPID,'capture',PrcPeriod)>0 and PMMOUT_L_IGRP(M1000)=NGPID.val - 50000),
  CO2_OPERATE(M1000,t)$(t.val=NCNTRL_CURCALYR)
   = NGPCO2RET_OpCapture.l(NGPID,'capture',PrcPeriod)*ProcessTable('CO2CCS','CO2NGPRET2','CO2NGPRET2_CD3')*365/1000 ;
);

PMMOUT_L_OPCAP(M1000,MNUMYR) = sum(t$tMNUM(t,MNUMYR), CO2_OPERATE(M1000,t));
...
Loop(M1000$(ord(M1000)<PMMOUT_L_PLTS),
  CheckValue(PMMOUT_L_RG(M1000));
  CheckValue(PMMOUT_L_RY(M1000));
  CheckValue(PMMOUT_L_IGRP(M1000));
  CheckValue(PMMOUT_L_CFR(M1000));
  CheckValue(PMMOUT_L_HRAT(M1000));
  CheckValue(PMMOUT_L_CPTY(M1000));
  CheckValue(PMMOUT_L_PTP(M1000));
);
...
Loop(MNUMYR,
  ...
  Loop(M8,
   Loop(M13,
     CheckValue(OGSMOUT_OGCO2QLFs(M8,M13,MNUMYR));
   );
  );
  ...
  Loop(MNUMPR,
   ...
   CheckValue(LFMMOUT_REFPRODETRET(MNUMPR,MNUMYR));
   CheckValue(LFMMOUT_REFPRODETNEW(MNUMPR,MNUMYR));
   CheckValue(LFMMOUT_REFCAPETRET(MNUMPR,MNUMYR));
   CheckValue(LFMMOUT_REFCAPETNEW(MNUMPR,MNUMYR));
  ...
  Loop(MNUMCR,
   ...
   loop(M10,
    CheckValue(HMMBLK_QHMM(M10,MNUMCR,MNUMYR));
   );
...
repStreamFlows(DomRefReg,GrainCrops,'Grains','Feedstocks','Biofuels',RunYears) =
    -sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
    ProcessTable(GrainCrops,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period))/1000;

repStreamFlows(DomRefReg,BioStr,'Cellulose','Feedstocks','Biofuels',RunYears) =
    -sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
    ProcessTable(BioStr,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period)) /
...
repStreamFlows(DomRefReg,RenewableOils,'Oils','Feedstocks','Biofuels',RunYears) =
    -sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
    ProcessTable(RenewableOils,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period) /
...
repStreamFlows(DomRefReg,EthStream,'Ethanol','Intermediates','Biofuels',RunYears) =
    sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
        ProcessTable(EthStream,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period));
repStreamFlows(DomRefReg,BioNaphtha,'Naphtha','Intermediates','Biofuels',RunYears) =
    sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
    ProcessTable(BioNaphtha,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period));
repStreamFlows(DomRefReg,BioDistillate,'Distillate','Intermediates','Biofuels',RunYears) =
    sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
    ProcessTable(BioDistillate,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period));
repStreamFlows(DomRefReg,NPFNaphtha,'Naphtha','Intermediates','NonPetroleumFossil',RunYears) =
    sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
    ProcessTable(NPFNaphtha,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period));
repStreamFlows(DomRefReg,NPFDistillate,'Distillate','Intermediates','NonPetroleumFossil',RunYears) =
    sum((RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period(PrcPeriod)),
    ProcessTable(NPFDistillate,LCFS_BioProcess_REP,LCFS_BioMode_REP) * PROCMODE.l(DomRefReg,RefType,LCFS_BioProcess_REP,LCFS_BioMode_REP,Period));
$ontext
repStreamFlows(DomRefReg,EthStream,'Ethanol','Intermediates','Biofuels',RunYears) =
    sum((RefType,CFP_BioProcess,CFP_BioMode,Period(PrcPeriod)),
...
*   COALEMM_N_HRAT,COALEMM_N_CPTY,COALEMM_N_PTP,
   PMMOUT_L_RG,PMMOUT_L_RY,PMMOUT_L_IGRP,PMMOUT_L_PLTS,PMMOUT_L_CFR,
   PMMOUT_L_HRAT,PMMOUT_L_CPTY,PMMOUT_L_PTP,PMMOUT_L_OPCAP,
   PMMOUT_XTL_CO2AVAIL, EMISSION_CCS_PMM,
   OGSMOUT_OGCO2QLF,OGSMOUT_OGCO2QLFS, OGSMOUT_OGCO2PLF, LFMMOUT_PROFIT_BBL, LFMMOUT_RFCRUDEWHP,
   CONVFACT_APICAMG,
   ...
   LFMMOUT_REFPRODETRET, LFMMOUT_REFPRODETNEW, LFMMOUT_REFCAPETRET, LFMMOUT_REFCAPETNEW,
   LFMMOUT_CO2TOEOR45q, LFMMOUT_CO2TOSALINE45q,LFMMOUT_CO2TOEOR245q, LFMMOUT_CO2TOSALINE245q
   HMMBLK_QHMM;
