...
Set  AllNode                         Plant + transship + Inj
     ...
     link_input(FromNode,ToNode,diameter)  possible pipeline links
...
$Load th, tm, tf, t_emm, t, tstart, tend, AllNode, FromNode, ToNode, Plant, source, OnePipeNodes, Inj, transShip, diameter, link, link_input, unitplt, N_IGRP, PS_IGRP, Nplant
...
Set     UnitType(source,SourceType)     source-type mapping
        ...
        ;
$Load UnitType HMM_Source Cem_Source_new Cem_Source_exs
...
N_RY(source)                              New plant go-live year
...
$LOAD SrcCapCost, CaptureCost, CaptureMax, CarbonOffset, N_RY,PV, I_RY
...
parameter
  ...
  maxPipesT(t,diameter)         Maximum number of 36" pipes between any two points you could need based on max capture
...
*Changed following line
  ...
  sum(HMM_Source$(t.val>=N_RY(HMM_Source)), CaptureMax(HMM_Source,t)) +
  sum(Cem_Source_exs$(t.val>=I_RY(Cem_Source_exs)), CaptureMax(Cem_Source_exs,t)) +
  sum(Cem_Source_new$(t.val>=N_RY(Cem_Source_new)), CaptureMax(Cem_Source_new,t));

maxPipes = min(100,max(1, ceil(smax(t,totCaptureMax(t))/PipeMax('36')))) ;
maxPipesT(t,diameter)$totCaptureMax(t) = min(100,max(1, ceil(totCaptureMax(t)/PipeMax(diameter)))) ;

EORSafetyPrice = smax((link,t),PipeCapCost(link,t)) * 2 ;
*BLDPIPE.up(Link(FromNode,ToNode,diameter),t) = 1 ;
BLDPIPE.up(Link(FromNode,ToNode,diameter),t) = maxPipesT(t,diameter) ;
...
* Increased upper bound from 99 to 999 to accomodate deep decarbonization cases
WELLNUM.up(InjSal,t) = min(999,MaxWells(InjSal)) ;
...
PIPED.up(link(FromNode,ToNode,diameter),t) $= PipeMax(diameter)*maxPipesT(t,diameter);
...
PipeReady.up(link(FromNode,ToNode,diameter),t) = maxPipesT(t,diameter) ;
*PipeReady.up(link(FromNode,ToNode,'36'),t)     = maxPipes ;

NEWSINK.up(InjSal,t) = max(1,ROUND(999*WellMax(InjSal)/SinkProps(InjSal,'SiteMax_MMT'),0)) ;
...
*** ECP RETROFITS ---- When set to 1, force retrofit according to ECP solution
   If( RunDefn('ECP_Retrofits')= 1,
      ...
      NEWSOURCE.fx(Cem_Source_exs,t)$(t.val=I_RY(Cem_Source_exs)) = 1 ;
      ...
      BLDPIPE.up(link(plant,ToNode,diameter),t)$(sum(Cem_Source_exs$(unitplt(Cem_Source_exs,plant) and (t.val>=I_RY(Cem_Source_exs))), 1)>0) = 1 ;
      PIPED.up(link(plant,ToNode,diameter),t) = BLDPIPE.up(plant,ToNode,diameter,t)*PipeMax(diameter);
   );
...
*** NEW BUILDS ---- When set to 1, force new builds on in the allocated year; otherwise, ignore new builds
   If( RunDefn('ECP_NewBuilds')= 1,
     ...
     NEWSOURCE.fx(HMM_Source,t)$(t.val=N_RY(HMM_Source)) = 1;
     BLDPIPE.up(link(plant,ToNode,diameter),t)$(sum(HMM_Source$(unitplt(HMM_Source,plant) and (t.val>=N_RY(HMM_Source))), 1)>0) = 1 ;
     NEWSOURCE.fx(Cem_Source_new,t)$(t.val=N_RY(Cem_Source_new)) = 1;
     BLDPIPE.up(link(plant,ToNode,diameter),t)$(sum(Cem_Source_new$(unitplt(Cem_Source_new,plant) and (t.val>=N_RY(Cem_Source_new))), 1)>0) = 1 ;
     PIPED.up(link(plant,ToNode,diameter),t) = BLDPIPE.up(plant,ToNode,diameter,t)*PipeMax(diameter);
   );
$label next4
...
* Objective Function
PipeLength(t)..      TOTPipeLength(t)  =E= sum(link(FromNode,ToNode,diameter), BLDPIPE(link,t)*LinkDist(FromNode,ToNode));
...
* 999 is a Big M for the most wells that will be drilled at a sink in a year
SinkWellBalance(InjSal,t)..     WELLNUM(InjSal,t) - min(999,MaxWells(InjSal)) * SinkReady(InjSal,t) =L= 0;
...
cts.optcr=0.001;
set        slv   Solve number /1_SLV*2_SLV/
parameter  CTSSolveTime(slv)   Model solve time
;
* Use pipe filters to figure out how much solve time remains
$Ifi %POPT% == 1 $goto POPT1
$gdxin %dir%ctsmodel.gdx
$load CTSSolveTime
$gdxin
$Ifi %POPT% == 2 $goto POPT2
$goto POPT3
$label POPT2
*cts.optcr=0.01;

* resolution limit in seconds (10800 = 3 hours; 5400 = 1.5 hours)
cts.reslim = max(1800,3600 - sum(slv,CTSSolveTime(slv))) ;
...
parameter slack_prop(t)
          sol_wt;

slack_prop(t)$totCaptureMax(t) = 1-sum(source, CaptureSlack.l(source,t))/totCaptureMax(t);
sol_wt = sum(t, slack_prop(t)*totCaptureMax(t))/sum(t, totCaptureMax(t));

* Use pipe filters to save the solve time
$Ifi %POPT% == 1 $goto SLV1
$Ifi %POPT% == 2 $goto SLV2
$Ifi %POPT% == 3 $goto SLV3
$label SLV1
CTSSolveTime('1_SLV') = cts.resUsd;
$goto SLV3
$label SLV2
CTSSolveTime('2_SLV') = cts.resUsd;
$label SLV3

Execute_Unload "ctsmodel.gdx";
