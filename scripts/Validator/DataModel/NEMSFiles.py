...
class Ctuslog(BaseFile):
    def __init__(self, path):
        super(Ctuslog, self).__init__(path=path)
    ...
    def scan_directory(self):
        for file in os.listdir(self.path):
            if fnmatch.fnmatch(file, "nohup.out"):
                ...
                self.ctuslog=Ctuslog(path=os.path.join(self.path, file))
...
class NEMSFiles(BaseFolder):
    ...
    def scan_directory(self):
        ...
        with open(lg, 'a') as f:
            ...
            p = f'{self.path}\\Validator_input\\validator_controller.csv'
