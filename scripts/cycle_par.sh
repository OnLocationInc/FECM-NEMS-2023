...
#       If using the new criteria, set the old criteria (keepgoing) outside of the acceptable range.
      ...
      if [ $returncode -eq 0 -a $nruns -gt 1 ] ; then
         ...
         cp intercvout.txt intercvout.$n.txt
...
# files is intended to speed solution time.
      ...
      if [ $n -lt $nruns ] ; then
        #================================ p1 ======================================================================        
        if [ -f RESTART.unf ] ; then cp RESTART.unf p1/RESTART.IN ;  fi
        if [ -f RESTART.txt ] ; then cp RESTART.txt p1/RESTART.IN ;  fi
        if [ -f RESTART.rlx ] ; then cp RESTART.rlx p1/RESTART.IN ;  fi
        cd p1
        if [ -f BASPMM1.dat ] ; then cp BASPMM1.dat baspmm2 ;  fi
        if [ -f BAXPMM1.dat ] ; then cp BAXPMM1.dat baxpmm2 ;  fi
        if [ -f IMPCURV.txt ] ; then cp IMPCURV.txt SPRFLRT ; fi
        if [ -f COALUNITO.txt ] ; then cp COALUNITO.txt COALUNITS.txt ;  fi
        cat bas[1-2][0-9][0-9][0-9].dat > basemmi 
        cat efd[1-2][0-9][0-9][0-9].dat > basefdi 
        cat cmm[1-2][0-9][0-9][0-9].dat > bascmmi 
        cat baspmm*.da > baspmm2 
        cat baxpmm*.da > baxpmm2 
        cat nofiles* > collectnofiles
        if [ -f epmout.txt ]  ; then cp epmout.txt epmout.$n.txt ; fi
        if [ -f restart.unf ]  ; then cp restart.unf restart.$n.unf ;  fi
        if [ -f MAINDBUG ] ; then cp MAINDBUG MAINDBUG.$n ;  fi
        if [ -f MNPQIT.daf ] ; then cp MNPQIT.daf MNPQIT.$n.daf ;  fi
        if [ -f MC_COMMON.csv ] ; then cp MC_COMMON.csv MC_COMMON.$n.csv ;  fi
        if [ -f INTLDBG.txt ] ; then cp INTLDBG.txt ACT_INTLDBG.txt ;  fi
        if [ -f MNEXPECT.txt ] ; then cp MNEXPECT.txt MNEXPECT.$n.txt ;  fi
        if [ -f ctusALL.gdx ]  ; then cp ctusALL.gdx ctusALL.$n.gdx ;  fi
#  These two are also done in hswrk.sh
        if [ -f COAL1CPS ]  ; then rm -f  COAL1CPS     ;   fi
        if [ -f EDN0SLC ]   ; then rm -f  EDN0SLC      ;   fi
        cd ..
        #================================ p2 ======================================================================        
        if [ -f RESTART.unf ] ; then cp RESTART.unf p2/RESTART.IN ;  fi
        if [ -f RESTART.txt ] ; then cp RESTART.txt p2/RESTART.IN ;  fi
        if [ -f RESTART.rlx ] ; then cp RESTART.rlx p2/RESTART.IN ;  fi
        cd p2
        if [ -f BASPMM1.dat ] ; then cp BASPMM1.dat baspmm2 ;  fi
        if [ -f BAXPMM1.dat ] ; then cp BAXPMM1.dat baxpmm2 ;  fi
        if [ -f IMPCURV.txt ] ; then cp IMPCURV.txt SPRFLRT ;  fi
        ...
        if [ -f INTLDBG.txt ] ; then cp INTLDBG.txt ACT_INTLDBG.txt ;  fi
        if [ -f MNEXPECT.txt ] ; then cp MNEXPECT.txt MNEXPECT.$n.txt ;  fi
        if [ -f EDBPGRP.txt ] ; then cp EDBPGRP.txt EDBPGRP.$n.txt ;  fi
        if [ -f ctusALL.gdx ]  ; then cp ctusALL.gdx ctusALL.$n.gdx ;  fi
#  These two are also done in hswrk.sh
        if [ -f COAL1CPS ]  ; then rm -f  COAL1CPS     ;   fi
        if [ -f EDN0SLC ]   ; then rm -f  EDN0SLC      ;   fi
...
# run validation
      egrep -i "nems run [1-9]|abort|infeas|Returncode|MAIN:|hr:min:sec|instances|GPA|ATTN|ERR:" nohup.out > infeas.txt
      
      if [ "$PCoutdir" != "$PCworkdir" ] ; then
         if [ -n $outdir ] ; then
            cp fort.$n.20 $outdir/.
            cp $scenario.$n.${datecode:1:4}${datecode:7:2}.xml $outdir/.
...
# get option to run a final, additional p1 run so that LFMM can incorporate final AB32 and other allowance prices 
 ...
 # create ran files for nth cycle before final p1 cycle
      if [ -f restart.unf ] ; then
         cp restart.unf restart.$nth.unf
         echo "Running ftab for cycle $nth:  " `date +%T`
         cp ftab.dat ftab.$nth.dat
         ls -al ftab.$nth.dat
         ...
         mv $scenario.${datecode:1:4}${datecode:7:2}.api.txt $scenario.$nth.${datecode:1:4}${datecode:7:2}.api.txt
       ...
       cd p1
       if [ -f epmout.txt ]  ; then cp epmout.txt epmout.$nth.txt ;  fi
       if [ -f restart.unf ]  ; then cp restart.unf restart.$nth.unf ;  fi
       if [ -f MAINDBUG ] ; then cp MAINDBUG MAINDBUG.$nth ;  fi
       if [ -f MNPQIT.daf ] ; then cp MNPQIT.daf MNPQIT.$nth.daf ;  fi
       if [ -f MC_COMMON.csv ] ; then cp MC_COMMON.csv MC_COMMON.$nth.csv ;  fi
       if [ -f INTLDBG.txt ] ; then cp INTLDBG.txt ACT_INTLDBG.txt ;  fi
       if [ -f MNEXPECT.txt ] ; then cp MNEXPECT.txt MNEXPECT.$nth.txt ;  fi
       if [ -f NEM_TO_LFMM1.gdx ] ; then cp  NEM_TO_LFMM1.gdx NEM_TO_LFMM1.$nth.gdx ;  fi
       if [ -f LFMM_TO_NEMS.gdx ] ; then cp  LFMM_TO_NEMS.gdx LFMM_TO_NEMS.$nth.gdx ;  fi
...
if [ -f indusa.csv ] ; then
  ...
  sh $NEMS/scripts/trim.sh ICHEM1.csv   temp.csv;mv temp.csv  ICHEM1.csv  
  sh $NEMS/scripts/trim.sh ICHEM2.csv   temp.csv;mv temp.csv  ICHEM2.csv  
  sh $NEMS/scripts/trim.sh ICHEM3.csv   temp.csv;mv temp.csv  ICHEM3.csv  
  sh $NEMS/scripts/trim.sh ICHEM4.csv   temp.csv;mv temp.csv  ICHEM4.csv  
  sh $NEMS/scripts/trim.sh ICHEMUSA.csv temp.csv;mv temp.csv  ICHEMUSA.csv
fi
