...
export TMPDIR
#sh $NEMS/scripts/record_mks_use.sh
...
alias    analyze="sh n:/default/analyze/analyze.s" ; export analyze
...
alias  gpa="egrep 'GPA \(run #|4 is bad' nohup.out"
...
alias  cremmdb="sh $NEMS/scripts/cremmdb.sh"      
#        create geothermal debug files from WDUMP.txt
alias  geodbug="sh $NEMS/scripts/geodbug.sh"
#        create hydro debug files from WDUMP.txt
alias  hyddbug="sh $NEMS/scripts/hyddbug.sh"
#        compile udbp code and create udbp executable
alias  compudbp="sh $NEMS/scripts/udbp.compile.sh"
