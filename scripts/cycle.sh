...
# files is intended to speed solution time.
      ...
      if [ $n -lt $nruns ] ; then
        if [ -f RESTART.unf ] ; then cp RESTART.unf RESTART.IN ;  fi
        if [ -f RESTART.txt ] ; then cp RESTART.txt RESTART.IN ;  fi
        if [ -f RESTART.rlx ] ; then cp RESTART.rlx RESTART.IN ;  fi
...
if [ -f indusa.csv ] ; then
  ...
  $NEMS/scripts/trim.sh ICHEM1.csv   temp.csv;mv temp.csv  ICHEM1.csv  
  $NEMS/scripts/trim.sh ICHEM2.csv   temp.csv;mv temp.csv  ICHEM2.csv  
  $NEMS/scripts/trim.sh ICHEM3.csv   temp.csv;mv temp.csv  ICHEM3.csv  
  $NEMS/scripts/trim.sh ICHEM4.csv   temp.csv;mv temp.csv  ICHEM4.csv  
  $NEMS/scripts/trim.sh ICHEMUSA.csv temp.csv;mv temp.csv  ICHEMUSA.csv
fi
...
./Validator/run_validator.bat ${NEMSPYENV:5} > validator.log 2>valiatorDebug.log
